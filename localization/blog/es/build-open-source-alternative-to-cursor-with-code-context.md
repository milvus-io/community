...
<p>Pídele a Claude Code que busque "dónde gestiona este proyecto la autenticación de usuarios", y vuelve a <code translate="no">grep -r &quot;auth&quot;</code>-escupiendo 87 coincidencias vagamente relacionadas entre comentarios, nombres de variables y nombres de archivos, probablemente omitiendo muchas funciones con lógica de autenticación pero que no se llaman "auth". Prueba Gemini CLI, y buscará palabras clave como "login" o "password", omitiendo por completo funciones como <code translate="no">verifyCredentials()</code>. Estas herramientas son geniales para generar código, pero cuando llega el momento de navegar, depurar o explorar sistemas desconocidos, se vienen abajo. A menos que envíen toda la base de código al LLM para contextualizarla (quemando tokens y tiempo), tienen dificultades para proporcionar respuestas significativas.</p>

...
<li><p><strong>La memoria</strong> es lo que persiste a lo largo de las sesiones. Vive en el disco local: el historial completo de conversaciones anteriores, los archivos con los que ha trabajado el agente y las preferencias del usuario. No está resumido. Sin comprimir. El material en bruto.</p></li>
</ul>
...
<p>¿El único inconveniente? Ahora mismo este sistema de memoria Markdown-first está estrechamente entrelazado con todo el ecosistema OpenClaw: el proceso Gateway, los conectores de plataforma, la configuración del espacio de trabajo y la infraestructura de mensajería. Si sólo quieres el modelo de memoria, hay mucha maquinaria que arrastrar.</p>
...
<h3 id="Readable-Markdown-Makes-Debugging-as-Simple-as-Editing-a-File" class="common-anchor-header">Markdown legible hace que la depuración sea tan sencilla como editar un archivo</h3><p>La depuración de la memoria de la IA suele ser dolorosa. Cuando un agente produce una respuesta errónea, la mayoría de los sistemas de memoria no ofrecen una forma clara de ver <em>lo que</em> realmente ha almacenado. El flujo de trabajo típico consiste en escribir código personalizado para consultar una API de memoria y, a continuación, escudriñar incrustaciones opacas o ampulosos blobs JSON, ninguno de los cuales dice mucho sobre el estado interno real de la IA.</p>
...
<li><p><strong>Cambia de modelo de incrustación:</strong> Vuelve a ejecutar el comando de indexación. Tardará cinco minutos, y los archivos markdown permanecen intactos.</p></li>
...
<p><strong>La fragmentación</strong> divide el texto a lo largo de los límites semánticos -encabezados y cuerpo- para que el contenido relacionado permanezca unido. Esto evita casos en los que una frase como "configuración de Redis" se divide en varios chunks.</p>

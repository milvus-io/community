...
<p>A alto nivel, una consulta ChatGPT completa consta de las siguientes capas, ordenadas de la más global a la más inmediata:</p>
...
<ul>
<li><p><strong>Actualizaciones implícitas</strong>: el sistema analiza periódicamente el contenido de las conversaciones y actualiza las memorias en segundo plano. Estas actualizaciones no se aplican en tiempo real, y las memorias asociadas a conversaciones borradas se van eliminando gradualmente como parte de la optimización en curso.</p></li>
...
<ul>
<li><p>La<strong>recuperación no es automática</strong>: Las llamadas a las herramientas se activan por el propio criterio del modelo. Por ejemplo, cuando un usuario menciona <em>"el proyecto que discutimos la última vez",</em> Claude puede decidir invocar <code translate="no">conversation_search</code> para recuperar el contexto relevante.</p></li>
...
<p>Si el planteamiento de Claude se hubiera intentado en 2020, probablemente habría sido inviable. En aquella época, las bases de datos vectoriales solían tener una latencia de cientos de milisegundos, las consultas híbridas no estaban bien soportadas y el uso de recursos aumentaba de forma prohibitiva a medida que crecían los datos. En esas condiciones, la recuperación bajo demanda se habría considerado un exceso de ingeniería.</p>

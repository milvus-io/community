...
<p>Así que construí un agente con OpenClaw que vigila el mercado en mi lugar y me impide cometer los mismos errores. No negocia ni toca mi dinero; evita que haga algo de lo que ya me he arrepentido una vez.</p>
<p>Este agente consta de tres partes y cuesta unos 20 dólares al mes:</p>
<ul>
<li><strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">OpenClaw</a></strong> <strong>para ejecutarlo todo en piloto automático.</strong> OpenClaw ejecuta el agente con un ritmo cardíaco de 30 minutos y sólo me avisa cuando algo realmente importa, lo que alivia el FOMO que solía mantenerme pegado a la pantalla. Antes, cuanto más miraba los precios, más reaccionaba por impulso.</li>
...
<h2 id="Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="common-anchor-header">Paso 1: Obtenga inteligencia de mercado en tiempo real con Exa<button data-href="#Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Antes, había probado a navegar por aplicaciones financieras, escribir scrapers y buscar en terminales de datos profesionales. Las aplicaciones enterraban la señal bajo el ruido, los scrapers se rompían constantemente y las API profesionales tienen precios para operadores institucionales. Exa es una API de búsqueda construida para agentes de IA que resuelve los problemas anteriores.</p>
<p>
...
<p><strong><a href="https://exa.ai/">Exa</a></strong> es una API de búsqueda web que devuelve datos estructurados y listos para la IA. Está impulsada por <a href="https://zilliz.com/cloud">Zilliz Cloud</a> (el servicio totalmente gestionado de Milvus).  Si Perplexity es un motor de búsqueda utilizado por los humanos, Exa es utilizado por la IA. El agente envía una consulta y Exa devuelve el texto del artículo, las frases clave y los resúmenes en formato JSON, una salida estructurada que el agente puede analizar y sobre la que puede actuar directamente, sin necesidad de raspado.</p>
...
<h2 id="Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="common-anchor-header">Paso 2: Almacenar el historial de operaciones en Milvus para tomar decisiones más inteligentes<button data-href="#Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Exa solucionó mi problema de información. Pero seguía repitiendo las mismas operaciones: ventas de pánico durante las caídas que se recuperaban a los pocos días, persiguiendo el impulso en valores que ya estaban sobrevalorados. Actuaba por emoción, me arrepentía y olvidaba la lección cuando se presentaba una situación similar.</p>
<p>Necesitaba una base de conocimiento personal: mis operaciones anteriores, mi razonamiento, mis meteduras de pata. No algo que tuviera que revisar manualmente (lo había intentado y nunca lo conseguí), sino algo que el agente pudiera buscar por sí mismo cada vez que se presentara una situación similar. Si estoy a punto de repetir un error, quiero que el agente me lo diga antes de pulsar el botón. Emparejar la "situación actual" con la "experiencia pasada" es un problema de búsqueda de similitudes que resuelven las bases de datos vectoriales, así que todos mis datos tenían que almacenarse en una.</p>
<p>Utilicé <a href="https://github.com/milvus-io/milvus-lite">Milvus Lite</a>, una versión ligera de Milvus que se ejecuta localmente sin configuración de servidor. Es perfecta para crear prototipos y experimentar. Dividí mis datos en tres colecciones. La dimensión de incrustación es 1536 para que coincida con el modelo text-embedding-3-small de OpenAI, que se puede utilizar directamente:</p>
...
<p>Mezclar estos datos en una sola colección supondría llenar cada consulta con historial de operaciones irrelevante o perder los sesgos principales cuando no coincidan lo suficiente con la consulta actual.</p>
<p>Una vez que existan las colecciones, el agente debe rellenarlas automáticamente. No quiero escribir notas después de cada conversación, así que he creado un extractor de memoria que se ejecuta al final de cada sesión de chat.</p>
<p>Hace dos cosas: extraer y deduplicar. El extractor pide al LLM que extraiga información estructurada de la conversación -decisiones, preferencias, patrones, lecciones- y dirige cada una de ellas a la colección adecuada. Antes de almacenar nada, comprueba la similitud con lo que ya existe. Si una nueva información es más de un 92% similar a otra ya existente, se omite.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Por ejemplo, cuando las acciones tecnológicas cayeron entre un 3% y un 4% por las tensiones en Oriente Medio a principios de marzo, el agente sacó tres cosas: una lección de octubre de 2024 sobre no vender por pánico durante una caída geopolítica, una nota de preferencia de que tiendo a sobreponderar el riesgo geopolítico, y un patrón que había registrado de que las ventas tecnológicas impulsadas por la geopolítica suelen recuperarse en una a tres semanas.</p>
<p>Mi compañero de trabajo lo llamó "aprendizaje por refuerzo para inversores minoristas", en el que yo soy el modelo que se entrena con mi propia función de pérdida. Me parece justo. Al menos ahora hay un bucle de retroalimentación entre lo que hice y lo que voy a hacer.</p>
<h2 id="Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="common-anchor-header">Paso 3: Enseñe a su agente a analizar con OpenClaw Skills<button data-href="#Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="anchor-icon" translate="no">
    ...
    </button></h2><p>En este punto el agente tiene buena información<a href="https://exa.ai/">(Exa</a>) y memoria personal<a href="https://github.com/milvus-io/milvus-lite">(Milvus</a>). Pero si le das ambas cosas a un LLM y le dices "analiza esto", obtendrás una respuesta genérica, de cobertura de todo. Menciona todos los ángulos posibles y concluye con "los inversores deben sopesar los riesgos", es decir, nada.</p>
<p>La solución consiste en escribir tu propio marco analítico y dárselo al agente en forma de instrucciones explícitas. Qué indicadores te importan, qué situaciones consideras peligrosas, cuándo ser conservador frente a agresivo. Estas reglas son diferentes para cada inversor, así que tienes que definirlas tú mismo.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>La última línea es la más importante: "Siempre afloran mis errores pasados. Tengo tendencia a dejar que el miedo anule los datos. Si mi historial de Milvus muestra que me arrepentí de vender después de una caída, dígalo explícitamente". Es un mecanismo de corrección creado específicamente para mi psicología. Su versión codificaría sus propias tendencias.</p>
...
<h2 id="Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="common-anchor-header">Paso 4: Inicie su agente con OpenClaw Heartbeat<button data-href="#Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Todo lo anterior aún requiere que lo active manualmente. Si tiene que abrir un terminal cada vez que desee una actualización, volverá a la casilla de salida y probablemente volverá a hacer doomscrolling en su aplicación de corretaje durante las reuniones.</p>
<p>El mecanismo Heartbeat de OpenClaw soluciona este problema. Una pasarela hace un ping al agente cada 30 minutos (configurable), y el agente comprueba un archivo HEARTBEAT.md para decidir qué hacer en ese momento. Es un archivo markdown con reglas basadas en el tiempo:</p>
...
<p>El coste total: <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mi>10/mes</mi></mrow></semantics></math></span></span>por <span class="katex"><span class="katex-html" aria-hidden="true"><span class="base"><span class="mord">OpenClaw</span></span></span><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mo separator="true">,</mo></mrow></semantics></math></span></span>10/mes <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><annotation encoding="application/x-tex">por OpenClaw,</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1em;vertical-align:-0.25em;"></span><span class="mord">10/mes por</span><span class="mord mathnormal" style="margin-right:0.02691em;">OpenClaw</span><span class="mpunct">,</span></span></span></span>10/mes por Exa, y un poco de electricidad para mantener Milvus Lite en funcionamiento.</p>
...
<ul>
<li><strong>Recopila automáticamente noticias fiables del mercado</strong> <strong>mediante</strong> <strong><a href="https://exa.ai/">Exa</a></strong>, de modo que empiezo cada mañana con un informe limpio en lugar de una hora de análisis catastrofista.</li>
<li><strong>Recuerda mis operaciones anteriores con <a href="https://milvus.io/">Milvus</a> y me avisa</strong> cuando estoy a punto de repetir un error, utilizando Milvus como base de datos personal de vectores.</li>
<li><strong>Funciona</strong> <strong>con el piloto automático</strong> <strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">de OpenClaw</a></strong> <strong>y sólo me avisa cuando es necesario</strong>, utilizando OpenClaw Skills y Heartbeat, sin cron jobs ni código adhesivo.</li>
</ul>
<p>Coste total: 20 dólares al mes. El agente no negocia ni toca mi dinero.</p>

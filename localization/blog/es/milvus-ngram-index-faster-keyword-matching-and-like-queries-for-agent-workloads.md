...
<ul>
<li><p><strong>Construir el índice:</strong> Generar n-gramas para cada documento y construir un índice invertido durante la ingestión de datos.</p></li>
...
<p>La subcadena literal es más corta que <code translate="no">min_gram</code> y, por tanto, no puede descomponerse en n-gramas indexados. En este caso, no se puede utilizar el índice Ngram y Milvus vuelve a la ruta de ejecución por defecto, evaluando la condición <code translate="no">LIKE</code> mediante un escaneo completo con coincidencia de patrones.</p>

...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Storing-Embeddings-in-Milvus" class="common-anchor-header">在 Milvus 中存储嵌入数据</h3><p>Milvus 将数据组织成 Collections，每个 Collections 由 Schema 定义。在这里，每条记录都会存储原始文本块及其嵌入向量：</p>
...
<li><strong>将知识库向量化。</strong>RustFS 文档（80 个 Markdown 文件）经过分块、文本嵌入（text-embedding-3-large），以 466 个向量的形式存储在 Milvus 中。</li>
...
<li><strong><a href="https://rustfs.com/">RustFS</a></strong> <strong>作为对象存储后端，使用起来非常愉快。</strong>在 MinIO 中使用它所花的精力比预期的要少。</li>

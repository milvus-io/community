...
<h3 id="What-is-vector-similarity-search" class="common-anchor-header">什么是向量相似性搜索？</h3><p>向量<a href="https://medium.com/unstructured-data-service/vector-similarity-search-hides-in-plain-view-654f8152f8ab">相似性搜索</a>的应用横跨人工智能、深度学习和传统向量计算等多种场景。人工智能技术的普及部分归功于向量搜索及其对非结构化数据的理解能力，其中包括图像、视频、音频、行为数据、文档等。</p>
...
<p>传统上，非结构化数据由于不遵循预定义的模型或组织结构，一直是大规模处理和分析的难题。神经网络（如 CNN、RNN 和 BERT）使得将非结构化数据转换为特征向量成为可能，特征向量是一种数字数据格式，可以很容易地被计算机解读。然后使用算法，利用余弦相似度或欧氏距离等指标计算向量之间的相似性。</p>

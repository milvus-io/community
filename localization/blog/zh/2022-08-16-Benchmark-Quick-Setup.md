...
<ul>
<li><p><code translate="no">benchmark</code> 和<code translate="no">benchmark-mac</code> 是使用 Go SDK 2.1.1 开发和编译的可执行文件。它们仅用于进行并发搜索。</p></li>
...
<button class="copy-code-btn"></button></code></pre>
<ol start="7">
<li>打开当前目录下的<code translate="no">go_log_file.log</code> 文件，查看详细的搜索日志。以下是搜索日志中的搜索信息。<ul>

...
<p>于是，我用 OpenClaw 建立了一个 Agents，代替我观察市场，阻止我犯同样的错误。它不进行交易，也不碰我的钱，只是防止我做已经后悔过一次的事。</p>
<p>这个 Agents 由三部分组成，费用约为 20 美元/月：</p>
<ul>
<li><strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">OpenClaw</a></strong> <strong>，用于自动运行这一切。</strong>OpenClaw 每 30 分钟心跳一次运行代理，只有在真正重要的事情发生时才会呼叫我，这缓解了过去让我目不转睛盯着屏幕的 FOMO。以前，我越是关注价格，就越会做出冲动的反应。</li>
...
<h2 id="Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="common-anchor-header">第一步：使用 Exa 获取实时市场情报<button data-href="#Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在此之前，我尝试过浏览金融应用程序、编写刮板程序以及研究专业数据终端。应用程序将信号掩盖在噪音之下，刮擦程序经常出现故障，而专业 API 的价格是针对机构交易者的。Exa 是专为人工智能 Agents 打造的搜索 API，可以解决上述问题。</p>
<p>
...
<p><strong><a href="https://exa.ai/">Exa</a></strong>是一个网络搜索 API，可返回结构化的人工智能就绪数据。它由<a href="https://zilliz.com/cloud">Zilliz Cloud</a>（Milvus 的全托管服务）提供支持。  如果说 Perplexity 是人类使用的搜索引擎，那么 Exa 则是人工智能使用的搜索引擎。代理发送查询，Exa 会以 JSON 格式返回文章文本、关键句和摘要--结构化输出，代理可以直接解析并采取行动，无需刮擦。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>内容参数完成了大部分繁重的工作--文本提取全文，高亮显示关键句子，摘要则根据你提供的问题生成重点摘要。调用一次应用程序接口就能代替二十分钟的标签页跳转。</p>
...
<h2 id="Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="common-anchor-header">第二步：在 Milvus 中存储交易历史，做出更明智的决策<button data-href="#Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Exa 解决了我的信息问题。但我仍在重复同样的交易--在跌势中恐慌性抛售，但几天后就恢复了；追逐势头，买入价格已经过高的股票。我凭情绪行事，一旦遇到类似情况就后悔莫及，忘记了教训。</p>
<p>我需要一个个人知识库：我过去的交易、我的推理、我的失误。这不是我必须手动回顾的东西（我试过，但从未坚持下来），而是每当出现类似情况时，Agent 可以自行搜索的东西。如果我即将重蹈覆辙，我希望 Agents 能在我按下按钮之前告诉我。将 "当前情况 "与 "过去经验 "进行匹配是向量数据库可以解决的相似性搜索问题，因此我的所有数据都必须存储在一个数据库中。</p>
<p>我使用了<a href="https://github.com/milvus-io/milvus-lite">Milvus Lite</a>，它是 Milvus 的轻量级版本，可在本地运行，无需服务器设置。它非常适合原型设计和实验。我将数据分成了三个 Collections。嵌入维度为 1536，以匹配 OpenAI 的文本嵌入-3-小模型，可以直接使用：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>这三个 Collections 映射为三种类型的个人数据，每种数据都有不同的检索策略：</p>
<table>
<thead>
...
<tr><td><strong>外部知识</strong></td><td>研究报告、美国证券交易委员会文件、公共数据</td><td>不存储于 Milvus - 可通过 Exa 搜索</td></tr>
</tbody>
</table>
<p>将这些内容混合到一个 Collections 中，意味着要么用不相关的交易历史来充实每个提示，要么在与当前查询不够匹配时丢失核心偏差。</p>
<p>一旦 Collection 存在，Agent 就需要自动填充它们。我不想在每次对话后都写笔记，所以我创建了一个记忆提取器，在每次聊天会话结束时运行。</p>
<p>它会做两件事：提取和重复。提取器要求 LLM 从对话中提取结构化的见解--决定、偏好、模式、经验教训--并将每条见解路由到正确的 Collections。在存储任何内容之前，它会对照已有内容检查相似性。如果新见解与现有条目相似度超过 92%，就会被跳过。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>例如，3 月初中东紧张局势导致科技股下跌 3-4%，Agents 调用了三件事：2024 年 10 月关于在地缘政治下跌期间不要恐慌性抛售的教训、我倾向于超配地缘政治风险的偏好说明，以及我记录的一种模式，即地缘政治导致的科技股大跌通常会在一到三周内恢复。</p>
<p>我的同事称这是 "散户投资者的强化学习"，我是根据自己的损失函数训练出来的模型。很公平。至少现在我做了什么和我将要做什么之间有了一个反馈回路。</p>
<h2 id="Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="common-anchor-header">第三步：教你的 Agents 使用 OpenClaw Skills 进行分析<button data-href="#Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>此时，Agent 拥有良好的信息<a href="https://exa.ai/">（Exa</a>）和个人记忆<a href="https://github.com/milvus-io/milvus-lite">（Milvus</a>）。但是，如果你把这两样东西都交给一个法学硕士，然后说 "分析一下这个"，你会得到一个通用的、对冲一切的回复。它提到了每一个可能的角度，并以 "投资者应权衡风险 "作为结论，换句话说，什么都没有。</p>
<p>解决的办法是编写自己的分析框架，并将其作为明确的指令交给 Agents。你关心哪些指标，你认为哪些情况是危险的，什么时候应该保守，什么时候应该激进。这些规则对每个投资者来说都是不同的，所以你必须自己定义它们。</p>
<p>OpenClaw通过 "技能"（skills/目录下的标记文件）来实现这一点。当 Agents 遇到相关情况时，它会加载匹配的 Skill，并遵循你的框架，而不是自由发挥。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>最后一行是最重要的："总是浮现我过去的错误。我有一种让恐惧凌驾于数据之上的倾向。如果我的 Milvus 历史记录显示我在下跌后后悔卖出，那就明确地说出来。"这是专门针对我的心理建立的纠正机制。你的版本将编码你自己的倾向。</p>
<p>我还为情绪分析、宏观指标和行业轮动信号编写了类似的 "技能"。我还编写了模拟我欣赏的投资者如何评估相同情况的 "技能"--巴菲特的价值框架、桥水的宏观方法。这些不是决策者，而是额外的视角。</p>
...
<h2 id="Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="common-anchor-header">第 4 步：使用 OpenClaw Heartbeat 启动 Agents<button data-href="#Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="anchor-icon" translate="no">
    ...
    </button></h2><p>以上所有步骤都需要手动触发。如果每次需要更新时都要打开终端，那就又回到了起点--可能又要在开会时滚动你的经纪人应用程序了。</p>
<p>OpenClaw 的心跳机制解决了这个问题。网关每 30 分钟（可配置）ping 一次 Agents，Agent 会检查 HEARTBEAT.md 文件，以决定此时该做什么。这是一个基于时间规则的标记文件：</p>
...
<ul>
<li><strong>早晨简报（上午 7:00）。</strong>Agents 过夜运行 Exa，从 Milvus 中提取我的仓位和相关历史记录，然后向我的手机推送一份个性化摘要--少于 500 字。一夜之间发生了什么，与我的持股有什么关系，以及一到三个行动项目。我一边刷牙一边阅读。</li>
<li><strong>日内提醒（东部时间上午 9:30 至下午 4:00）。</strong>每隔 30 分钟，Agent 会检查我的观察列表。如果有股票变动超过 3%，我就会收到通知，并说明相关情况：我为什么买入它，我的止损点在哪里，以及我以前是否遇到过类似情况。</li>
...
<ul>
<li><strong>使用</strong> <strong><a href="https://exa.ai/">Exa</a></strong><strong>自动收集可靠的市场新闻</strong>，这样我每天早上就能在简明扼要的简报中开始一天的工作，而不是花上一个小时的时间来滚滚向前。</li>
<li>使用<strong> <a href="https://milvus.io/">Milvus</a></strong>作为个人向量数据库<strong>，记住我过去的交易，并</strong>在我即将重蹈覆辙时<strong>发出警告</strong>。</li>
<li>使用<strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">OpenClaw</a></strong>Skills 和 Heartbeat，在 OpenClaw<strong>自动驾驶仪</strong><strong>上运行</strong> <strong>，只在关键时刻鸣叫我</strong>--没有 cron jobs，没有胶水代码。</li>
</ul>
<p>总成本：20 美元/月。Agents 不进行交易，也不碰我的钱。</p>
<p>最大的变化不是数据或警报。而是我不再关注市场了。上周三我完全忘记了它，这在我多年的交易生涯中从未发生过。我有时还是会亏钱，但次数少了很多，而且我又开始享受周末了。我的同事们还没有更新这个笑话，但给它点时间吧。</p>

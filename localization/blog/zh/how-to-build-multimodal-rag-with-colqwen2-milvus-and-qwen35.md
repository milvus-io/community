...
title: 如何使用 ColQwen2、Milvus 和 Qwen3.5 构建多模式 RAG
author: Lumina Wang
date: 2026-3-6
...
desc: 使用 ColQwen2、Milvus 和 Qwen3.5 构建多模态 RAG 管道，检索 PDF 页面图像而非提取文本。分步教程。
origin: >-
  https://milvus.io/blog/how-to-build-multimodal-rag-with-colqwen2-milvus-and-qwen35.md
---
<p>如今，您可以将 PDF 文件上传到任何现代法律硕士学院，并就其提出问题。对于少数文件来说，这样做很好。但是，大多数 LLM 的上限是几百页的上下文，因此大量的语料根本无法满足要求。即使符合要求，您也要为处理每个查询的每一页付出代价。就同一个 500 页的文档集提出 100 个问题，你要为 500 页支付 100 倍的费用。这样一来，费用就变得非常昂贵。</p>
<p>检索增强生成（RAG）通过将索引与回答分离来解决这个问题。你只需对文档进行一次编码，将其表示法存储在向量数据库中，然后在查询时只检索最相关的页面发送给 LLM。该模型每次查询只读取三个页面，而不是整个语料库。因此，在不断增长的 Collections 上建立文档问答是非常实用的。</p>
<p>本教程将指导您使用三个公开授权的组件构建多模态 RAG 管道：</p>
<ul>
<li><strong><a href="https://huggingface.co/vidore/colqwen2-v1.0-merged">ColQwen2</a></strong> <a href="https://huggingface.co/vidore/colqwen2-v1.0-merged"></a>将每个 PDF 页面的图像编码为多向量嵌入，取代了传统的 OCR 和文本分块步骤。</li>
<li><strong><a href="http://milvus.io">Milvus</a></strong>存储这些向量，并在查询时处理相似性搜索，只检索最相关的页面。</li>
<li><strong><a href="https://qwen.ai/blog?id=qwen3.5">Qwen3.5-397B-A17B</a></strong>会读取检索到的页面图像，并根据看到的内容生成答案。</li>
</ul>
<p>最后，您将拥有一个工作系统，它能接收 PDF 和问题，找到最相关的页面，并根据模型所看到的内容返回答案。</p>
<h2 id="What-is-Multimodal-RAG" class="common-anchor-header">什么是多模态 RAG？<button data-href="#What-is-Multimodal-RAG" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>上文介绍了 RAG 在规模上的重要性。下一个问题是您需要什么样的 RAG，因为传统方法存在盲点。</p>
<p>传统的 RAG 从文档中提取文本，将其嵌入向量，在查询时检索最接近的匹配项，然后将这些文本块传递给 LLM。这对于格式简洁的大量文本内容非常有效。但当文档包含以下内容时，就会出现问题</p>
<ul>
<li>表格，其意义取决于行、列和标题之间的关系。</li>
<li>图表，信息完全可视化，没有对应的文本。</li>
<li>扫描文档或手写笔记，OCR 输出不可靠或不完整。</li>
</ul>
<p>多模态 RAG 以图像编码取代文本提取。您将每一页渲染为图像，用视觉语言模型对其进行编码，并在查询时检索页面图像。LLM 会看到原始页面，包括表格、数字、格式等所有内容，并根据所看到的内容作出回答。</p>
<p>
...
<h2 id="Structure-of-Multimodal-RAG-Pipeline-ColQwen2-for-Encoding-Milvus-for-Search-Qwen35-for-Generation" class="common-anchor-header">多模态 RAG 管道结构：用于编码的 ColQwen2、用于搜索的 Milvus、用于生成的 Qwen3.5<button data-href="#Structure-of-Multimodal-RAG-Pipeline-ColQwen2-for-Encoding-Milvus-for-Search-Qwen35-for-Generation" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><h3 id="How-the-Pipeline-Works-httpsassetszillizcomblogColQwen2MilvusQwen35397BA17B284c822b9efpng" class="common-anchor-header">管道如何工作  <span class="img-wrapper">
...
</h3><h3 id="Tech-Stack" class="common-anchor-header">技术栈</h3><table>
<thead>
<tr><th><strong>组件</strong></th><th><strong>选择</strong></th><th><strong>作用</strong></th></tr>
</thead>
<tbody>
<tr><td>PDF 处理</td><td>pdf2image + poppler</td><td>将 PDF 页面渲染为高分辨率图像</td></tr>
<tr><td>Embeddings 模型</td><td><a href="https://huggingface.co/vidore/colqwen2-v1.0-merged">colqwen2-v1.0</a></td><td>视觉语言模型；将每个页面编码为 ~755 128-dim 补丁向量</td></tr>
<tr><td>向量数据库</td><td><a href="https://milvus.io/">Milvus Lite</a></td><td>存储补丁向量并处理相似性搜索；本地运行，无需服务器设置</td></tr>
<tr><td>生成模型</td><td><a href="https://qwen.ai/blog?id=qwen3.5">Qwen3.5-397B-A17B</a></td><td>通过 OpenRouter API 调用多模态 LLM；读取检索到的页面图像生成答案</td></tr>
</tbody>
</table>
<h2 id="Step-by-Step-Implementation-for-Multi-Modal-RAG-with-ColQwen2+-Milvus+-Qwen35-397B-A17B" class="common-anchor-header">使用 ColQwen2+ Milvus+ Qwen3.5-397B-A17B 分步实现多模态 RAG<button data-href="#Step-by-Step-Implementation-for-Multi-Modal-RAG-with-ColQwen2+-Milvus+-Qwen35-397B-A17B" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<button class="copy-code-btn"></button></code></pre>
<ol start="3">
<li>下载嵌入模型 ColQwen2</li>
</ol>
<p>从 HuggingFace 下载 vidore/colqwen2-v1.0-merged（约 4.4 GB）并保存到本地：</p>
...
<button class="copy-code-btn"></button></code></pre>
<ol start="4">
<li>获取 OpenRouter API 密钥</li>
</ol>
<p>在<a href="https://openrouter.ai/settings/keys"></a><a href="https://openrouter.ai/settings/keys">https://openrouter.ai/settings/keys</a> 注册并生成密钥。</p>
<h3 id="Step-1-Import-Dependencies-and-Configure" class="common-anchor-header">第 1 步：导入依赖项并配置</h3><pre><code translate="no"><span class="hljs-keyword">import</span> os, io, base64
...
<button class="copy-code-btn"></button></code></pre>
<p>输出：设备：Cpu</p>
<h3 id="Step-2-Load-the-Embedding-Model" class="common-anchor-header">第 2 步：加载 Embeddings 模型</h3><p><strong>ColQwen2</strong>是一种视觉语言模型，可将文档图像编码为 ColBERT 风格的多向量表示。每个页面会产生几百个 128 维的补丁向量。</p>
...
<h3 id="Step-3-Initialize-Milvus" class="common-anchor-header">步骤 3：初始化 Milvus</h3><p>本教程使用 Milvus Lite，它以本地文件形式运行，零配置--无需单独的服务器进程。</p>
<p><strong>数据库 Schema：</strong></p>
<p><strong>id</strong>：INT64，自动递增主键</p>
<p><strong>doc_id</strong>：INT64，页码（PDF 的哪一页）</p>
<p><strong>patch_idx</strong>：INT64，该页中的补丁索引</p>
<p><strong>向量</strong>：FLOAT_VECTOR(128)，补丁的 128 维 Embeddings</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>输出：创建 Milvus Collections。</p>
<h3 id="Step-4-Convert-PDF-Pages-to-Images" class="common-anchor-header">第 4 步：将 PDF 页面转换为图像</h3><p>以 150 DPI 渲染每一页。这里不进行文本提取，而是将每一页纯粹作为图像处理。</p>
...
<h3 id="Step-5-Encode-Images-and-Insert-into-Milvus" class="common-anchor-header">第 5 步：编码图像并插入 Milvus</h3><p>ColQwen2 会将每一页编码为多向量补丁嵌入。然后，将每个补丁作为单独一行插入 Milvus。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>输出：编码 17 页，每页 ~755 个补丁，dim=128</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>输出：索引 17 页，共 12835 个补丁。</p>
<p>一份 17 页的 PDF 文件产生 12,835 条补丁矢量记录，每页大约有 755 个补丁。</p>
<h3 id="Step-6-Retrieve--Query-Encoding-+-MaxSim-Reranking" class="common-anchor-header">步骤 6：检索 - 查询编码 + MaxSim Rerankers</h3><p>这是核心检索逻辑。它分为三个阶段</p>
<p><strong>将查询编码</strong>为多个标记向量。</p>
<p>在<strong>Milvus 中搜索</strong>每个标记向量的最近补丁。</p>
<p>使用 MaxSim<strong>按页面进行汇总</strong>：对于每个查询令牌，在每个页面中提取得分最高的补丁，然后将所有令牌的得分相加。总分最高的页面即为最佳匹配。</p>
<p><strong>MaxSim 如何工作</strong>对于每个查询标记向量，您都要找到内积（MaxSim 中的 "最大"）最高的文档补丁。然后将所有查询标记中的这些最大得分相加，得出每页的总相关性得分。得分越高，说明查询与页面视觉内容之间的语义匹配度越高。</p>
...
<h3 id="Step-7-Generate-an-Answer-with-the-Multimodal-LLM" class="common-anchor-header">步骤 7：使用多模态 LLM 生成答案</h3><p>将检索到的页面图片（而非提取的文本）连同用户的问题一起发送给 Qwen3.5。LLM 会直接读取图像并生成答案。</p>
...
<h2 id="Conclusion" class="common-anchor-header">结论<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在本教程中，我们构建了一个多模态 RAG 管道，它可以获取 PDF 文件，将每一页转换为图像，使用 ColQwen2 将这些图像编码为多向量补丁嵌入，将其存储在 Milvus 中，并在查询时使用 MaxSim 评分检索最相关的页面。该管道将原始页面图像发送到 Qwen3.5，Qwen3.5 可以直观地读取这些图像并生成答案，而不是提取文本并希望 OCR 保留版面。</p>
<p>本教程只是一个起点，而非生产部署。在进一步学习时，有几点需要注意。</p>
<p>关于取舍：</p>
<ul>
<li><strong>存储量随页数增加而增加。</strong>每页产生约 755 个向量，因此 1,000 页的语料库意味着在 Milvus 中大约有 755,000 行。这里使用的 FLAT 索引适用于演示，但对于较大的 Collections，您需要 IVF 或 HNSW。</li>
<li><strong>编码比文本嵌入慢。</strong>ColQwen2 是一个 4.4 GB 的视觉模型。与嵌入文本块相比，每页图像的编码时间更长。对于运行一次的批量索引工作来说，这通常没有问题。对于实时摄取，则值得进行基准测试。</li>
<li><strong>这种方法最适合视觉效果丰富的文档。</strong>如果您的 PDF 文件大多是简洁的单栏文本，没有表格或数字，传统的基于文本的 RAG 可能会检索得更精确，运行成本也更低。</li>
</ul>
<p>下一步该怎么做？</p>
<ul>
<li><strong>换一种多模态 LLM。</strong>本教程通过 OpenRouter 使用 Qwen3.5，但检索管道与模型无关。你可以将生成步骤指向 GPT-4o、Gemini 或任何接受图像输入的多模态模型。</li>
<li><strong>扩展<a href="http://milvus.io">Milvus</a>。</strong>Milvus Lite 以本地文件的形式运行，非常适合原型开发。对于生产工作负载，Docker/Kubernetes 或 Zilliz Cloud（完全托管 Milvus）上的 Milvus 可处理更大的语料库，而无需您管理基础设施。</li>
<li><strong>尝试使用不同的文档类型。</strong>这里的管道使用的是对比 PDF，但它在扫描的合同、工程图纸、财务报表或包含密集数字的研究论文上也能以同样的方式工作。</li>
</ul>
<p>要开始使用，请使用 pip install<a href="https://github.com/milvus-io/milvus-lite">pymilvus 安装 Milvus Lite</a>，并从 HuggingFace 获取 ColQwen2 权重。</p>
<p>有问题或想展示自己的成果？<a href="https://milvus.io/slack">Milvus Slack</a>是获得社区和团队帮助的最快途径。如果您希望一对一交流，可以在我们的<a href="https://meetings.hubspot.com/chloe-williams1/milvus-office-hour?uuid=4cb203e5-482a-47e0-90a6-7acc511d61f4">办公时间</a>预约时间。</p>
...
<li><p><a href="https://milvus.io/blog/debugging-rag-in-3d-with-projectgolem-and-milvus.md">如果您能看到 RAG 为什么会失败？使用 Project_Golem 和 Milvus 在 3D 中调试 RAG</a></p></li>
<li><p><a href="https://milvus.io/blog/is-rag-become-outdated-now-long-running-agents-like-claude-cowork-are-emerging.md">克劳德-科沃克（Claude Cowork）这样的长期运行 Agents 正在崛起，RAG 是否已经过时？</a></p></li>
<li><p><a href="https://milvus.io/blog/semantic-highlighting-model-for-rag-context-pruning-and-token-saving.md">我们如何为 RAG 的上下文剪枝和标记保存建立语义高亮模型？</a></p></li>
<li><p><a href="https://milvus.io/blog/ai-code-review-gets-better-when-models-debate-claude-vs-gemini-vs-codex-vs-qwen-vs-minimax.md">当模型发生争执时，人工智能代码审查会变得更好：Claude vs Gemini vs Codex vs Qwen vs MiniMax</a></p></li>
</ul>

...
desc: 谷歌的双子座嵌入 2 将文本、图片、视频和音频映射到一个向量中。这是否会使多向量搜索过时？我们将分析每种方法的适用范围。
origin: >-
  https://milvus.io/blog/will-gemini-embedding-2-kill-multi-vector-search-in-vector-databases.md
---
<p>谷歌发布了 "<a href="https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-embedding-2/">双子座嵌入 2</a>"（<a href="https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-embedding-2/">Gemini Embedding 2</a>）--首个多模态嵌入模型，可将文本、图像、视频、音频和文档映射到单一向量空间。</p>
<p>只需调用一次 API，就能嵌入视频剪辑、产品照片和一段文字，而且它们都在同一个语义邻域中。</p>
<p>在使用这样的模型之前，您别无选择，只能将每种模式通过各自的专业模型运行，并将每种输出存储在单独的向量列中。<a href="https://milvus.io/docs/multi-vector-search.md">Milvus</a>等向量数据库中的多向量列正是为这种情况而设计的。</p>
<p>那么现在的问题是：Gemini Embeddings 2 可以取代多少复杂性，它的不足又在哪里？本篇文章将介绍每种方法的适用范围以及它们如何协同工作。</p>
<h2 id="What’s-Different-About-Gemini-Embedding-2-When-Compared-to-CLIPCLAP" class="common-anchor-header">与 CLIP/CLAP 相比，Gemini Embedding 2 有哪些不同之处？<button data-href="#What’s-Different-About-Gemini-Embedding-2-When-Compared-to-CLIPCLAP" class="anchor-icon" translate="no">
    ...
    </button></h2><p>嵌入模型能将非结构化数据转换为密集向量，从而使语义相似的项目在向量空间中聚集在一起。Gemini Embedding 2 的与众不同之处在于，它可以跨模态原生实现这一功能，无需单独的模型和拼接管道。</p>
<p>到目前为止，多模态嵌入意味着使用对比学习训练的双编码器模型：<a href="https://openai.com/index/clip/">CLIP</a>用于图像-文本，<a href="https://arxiv.org/abs/2211.06687">CLAP</a>用于音频-文本，每个模型正好处理两种模态。如果需要三种模式，则需要运行多个模型，并自行协调它们的嵌入空间。</p>
<p>例如，为带有封面艺术的播客编制索引意味着要运行 CLIP 来处理图像、CLAP 来处理音频，以及一个文本编码器来处理文字记录--三个模型、三个向量空间，以及自定义的融合逻辑，以便在查询时使它们的得分具有可比性。</p>
...
<p><strong>现在</strong>，您只需调用一次 API，就能将视频的帧数、音频和字幕输入其中，并获得一个能捕捉完整语义的统一向量。</p>
...
<h2 id="What-is-Multi-Vector-Search-in-Milvus" class="common-anchor-header">Milvus 中的多向量检索是什么？<button data-href="#What-is-Multi-Vector-Search-in-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在<a href="http://milvus.io">Milvus</a> 中，多向量检索指的是同时通过多个向量场检索同一个项目，然后将这些结果与重排结合起来。</p>
...
<p><a href="https://milvus.io/">Milvus</a>以并行<a href="https://milvus.io/docs/multi-vector-search.md">ANN 搜索的</a>方式运行这两种模式，并通过 RRFReranker 进行本地重排。Schema 定义、多索引配置、内置 BM25 - 全部在一个系统中完成。</p>
<p>为了具体说明这一点，请考虑产品目录，其中每个项目都包括文字说明和图片。您可以针对这些数据并行运行三种搜索：</p>
<ul>
...
<li><strong>跨模态图像搜索。</strong>使用文本查询和来自<a href="https://zilliz.com/learn/exploring-openai-clip-the-future-of-multimodal-ai-learning">CLIP</a> 等模型的密集向量对产品图片进行查询。</li>
</ul>
<h2 id="Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="common-anchor-header">现在，有了 Gemini Embedding 2，多向量搜索还重要吗？<button data-href="#Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Gemini Embeddings 2 可以在一次调用中处理更多模态，从而大大简化了管道。但统一的多模态嵌入与多向量检索并不是一回事。换句话说，多向量检索不会过时。</p>
<p>双子座嵌入 2 将文本、图像、视频、音频和文档映射到一个共享的向量空间中。谷歌将<a href="https://developers.googleblog.com/en/gemini-embedding-model-now-available/">其定位</a>为多模态语义搜索、文档检索和推荐--在这些场景中，所有模态都描述相同的内容，而高度的跨模态重叠使得单一向量变得可行。</p>
...
<p>这两种方法并不相互排斥。您可以将 Gemini Embeddings 2 用于统一的多模态列，同时仍可在同一<a href="https://milvus.io/"></a><a href="https://milvus.io/">Milvus</a>Collections 中的单独列中存储额外的稀疏或特定属性向量。</p>
<h2 id="Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="common-anchor-header">快速入门：设置双子座嵌入 2 + Milvus<button data-href="#Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>下面是一个工作演示。您需要一个正在运行的<a href="https://milvus.io/docs/install-overview.md"></a><a href="https://milvus.io/docs/install-overview.md">Milvus 或 Zilliz Cloud 实例</a>和一个 GOOGLE_API_KEY。</p>

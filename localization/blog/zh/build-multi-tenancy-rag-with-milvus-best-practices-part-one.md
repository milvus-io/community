...
<h2 id="Introduction" class="common-anchor-header">简介<button data-href="#Introduction" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在过去几年中，<a href="https://zilliz.com/learn/Retrieval-Augmented-Generation">检索增强生成（RAG）</a>已成为大型企业增强其<a href="https://zilliz.com/glossary/large-language-models-(llms)">LLM 驱动的</a>应用程序（尤其是那些拥有不同用户的应用程序）的一种值得信赖的解决方案。随着此类应用的增长，实施多租户框架变得至关重要。<strong>多租户</strong>可为不同用户群提供安全、隔离的数据访问，确保用户信任，满足监管标准，提高操作效率。</p>
...
<ul>
<li><p>租户 A 是一家医疗机构，负责存储面向患者的常见问题和合规文档。</p></li>
...
<p>这种设计确保了从用户交互到系统管理的每个流程层都能无缝运行，帮助企业建立一个强大且不断改进的知识库。</p>
...
<p>然而，在其他情况下，RAG 系统必须具有上下文感知能力。当需要这样做时，Milvus 需要与应用层协作，对每个用户的上下文保持个性化记忆。这种设计对于拥有大量终端用户的应用程序尤为重要，我们将在下一篇文章中详细探讨。敬请期待更多见解！</p>

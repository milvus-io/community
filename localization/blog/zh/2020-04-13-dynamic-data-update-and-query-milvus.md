...
<p>不可变队列将为后台序列化线程提供不可变状态和已准备好序列化的 MemTableFile。每个 MemTable 都管理自己的不可变队列，当 MemTable 的唯一可变 MemTableFile 的大小达到阈值时，它将进入不可变队列。负责 ToImmutable 的后台线程会定期提取 MemTable 管理的不可变队列中的所有 MemTableFile，并将它们发送到总的不可变队列中。需要注意的是，向内存中写入数据和将内存中的数据变为不能写入的状态这两种操作不能同时进行，需要一个公共锁。不过，ToImmutable 的操作非常简单，几乎不会造成任何延迟，因此对插入数据的性能影响很小。</p>

...
---
<p><a href="https://openclaw.ai/">OpenClaw</a>（前身为 Moltbot 和 Clawdbot）是一个开源的人工智能代理，它运行在你的机器上，通过你已经使用的消息应用程序（WhatsApp、Telegram、Slack、Signal 等）进行连接，并代表你进行操作--shell 命令、浏览器自动化、电子邮件、日历和文件操作。心跳调度程序会以可配置的时间间隔唤醒它，因此无需提示即可运行。在 2026 年 1 月底发布后不到一周的时间里，它就获得了超过<a href="https://github.com/openclaw/openclaw"> <span class="img-wrapper"> <img translate="no" src="https://img.shields.io/github/stars/openclaw/openclaw?label=⭐%20GitHub%20Stars" alt="GitHub Stars" class="doc-image" id="github-stars" /><span>GitHub Stars</span>GitHub<span>stars</span> </span></a>，成为 GitHub 历史上增长最快的开源软件源之一。</p>
<p>
...
<li><p><strong>多通道网关</strong>--WhatsApp、Telegram、Discord 和 iMessage 只需一个网关流程。通过扩展包添加 Mattermost 等功能。</p></li>
...
<p>Agents 的自主权大小取决于配置。工具策略和执行审批管理着高风险操作：你可能允许读取电子邮件，但在发送前需要审批；允许读取文件，但阻止删除。禁用这些防护措施，它就可以不经询问直接执行。</p>
...
<tr><td>会话内存</td><td>磁盘上基于文件的内存（Markdown + 日志）；可选插件增加语义/长期内存</td><td>具有历史记录的每个项目会话，以及账户上的可选 Claude 内存</td><td>CLI / 编辑器中的每个会话状态；没有内置的长期用户内存</td><td>由 ChatGPT 帐户级内存功能（如果启用）支持的每任务 "代理运行</td><td>跨运行的云端、账户范围内存，针对重复性工作流进行调整</td></tr>
...
<p>一位名叫 Hormold 的用户曾被 Lemonade Insurance 拒绝理赔。他的 OpenClaw 发现了这封拒绝邮件，并起草了一份引用保单语言的反驳，然后在没有明确许可的情况下发送了出去。Lemonade 重新启动了调查。他在推特上写道：&quot;我的 @openclaw 意外地与柠檬水保险公司打了一架，&quot;&quot;谢谢，人工智能。</p>
...
<h2 id="Conclusion" class="common-anchor-header">结论<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在不到两周的时间里，OpenClaw 的<a href="https://github.com/openclaw/openclaw"> <span class="img-wrapper"> <span>GitHub Stars</span> </span></a>超过了<a href="https://github.com/openclaw/openclaw"> <span class="img-wrapper"> <img translate="no" src="https://img.shields.io/github/stars/openclaw/openclaw?label=⭐%20GitHub%20Stars" alt="GitHub Stars" class="doc-image" id="github-stars" />，成为 GitHub 历史上增长最快的开源软件仓库之一。采用率是真实的，其下的架构也值得关注。 </span></a></p>

...
---
<p>在向量数据库中，我们经常需要在庞大的高维向量 Collections（如图像特征、文本嵌入或音频表示）中快速找到最相似的结果。如果没有索引，唯一的选择就是将查询向量与数据集中的每一个向量进行比较。在只有几千个向量的情况下，这种<strong>粗暴的搜索</strong>方式可能会奏效，但一旦你要处理的是几千万或上亿个向量，这种<strong>搜索</strong>方式就会变得异常缓慢，计算成本也会变得异常昂贵。</p>

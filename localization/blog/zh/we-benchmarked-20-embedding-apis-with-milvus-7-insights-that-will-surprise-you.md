...
<p>当 MTEB 排名沉迷于召回分数和模型大小时，他们忽略了用户的实际感受指标--他们在看到任何响应之前需要等待多长时间。我们在实际条件下测试了各大 Embeddings 提供商，发现延迟差异如此之大，以至于让你怀疑自己的整个供应商选择策略。</p>
...
<p>许多开发人员认为 LLM 的答案生成是最慢的部分。然而，许多 LLM 的流式输出功能会给人一种速度很快的错觉--你很快就能看到第一个标记。实际上，如果您的嵌入式 API 调用需要数百毫秒甚至数秒的时间，那么它就会成为您响应链中第一个也是最明显的瓶颈。</p>
...
<h2 id="Measuring-Real-World-Embedding-API-Latency-with-Milvus" class="common-anchor-header">使用 Milvus 测量真实世界中的嵌入式 API 延迟<button data-href="#Measuring-Real-World-Embedding-API-Latency-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus 是一个开源的高性能向量数据库，提供全新的<code translate="no">TextEmbedding</code> 函数接口。该功能将 OpenAI、Cohere、AWS Bedrock、Google Vertex AI、Voyage AI 以及更多提供商的流行嵌入模型直接集成到您的数据管道中，只需一次调用即可简化您的向量搜索管道。</p>
<p>利用这个新的功能接口，我们测试了来自 OpenAI 和 Cohere 等美国模型提供商以及 AliCloud 和 SiliconFlow 等亚洲提供商的各种流行嵌入 API，并对其进行了基准测试，测量了它们在实际部署场景中的端到端延迟。</p>
...
<h2 id="7-Key-Findings-from-Our-Benchmarking-Results" class="common-anchor-header">7 基准测试结果的主要发现<button data-href="#7-Key-Findings-from-Our-Benchmarking-Results" class="anchor-icon" translate="no">
    ...
    </button></h2><p>我们在不同的批量大小、令牌长度和网络条件下测试了北美和亚洲的知名嵌入模型，测量了所有情况下的中位延迟。我们的研究结果揭示了一些重要的见解，这些见解将重塑您对嵌入式 API 选择和优化的看法。让我们一起来看看。</p>
<h3 id="1-Global-Network-Effects-Are-More-Significant-Than-You-Think" class="common-anchor-header">1.全球网络效应比您想象的更重要</h3><p>网络环境可能是影响嵌入式 API 性能的最关键因素。同一家嵌入式 API 服务提供商在不同网络环境下的性能可能大相径庭。</p>
<p>
...
<p>这意味着您必须始终根据部署地点和用户地理位置来选择 Embeddings 提供商--没有网络背景的性能要求是毫无意义的。</p>
<h3 id="2-Model-Performance-Rankings-Reveal-Surprising-Results" class="common-anchor-header">2.模型性能排名揭示了令人惊讶的结果</h3><p>我们的全面延迟测试显示了清晰的性能等级：</p>
<ul>
<li><p><strong>北美模型（延迟中位数）</strong>：Cohere &gt; Google Vertex AI &gt; VoyageAI &gt; OpenAI &gt; AWS Bedrock</p></li>
<li><p><strong>亚洲模型（延迟中位数）</strong>：SiliconFlow &gt; 阿里云 Dashscope</p></li>
</ul>
...
<p>注：由于网络环境和服务器地理区域对实时嵌入 API 延迟的影响很大，我们分别比较了北美和亚洲模型的延迟。</p>
...
<h3 id="4-Token-Length-and-Batch-Size-Create-Complex-Trade-offs" class="common-anchor-header">4.令牌长度和批量大小会产生复杂的权衡关系</h3><p>根据后端实施情况，尤其是批处理策略，令牌长度可能不会对延迟产生显著影响，直到批处理规模增大。我们的测试揭示了不同的模式：</p>
<ul>
...
<p>TEI 延迟</p>
<h3 id="7-Milvus-Overhead-Is-Negligible" class="common-anchor-header">7.Milvus 的开销微不足道</h3><p>由于我们使用 Milvus 测试嵌入 API 的延迟，我们验证了 Milvus 的 TextEmbedding 函数带来的额外开销极小，几乎可以忽略不计。我们的测量结果表明，Milvus 操作总共只增加了 20-40ms 的时间，而嵌入 API 调用则需要数百毫秒到数秒的时间--这意味着<strong>Milvus</strong>在总操作时间上只<strong>增加了不到 5% 的开销</strong>。性能瓶颈主要在于网络传输和嵌入式 API 服务提供商自身的处理能力，而不是 Milvus 服务器层。</p>
<h2 id="Tips-How-to-Optimize-Your-RAG-Embedding-Performance" class="common-anchor-header">小贴士如何优化 RAG 嵌入性能<button data-href="#Tips-How-to-Optimize-Your-RAG-Embedding-Performance" class="anchor-icon" translate="no">
    ...
    </button></h2><p>根据我们的综合基准测试，我们建议采取以下策略来优化 RAG 系统的嵌入性能：</p>
<h3 id="1-Always-Localize-Your-Testing" class="common-anchor-header">1.始终进行本地化测试</h3><p>不要盲目相信任何通用基准报告（包括本报告！）。您应该始终在实际部署环境中测试模型，而不是仅仅依赖于公布的基准。网络条件、地理位置的远近以及基础设施的差异都会极大地影响实际性能。</p>
...
<h3 id="5-Implement-Strategic-Caching" class="common-anchor-header">5.实施战略性缓存</h3><p>对于高频查询，同时缓存查询文本及其生成的 Embeddings（使用 Redis 等解决方案）。随后的相同查询可以直接访问缓存，从而将延迟降低到毫秒级。这是目前最具成本效益和影响力的查询延迟优化技术之一。</p>
...
<p>对于许多个人开发者或小型团队来说，缺乏企业级 GPU 似乎是本地部署高性能嵌入式模型的障碍。但是，这并不意味着完全放弃本地推理。与高性能推理引擎（如<a href="https://github.com/huggingface/text-embeddings-inference">Hugging Face 的文本嵌入式</a>推理引擎）相结合，即使在 CPU 上运行中小型嵌入式模型，也能获得不错的性能，可能会优于高延迟 API 调用，尤其是在大规模离线嵌入式生成方面。</p>
...
<p>嵌入式提供商的选择是 RAG 性能拼图的关键一环。通过在实际部署环境中进行测试、选择地理位置合适的提供商并考虑本地推理等替代方案，您可以消除面向用户的延迟的主要来源，并构建真正响应迅速的人工智能应用程序。</p>

...
<h2 id="SQLite" class="common-anchor-header">SQLite<button data-href="#SQLite" class="anchor-icon" translate="no">
    ...
    </button></h2><p>如果使用 SQLite，在启动 Milvus 后，数据目录（在配置文件<code translate="no">server_config.yaml</code> 的<code translate="no">primary_path</code> 中定义）中将生成一个<code translate="no">meta.sqlite</code> 文件。要查看该文件，只需安装一个 SQLite 客户端。</p>
<p>从命令行安装 SQLite3：</p>

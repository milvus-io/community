...
<p>无论你是在构建一个需要闪电般快速查询的推荐系统，还是在优化一个有成本限制的向量搜索应用程序，我都会告诉你到底该用哪些实用的、经过测试的值来修改参数。在本指南结束时，你将知道如何根据实际部署情况调整 Milvus 配置，以获得最佳性能。</p>
...
<ul>
<li><p>较低的值（如 0.12）会更快触发封存，这有助于更快地创建索引，对频繁更新的工作负载很有用处。</p></li>
<li><p>较高的值（如 0.3 到 0.5）会延迟封存，减少索引开销，更适合离线或批量摄取场景。</p></li>
</ul>
<ol start="4">
...
<li><p>使用<code translate="no">mmap</code> 构建的 HNSW 索引最多可将总数据量扩大<strong>1.8 倍</strong>。</p></li>

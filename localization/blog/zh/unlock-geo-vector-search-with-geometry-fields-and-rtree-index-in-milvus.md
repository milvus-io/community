...
desc: 了解 Milvus 2.6 如何利用几何字段和 RTREE 索引将向量搜索与地理空间索引统一起来，从而实现准确的、位置感知的人工智能检索。
origin: >-
...
<p><strong>2.组合成更大的方框：</strong>将附近的叶节点分组，并将每个分组包在一个新的 MBR 内，生成内部节点。</p>
...
<h2 id="Conclusion" class="common-anchor-header">结论<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>地理定位不仅仅是经度和纬度。在位置敏感型应用中，它提供了有关<strong>事件发生位置、实体空间关系以及这些关系如何影响系统行为的</strong>重要背景信息。当地理空间数据与来自机器学习模型的语义信号相结合时，就能实现更丰富的查询，而这些查询在单独处理空间和向量数据时难以表达或执行效率低下。</p>
...
<p>随着人工智能系统不断向现实世界的决策靠拢，了解<strong><em>哪些</em></strong>内容是相关的将越来越需要与它的<strong><em>适用范围</em></strong>和适用<strong><em>时间</em></strong>相匹配。Milvus 为这类空间语义工作负载提供了构件，这种构件既有表现力，又实用于大规模操作。</p>

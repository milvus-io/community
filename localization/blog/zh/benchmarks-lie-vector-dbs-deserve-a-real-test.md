...
<p>VDBBench 还支持自定义数据集，让您可以使用自己的数据进行基准测试，这些数据由您针对特定工作负载的特定嵌入模型生成。毕竟，没有数据集比您自己的生产数据更能说明问题。</p>
...
<h3 id="Static-Collection" class="common-anchor-header">静态收集</h3><p>与匆忙进行测试的其他基准不同，VDBBench 会首先确保每个数据库已完全优化其索引--这是许多基准经常忽略的关键生产前提条件。这将为您提供完整的信息：</p>
<ul>
<li><p>数据摄取时间</p></li>

...
<h2 id="What-if-You-Could-Build-Your-Own-Cursor" class="common-anchor-header">如果你能创建自己的光标会怎样？<button data-href="#What-if-You-Could-Build-Your-Own-Cursor" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><em>因此，我们提出了一个问题：</em> <strong><em>如果任何人都能构建自己的 Cursor 会怎样？</em></strong>在你的基础设施上运行。无需订阅费。完全可定制。完全控制您的代码和数据。</p>
...
<li><p><strong>向量数据库接口</strong>将这些嵌入存储在自托管的<a href="https://milvus.io/">Milvus</a>实例中（默认情况下），或存储在托管版 Milvus 的<a href="https://zilliz.com/cloud">Zilliz Cloud 中</a>。</p></li>
</ul>
...
<h2 id="Getting-Started-with-Code-Context" class="common-anchor-header">开始使用代码上下文<button data-href="#Getting-Started-with-Code-Context" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Code Context 可插入您已使用的编码工具，也可用于从头开始构建自定义的人工智能编码助手。在本节中，我们将介绍这两种情况：</p>
<ul>
...
<p>这种架构非常适合对合规性有严格要求的行业，如金融、政府和国防等，在这些行业中，代码的保密性是不容置疑的。</p>
...
<p>代码上下文的一些应用实例：</p>
<ul>
<li><p><strong>上下文感知的自动完成插件</strong>，可提取相关代码片段以更好地完成 LLM</p></li>

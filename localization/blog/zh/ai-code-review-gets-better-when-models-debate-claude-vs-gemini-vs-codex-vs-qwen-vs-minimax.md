...
---
<p>最近，我使用人工智能模型审查了一个拉取请求，结果却自相矛盾：Claude 标记了数据竞赛，而 Gemini 则说代码是干净的。这让我对其他人工智能模型的表现产生了好奇，于是我将 Claude、Gemini、Codex、Qwen 和 MiniMax 的最新旗舰模型进行了结构化代码审查基准测试。结果如何？表现最好的模型只捕获了 53% 的已知错误。</p>
...
<h2 id="Conclusion" class="common-anchor-header">结论<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p><strong>克劳德</strong>是最值得信赖的评审专家。它能处理整个调用链，遵循深层逻辑路径，并在不需要你灌输的情况下引入自己的上下文。在处理 L3 系统级错误方面，没有其他工具能与它相媲美。它有时会对数学过于自信，但当另一个模型证明它是错的时候，它就会承认错误，并说明自己的推理在哪里出了问题。用它来处理核心代码和你不能错过的错误。</p>
...
<li><p><a href="https://milvus.io/blog/adding-persistent-memory-to-claude-code-with-the-lightweight-memsearch-plugin.md">使用轻量级 memsearch 插件为克劳德代码添加持久内存</a></p></li>

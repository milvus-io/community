...
<ul>
<li><strong>最适合</strong>：单人开发者或小型团队，他们的代码库只适合在一台机器上运行，正在寻求比 grep 更快的改进。</li>
<li><strong>当</strong>您需要多用户访问同一索引，或您的数据超出了单个 SQLite 文件可轻松处理的范围<strong>时</strong>，<strong>它</strong>就会过时。</li>
</ul>
<h4 id="QMD-higher-accuracy-through-local-LLM-re-ranking" class="common-anchor-header">QMD：通过本地 LLM 重新排序提高准确性</h4><p>由 Shopify 创始人托比-吕特克（Tobi Lütke）创建的<a href="https://github.com/tobi/qmd">QMD</a>（查询标记文档）增加了第三个阶段：LLM 重新排序。在 BM25 和向量搜索各自返回候选结果后，本地语言模型会重新读取排名靠前的结果，并根据与您的查询的实际相关性重新排序。这样就能捕捉到关键字和语义匹配都返回可信但错误结果的情况。</p>
...
<p>在引擎盖下，memsearch 采用了与上述相同的混合搜索模式：按标题结构和段落边界分割的大块、BM25 + 向量检索，以及一个由 LLM 驱动的精简命令，当日志变大时，该命令会汇总旧的记忆。  <span class="img-wrapper">

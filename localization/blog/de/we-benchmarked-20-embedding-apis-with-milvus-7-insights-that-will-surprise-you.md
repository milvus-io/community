...
<p>Während MTEB-Rankings sich auf Recall-Werte und Modellgrößen konzentrieren, ignorieren sie die Metrik, die Ihre Benutzer tatsächlich spüren - wie lange sie warten, bevor sie eine Antwort sehen. Wir haben jeden großen Einbettungsanbieter unter realen Bedingungen getestet und dabei so extreme Latenzunterschiede festgestellt, dass Sie Ihre gesamte Anbieterauswahlstrategie in Frage stellen werden.</p>
...
<h2 id="Why-Embedding-API-Latency-Is-the-Hidden-Bottleneck-in-RAG" class="common-anchor-header">Warum die Einbettung der API-Latenzzeit der versteckte Engpass in RAG ist<button data-href="#Why-Embedding-API-Latency-Is-the-Hidden-Bottleneck-in-RAG" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Beim Aufbau von RAG-Systemen, E-Commerce-Suchen oder Empfehlungsmaschinen dienen Einbettungsmodelle als Kernkomponente, die Text in Vektoren umwandelt und es Maschinen ermöglicht, Semantik zu verstehen und effiziente Ähnlichkeitssuchen durchzuführen. Während wir Einbettungen für Dokumentenbibliotheken in der Regel vorberechnen, erfordern Benutzeranfragen immer noch Echtzeit-Einbettungs-API-Aufrufe, um Fragen vor dem Abruf in Vektoren umzuwandeln, und diese Echtzeit-Latenz wird oft zum Leistungsengpass in der gesamten Anwendungskette.</p>
...
<li><p>Suche nach ähnlichen Vektoren in Milvus</p></li>
<li><p>Die Ergebnisse und die ursprüngliche Frage an ein LLM weiterleiten</p></li>
...
<p>Beide Szenarien machen die Latenz der Einbettungs-API zu einer nicht verhandelbaren Leistungskennzahl für produktive RAG-Systeme.</p>
...
<p>Mithilfe dieser neuen Funktionsschnittstelle haben wir verschiedene beliebte Einbettungs-APIs von amerikanischen Modellanbietern wie OpenAI und Cohere sowie von asiatischen Anbietern wie AliCloud und SiliconFlow getestet und ihre End-to-End-Latenz in realistischen Einsatzszenarien gemessen.</p>
<p>Unsere umfassende Testsuite deckte verschiedene Modellkonfigurationen ab:</p>
<table>
<thead>
...
<h2 id="7-Key-Findings-from-Our-Benchmarking-Results" class="common-anchor-header">7 Wichtige Erkenntnisse aus unseren Benchmarking-Ergebnissen<button data-href="#7-Key-Findings-from-Our-Benchmarking-Results" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Wir haben renommierte Einbettungsmodelle aus Nordamerika und Asien unter verschiedenen Stapelgrößen, Token-Längen und Netzwerkbedingungen getestet und die mittlere Latenz über alle Szenarien hinweg gemessen. Unsere Ergebnisse liefern wichtige Erkenntnisse, die Ihre Überlegungen zur Auswahl und Optimierung von Einbettungs-APIs verändern werden. Werfen wir einen Blick darauf.</p>
<h3 id="1-Global-Network-Effects-Are-More-Significant-Than-You-Think" class="common-anchor-header">1. Globale Netzwerkeffekte sind signifikanter als Sie denken</h3><p>Die Netzwerkumgebung ist vielleicht der wichtigste Faktor, der die Leistung von Embedding APIs beeinflusst. Ein und derselbe Anbieter von Einbettungs-APIs kann in verschiedenen Netzwerkumgebungen sehr unterschiedliche Leistungen erbringen.</p>
<p>
...
<ul>
<li><p><strong>Nordamerikanische Modelle (mediane Latenz)</strong>: Cohere &gt; Google Vertex AI &gt; VoyageAI &gt; OpenAI &gt; AWS Bedrock</p></li>
<li><p><strong>Asiatische Modelle (mittlere Latenz)</strong>: SiliconFlow &gt; AliCloud Dashscope</p></li>
</ul>
...
<p>Hinweis: Aufgrund des erheblichen Einflusses der Netzwerkumgebung und der geografischen Region des Servers auf die Latenz der Echtzeit-Einbettungs-API haben wir die Latenzen der nordamerikanischen und asiatischen Modelle separat verglichen.</p>
...
<h3 id="4-Token-Length-and-Batch-Size-Create-Complex-Trade-offs" class="common-anchor-header">4. Token-Länge und Batch-Größe schaffen komplexe Kompromisse</h3><p>Je nach Backend-Implementierung, insbesondere bei Batching-Strategien, wirkt sich die Token-Länge möglicherweise erst bei größeren Stapelgrößen signifikant auf die Latenz aus. Unsere Tests zeigten deutliche Muster:</p>
<ul>
...
<p>TEI-Latenz</p>
<h3 id="7-Milvus-Overhead-Is-Negligible" class="common-anchor-header">7. Der Milvus-Overhead ist vernachlässigbar</h3><p>Da wir Milvus verwendet haben, um die Latenz der Einbettungs-API zu testen, konnten wir feststellen, dass der zusätzliche Overhead, der durch die TextEmbedding-Funktion von Milvus entsteht, extrem gering und praktisch vernachlässigbar ist. Unsere Messungen zeigen, dass Milvus-Operationen insgesamt nur 20-40 ms hinzufügen, während die Einbettungs-API-Aufrufe Hunderte von Millisekunden bis zu mehreren Sekunden dauern - was bedeutet, dass <strong>Milvus weniger als 5 % Overhead</strong> zur Gesamtoperationszeit <strong>hinzufügt</strong>. Der Leistungsengpass liegt in erster Linie in der Netzwerkübertragung und den eigenen Verarbeitungsfähigkeiten der Anbieter von Einbettungs-APIs, nicht in der Milvus-Serverschicht.</p>
...
<h3 id="5-Implement-Strategic-Caching" class="common-anchor-header">5. Strategisches Caching implementieren</h3><p>Speichern Sie bei hochfrequenten Abfragen sowohl den Abfragetext als auch die generierten Einbettungen im Cache (mit Lösungen wie Redis). Nachfolgende identische Abfragen können direkt auf den Cache zugreifen und die Latenzzeit auf Millisekunden reduzieren. Dies ist eine der kosteneffektivsten und wirkungsvollsten Techniken zur Optimierung der Abfragelatenz, die es gibt.</p>
<h3 id="6-Consider-Local-Inference-Deployment" class="common-anchor-header">6. Lokale Inferenz-Bereitstellung in Betracht ziehen</h3><p>Wenn Sie extrem hohe Anforderungen an die Latenzzeit bei der Dateneingabe, die Abfragelatenz und den Datenschutz haben oder wenn die Kosten für API-Aufrufe unerschwinglich sind, sollten Sie die lokale Bereitstellung von Einbettungsmodellen für die Inferenz in Betracht ziehen. Standard-API-Pläne sind oft mit QPS-Beschränkungen, instabilen Latenzzeiten und fehlenden SLA-Garantien verbunden - Einschränkungen, die für Produktionsumgebungen problematisch sein können.</p>
<p>Für viele einzelne Entwickler oder kleine Teams mag das Fehlen von GPUs in Unternehmensqualität wie ein Hindernis für die lokale Bereitstellung von Hochleistungs-Einbettungsmodellen erscheinen. Dies bedeutet jedoch nicht, dass die lokale Inferenz völlig aufgegeben werden muss. In Kombination mit leistungsstarken Inferenz-Engines wie <a href="https://github.com/huggingface/text-embeddings-inference">Hugging Face's text-embeddings-inference</a> können selbst kleine bis mittelgroße Einbettungsmodelle auf der CPU eine anständige Leistung erzielen, die API-Aufrufe mit hoher Latenz übertreffen kann, insbesondere bei der Offline-Erzeugung von Einbettungen in großem Maßstab.</p>
...
<p>Die Wahl des Einbettungsanbieters ist ein wichtiger Teil des RAG-Leistungspuzzles. Durch Tests in Ihrer tatsächlichen Bereitstellungsumgebung, die Auswahl geografisch geeigneter Anbieter und die Erwägung von Alternativen wie lokaler Inferenz können Sie eine wichtige Ursache für Verzögerungen bei der Benutzererfahrung beseitigen und wirklich reaktionsschnelle KI-Anwendungen erstellen.</p>

...
desc: >-
  ...
  extrahiertem Text abruft, mit ColQwen2, Milvus und Qwen3.5.
...
<p>Retrieval-augmented generation (RAG) löst dieses Problem, indem es die Indizierung von der Beantwortung trennt. Sie kodieren Ihre Dokumente einmal, speichern die Repräsentationen in einer Vektordatenbank und rufen zur Abfragezeit nur die relevantesten Seiten ab, um sie an den LLM zu senden. Das Modell liest drei Seiten pro Abfrage, nicht Ihren gesamten Korpus. Das macht es praktisch, Dokument-Q&amp;A über Sammlungen aufzubauen, die ständig wachsen.</p>
...
<li>Schaubilder und Diagramme, deren Informationen rein visuell sind und keine Entsprechung im Text haben.</li>
...
<h3 id="Step-3-Initialize-Milvus" class="common-anchor-header">Schritt 3: Initialisierung von Milvus</h3><p>In diesem Tutorial wird Milvus Lite verwendet, das als lokale Datei ohne Konfiguration läuft - es ist kein separater Serverprozess erforderlich.</p>
...
<li><strong>Die Kodierung ist langsamer als die Texteinbettung.</strong> ColQwen2 ist ein 4,4 GB großes Bildmodell. Das Kodieren von Bildern dauert pro Seite länger als das Einbetten von Textabschnitten. Für einen einmaligen Batch-Indizierungsauftrag ist dies in der Regel in Ordnung. Für die Echtzeit-Ingestion lohnt sich ein Benchmarking.</li>
<li><strong>Dieser Ansatz eignet sich am besten für visuell reichhaltige Dokumente.</strong> Wenn es sich bei Ihren PDF-Dokumenten hauptsächlich um einspaltigen Text ohne Tabellen oder Abbildungen handelt, ist die herkömmliche textbasierte RAG möglicherweise präziser und kostengünstiger in der Ausführung.</li>
</ul>
<p>Was Sie als nächstes versuchen sollten:</p>
<ul>
<li><strong>Wechseln Sie zu einem anderen multimodalen LLM.</strong> In diesem Tutorium wird Qwen3.5 über OpenRouter verwendet, aber die Abrufpipeline ist modellunabhängig. Sie könnten den Generierungsschritt auf GPT-4o, Gemini oder ein anderes multimodales Modell richten, das Bildeingaben akzeptiert.</li>

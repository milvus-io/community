...
---
<p>Große Sprachmodelle (Large Language Models, LLMs) haben die KI-Landschaft durch ihre Fähigkeit, Code zu schreiben, Inhalte zu erstellen und komplexe Probleme zu lösen, verändert. Diese leistungsstarken Modelle benötigen jedoch enorme Mengen an hochwertigen Daten, um ihr Training zu unterstützen.</p>
<p>Die Herausforderung besteht darin, dass die Rohdaten für das Training oft erhebliche Redundanzen enthalten. Das ist so, als würde man einem Kind immer wieder dieselben Lektionen beibringen, während man andere wichtige Themen auslässt. Ein großes KI-Unternehmen trat mit genau diesem Problem an uns heran: Es wollte ein ehrgeiziges neues Sprachmodell entwickeln, hatte aber Probleme mit der Deduplizierung von mehreren Milliarden Dokumenten. Herkömmliche Abgleichsmethoden waren für dieses Volumen nicht geeignet, und spezialisierte Deduplizierungstools erforderten enorme Rechenressourcen, was sie wirtschaftlich unrentabel machte.</p>
<p>Unsere Lösung für dieses Problem lautet: MinHash LSH (Locality Sensitive Hashing) Indexierung, die in Milvus 2.6 verfügbar sein wird. In diesem Artikel werden wir untersuchen, wie MinHash LSH das Problem der Datendeduplizierung für LLM-Training effizient löst.</p>
<p>
...
<p>Bei Pre-Training-Korpora, die Terabytes oder sogar Petabytes umfassen, sind herkömmliche exakte Matching-Methoden wie paarweise Vergleiche rechnerisch nicht durchführbar. Die semantische Deduplizierung fügt durch die Verwendung von Einbettungsmodellen zur Erzeugung von Vektoren einen erheblichen Overhead hinzu. Wir brauchen intelligentere Näherungsmethoden - wie <strong>MinHash LSH -, die</strong>ein Gleichgewicht zwischen Recall und Präzision herstellen und gleichzeitig die Kosten überschaubar halten, so dass Deduplizierung in großem Maßstab praktikabel ist.</p>
...
<p>Bei der Berechnung der Ähnlichkeit liefert die Wahrscheinlichkeit, dass die Hash-Werte an denselben Positionen in den MinHash-Signaturen zweier Dokumente übereinstimmen (was dem Jaccard-Abstand dieser Signaturen entspricht), eine gute Annäherung an die Jaccard-Ähnlichkeit ihrer ursprünglichen Schindelsätze. Auf diese Weise können wir die Ähnlichkeit von Dokumenten effektiv schätzen, ohne die größeren Originaltexte direkt vergleichen zu müssen; stattdessen können wir ihre kompakten MinHash-Signaturen analysieren.</p>
<p>Beim MinHash-Prinzip wird das Wort mit dem kleinsten Hash-Wert verwendet, um das gesamte Dokument zu repräsentieren, wobei die Genauigkeit durch die Einbeziehung zusätzlicher Hash-Funktionen erhöht wird. Geringfügige Wortänderungen werden wahrscheinlich übersehen, da sie sich in der Regel nicht auf den minimalen Hash-Wert auswirken, während umfangreichere Änderungen den Hash-Wert verändern und leichter zu erkennen sind. Diese Methode kann als ein Min-Pooling von Hash-Werten über verschiedene Wörter hinweg betrachtet werden. Neben MinHash gibt es Alternativen wie SimHash zur Erzeugung von Dokumentensignaturen, auf die hier jedoch nicht eingegangen werden soll.</p>
...
<h2 id="Integrating-MinHash-LSH-in-Milvus-26" class="common-anchor-header">Einbindung von MinHash LSH in Milvus 2.6<button data-href="#Integrating-MinHash-LSH-in-Milvus-26" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Die Integration von MinHash LSH in Milvus 2.6 wurde durch einen realen Bedarf angetrieben. Wie bereits erwähnt, trat ein Milvus-Anwender - eines der führenden LLM-Unternehmen - mit einer Herausforderung an uns heran: die effiziente Deduplizierung großer Mengen von Textdaten für das LLM-Pre-Training.</p>
<p>Herkömmliche Deduplizierungspipelines sind in der Regel auf externe Tools angewiesen, die von den Speicher- und Abfragesystemen entkoppelt sind und kostspielige Datentransfers zwischen den Komponenten erfordern. Dieser fragmentierte Arbeitsablauf erhöht den betrieblichen Aufwand und verhindert die volle Ausnutzung der verteilten Computerressourcen.</p>
...
<p>Wenn Sie einen ähnlichen Anwendungsfall haben, kontaktieren Sie uns bitte auf dem Milvus Discord, um sich für eine Sprechstunde anzumelden.</p>

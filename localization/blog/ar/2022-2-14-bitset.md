...
<h3 id="Case-one" class="common-anchor-header">الحالة الأولى</h3><p>لنفترض أن القيمة التي يحددها المستخدم لـ <code translate="no">time_travel</code> هي 150. بعبارة أخرى، يجري المستخدم استعلامًا على البيانات المخزنة في ميلفوس عندما يكون <code translate="no">ts</code> = 150. يوضح الشكل 1 عملية توليد مجموعة البتات.</p>
...
<p>يتم إدراج جميع الكيانات [1، 2، 3، 4، 4، 5، 6، 7، 8] في قاعدة بيانات المتجهات عندما <code translate="no">ts</code>= 350. لذلك، تكون النتيجة النهائية المقلوبة <code translate="no">filter_bitset</code> هي [0، 1، 1، 0، 0، 1، 0، 1، 0، 0، 1]، كما في الحالة الثانية.</p>

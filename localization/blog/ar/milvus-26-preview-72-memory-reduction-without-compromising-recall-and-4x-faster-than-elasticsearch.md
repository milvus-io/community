...
<p><em>الجدول: تقييم VectorDBBench مع 1 مليون متجه من 768 بُعدًا، تم اختباره على AWS m6id.2xlarge</em></p>
<p>الإنجاز الحقيقي هنا ليس فقط في تقليل الذاكرة، ولكن تحقيق ذلك مع تحقيق تحسين في الإنتاجية بمعدل 4 أضعاف دون المساس بالدقة. هذا يعني أنك ستتمكن من خدمة عبء العمل نفسه باستخدام خوادم أقل بنسبة 75% أو التعامل مع حركة مرور أكثر 4 أضعاف على بنيتك التحتية الحالية.</p>
<p>بالنسبة لمستخدمي المؤسسات الذين يستخدمون Milvus المُدار بالكامل على<a href="https://zilliz.com/cloud"> Zilliz Cloud،</a> فإننا نعمل على تطوير ملفات تعريف تهيئة آلية من شأنها ضبط معلمات RaBitQ ديناميكيًا بناءً على خصائص عبء العمل ومتطلبات الدقة الخاصة بك.</p>
...
<p>الشكل: Milvus مقارنةً بـ Elasticsearch على فهرس مسار JSON من حيث الإنتاجية: زمن انتقال أقل بنسبة 99% للتصفية المعقدة</p>
<p>نادرًا ما تعتمد تطبيقات الذكاء الاصطناعي الحديثة على التشابه المتجه وحده - فهي دائمًا ما تجمع بين البحث المتجه وتصفية البيانات الوصفية. عندما تصبح شروط التصفية هذه أكثر تعقيدًا (خاصةً مع كائنات JSON المتداخلة)، يمكن أن يتدهور أداء الاستعلام بسرعة.</p>
<p>سيقدم Milvus 2.6 آلية فهرسة مستهدفة لمسارات JSON المتداخلة التي تسمح لك بإنشاء فهارس على مسارات محددة (على سبيل المثال، $meta. <code translate="no">user_info.location</code>) داخل حقول JSON. بدلاً من مسح كائنات كاملة، سيبحث ميلفوس مباشرةً عن القيم من الفهارس المبنية مسبقًا.</p>
...
<li><p><strong>معدل التضاؤل</strong>: مدى سرعة تضاؤل الأهمية مع مرور الوقت</p></li>

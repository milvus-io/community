...
desc: >-
  لقد استخرجنا بنية ذاكرة OpenClaw للذكاء الاصطناعي في memsearch - مكتبة بايثون
  مستقلة مع سجلات Markdown، والبحث المتجه الهجين، ودعم Git.
origin: >-
...
<p>الجانب السلبي الوحيد؟ في الوقت الحالي يرتبط نظام الذاكرة الذي يعتمد على Markdown أولاً بشكل وثيق مع نظام OpenClaw البيئي الكامل - عملية البوابة، وموصلات النظام الأساسي، وتكوين مساحة العمل، والبنية التحتية للمراسلة. إذا كنت تريد فقط نموذج الذاكرة، فهذا يتطلب الكثير من الآلات لسحبها.</p>
...
<p>يتم تخزين جميع ذاكرة الوكيل في memsearch كنص عادي Markdown في دليل محلي. البنية بسيطة عن قصد حتى يتمكن المطورون من فهمها في لمحة:</p>
...
<p>في الممارسة العملية، هذا يعني أنه يمكن للمطوّر كتابة التعليمات البرمجية في نافذة وتحرير <code translate="no">MEMORY.md</code> في نافذة أخرى، أو إضافة عنوان URL لمستندات واجهة برمجة التطبيقات أو تصحيح إدخال قديم. احفظ الملف، وسيلتقط استعلام الذكاء الاصطناعي التالي الذاكرة الجديدة. لا إعادة تشغيل ولا إعادة فهرسة يدوية.</p>
...
<li><p>يتعامل<strong>BM25</strong> مع المطابقة التامة. لا يُرجع الاستعلام عن "PostgreSQL 16" نتائج حول "PostgreSQL 15". هذا الأمر مهم بالنسبة لرموز الأخطاء وأسماء الدوال والسلوك الخاص بالإصدار، حيث لا يكون التقارب جيدًا بما فيه الكفاية.</p></li>
</ul>
...
<tr><td>ميم0</td><td>مُدار بالكامل، لا توجد بنية تحتية للتشغيل أو الصيانة</td><td>مبهمة - لا يمكن فحص الذاكرة أو تحريرها يدويًا؛ التضمينات هي التمثيل الوحيد</td><td>فرق العمل التي تريد خدمة مُدارة بدون تدخل ولا تمانع في رؤية أقل</td></tr>

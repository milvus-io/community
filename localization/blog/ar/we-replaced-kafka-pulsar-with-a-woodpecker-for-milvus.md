...
<p>دعوني أطلعكم على رحلتنا وأشرح لكم سبب قيامنا بهذا التغيير، الذي قد يبدو للوهلة الأولى غير بديهي.</p>
<h2 id="Cloud-Native-From-Day-One" class="common-anchor-header">سحابة أصلية منذ اليوم الأول<button data-href="#Cloud-Native-From-Day-One" class="anchor-icon" translate="no">
    ...
    </button></h2><p>كانت Milvus قاعدة بيانات سحابية أصلية منذ بدايتها. نحن نستفيد من Kubernetes للتوسع المرن والتعافي السريع من الأعطال، إلى جانب حلول تخزين الكائنات مثل Amazon S3 وMinIO لاستمرار البيانات.</p>
...
<p>أثبتت الحلول الحالية مثل Bookie أنها غير ملائمة لاحتياجاتنا بسبب تصميم العميل الثقيل وغياب حزم SDKs الجاهزة للإنتاج في Golang و C++. قادتنا هذه الفجوة التكنولوجية إلى نهجنا الأولي باستخدام قوائم انتظار الرسائل.</p>
...
<li><p>تعالج QueryNode وDataNode هذه البيانات بشكل غير متزامن، مما يضمن إنتاجية كتابة عالية مع الحفاظ على حداثة البيانات</p></li>
</ul>
<p>
  <span class="img-wrapper">
...
<p>زادت هذه التبعيات الخارجية من صعوبة تنفيذ الميزات الهامة - مثل جمع نفايات السجل - وزادت من احتكاك التكامل مع وحدات النظام الأخرى. بمرور الوقت، أصبح عدم التطابق المعماري بين قوائم انتظار الرسائل ذات الأغراض العامة والمتطلبات المحددة عالية الأداء لقاعدة بيانات المتجهات واضحًا بشكل متزايد، مما دفعنا إلى إعادة تقييم خيارات التصميم الخاصة بنا.</p>
<h3 id="Resource-Inefficiency" class="common-anchor-header">عدم كفاءة الموارد</h3><p>يتطلب ضمان التوافرية العالية مع أنظمة مثل كافكا وبولسار عادةً</p>
<ul>
...
<h2 id="Introducing-Woodpecker---A-Cloud-Native-High-Performance-WAL-Engine" class="common-anchor-header">تقديم نقار الخشب - محرك سحابي أصلي عالي الأداء في السحابة<button data-href="#Introducing-Woodpecker---A-Cloud-Native-High-Performance-WAL-Engine" class="anchor-icon" translate="no">
    ...
    </button></h2><p>في الإصدار Milvus 2.6، استبدلنا Kafka/Pulsar ب <strong>Woodpecker،</strong> وهو نظام WAL مصمم خصيصًا للسحابة الأصلية. صُمم Woodpecker لتخزين الكائنات، وهو يعمل على تبسيط العمليات مع تعزيز الأداء وقابلية التوسع.</p>
...
<p>يقلل هذا النهج من النفقات التشغيلية بشكل كبير مع زيادة المتانة والكفاءة السحابية إلى أقصى حد. من خلال التخلص من التبعيات المحلية للقرص المحلي، يتماشى Woodpecker بشكل مثالي مع مبادئ السحابة الأصلية ويقلل بشكل كبير من العبء التشغيلي على مسؤولي النظام.</p>
...
<li><p><strong>وضع التخزين السحابي (S3)</strong>: عند الكتابة مباشرةً إلى S3، وصلت سرعة Woodpecker إلى 750 ميغابايت/ثانية (حوالي 68% من الحد النظري لـ S3)، أي أعلى بـ 5.8 ضعف من Kafka و7 أضعاف من Pulsar. في حين أن زمن الاستجابة أعلى (166 مللي ثانية)، يوفر هذا الإعداد إنتاجية استثنائية لأحمال العمل الموجهة نحو الدُفعات.</p></li>
...
<ul>
<li><p>استيعاب بيانات عالية الإنتاجية مع ثبات دائم عبر منطقة التوافر</p></li>
...
<p><strong>وضع MemoryBuffer - خفيف الوزن وخالي من الصيانة</strong></p>
...
<h2 id="Conclusion---Streaming-on-a-Zero-Disk-Architecture" class="common-anchor-header">خاتمة - التدفق على بنية خالية من الأقراص<button data-href="#Conclusion---Streaming-on-a-Zero-Disk-Architecture" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Getting-Started-with-Milvus-26" class="common-anchor-header">بدء العمل مع Milvus 2.6<button data-href="#Getting-Started-with-Milvus-26" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"

...
---
<p>إذا كنت تعمل على أنظمة الواجهة الخلفية أو التخزين الموزعة، فربما تكون قد رأيت ذلك: الشبكة غير مشبعة، والأجهزة غير مثقلة بالأعباء، ومع ذلك فإن عملية بحث بسيطة تؤدي إلى آلاف عمليات الإدخال/الإخراج على القرص أو استدعاءات واجهة برمجة تطبيقات تخزين الكائنات - ولا يزال الاستعلام يستغرق ثوانٍ.</p>
...
<p>هذه هي نفس الفكرة وراء كل نظام تخزين عالي الأداء سنفحصه: استبدل عمليات المسح بالحساب، ومسارات البحث غير المباشرة بالعنونة المباشرة.</p>
...
<p>في المركز توجد عقدة الاسم. حيث تقوم بتحميل شجرة نظام الملفات بالكامل - بنية الدليل، وتعيينات من ملف إلى كتلة، وتعيينات من كتلة إلى عقدة بيانات - في الذاكرة. نظرًا لأن البيانات الوصفية لا تلامس القرص أبدًا أثناء القراءة، فإن HDFS يحل جميع أسئلة العنونة من خلال عمليات البحث في الذاكرة فقط.</p>
...
<li>ملف <code translate="no">.index</code> يعمل بمثابة فهرس متناثر في السجل</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="How-Does-Kafka-Locate-Data" class="common-anchor-header">كيف يحدد كافكا موقع البيانات؟</h3><p>لنفترض أن المستهلك يقرأ الرسالة عند الإزاحة 500,000. تحل كافكا هذا في ثلاث خطوات:</p>
...
<p><strong>3. القراءة المتسلسلة من السجل</strong> (.log)</p>
<ul>
...
<ul>
<li><strong>استعلامات أكثر ثراءً.</strong> تتعامل الأنظمة الحديثة مع عوامل التصفية متعددة الحقول، <a href="https://zilliz.com/glossary/similarity-search">والبحث عن التشابه،</a> والمسندات المعقدة - وليس فقط المفاتيح البسيطة والإزاحات.</li>
...
<ul>
<li>تُخزَّن<strong> <a href="https://milvus.io/docs/scalar_index.md">الحقول القياسية</a> الصغيرة</strong> (مثل المعرف والطابع الزمني) معًا</li>
...
<li><strong>اقرأ ما هو مطلوب فقط</strong> - يقرأ تقليم الأعمدة في الباركيه عمود المعرف فقط من ملف الحقل الصغير وعمود <a href="https://milvus.io/docs/index-vector-fields.md">المتجه</a> فقط من ملف الحقل الكبير. يتم الوصول إلى مجموعات الصفوف المطابقة فقط.</li>
</ol>
...
<p>
   ...
   </span> <span class="img-wrapper"> <span>بنية Iceberg المكونة من أربع طبقات: يشير metadata.json إلى قوائم البيانات، والتي تشير إلى ملفات البيان التي تحتوي على إحصائيات على مستوى الملف، والتي تشير إلى ملفات بيانات الباركيه الفعلية</span> </span></p>
...
<li><strong>قراءة ملفات البيان</strong> (2 إدخال/إخراج) - استخدم إحصائيات على مستوى الملف (الحد الأدنى/الأقصى للتاريخ، الحد الأدنى/الأقصى للمبلغ) لحذف الملفات التي لا تتطابق مع الاستعلام</li>
...
<p>إنشاء عدد كبير جدًا من الملفات الصغيرة. يحتوي كل طلب S3 GET على حد زمني ثابت للكمون (حوالي 50 مللي ثانية)، بغض النظر عن كمية البيانات التي يتم إرجاعها. فالنظام الذي يقرأ 10000 ملف صغير يستغرق 500 ثانية من زمن الاستجابة - حتى لو كان إجمالي حجم البيانات متواضعًا. الإصلاح هو الدمج: دمج الملفات الصغيرة في ملفات أكبر، واستخدام تنسيقات عمودية مثل Parquet للقراءات الانتقائية، والاحتفاظ بالبيانات الوصفية التي تتيح لك تخطي الملفات بالكامل.</p>

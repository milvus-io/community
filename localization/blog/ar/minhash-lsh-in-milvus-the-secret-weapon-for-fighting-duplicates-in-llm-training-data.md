...
<h3 id="Prerequisite-Generating-MinHash-Signatures" class="common-anchor-header">المتطلبات الأساسية: إنشاء توقيعات MinHash MinHash</h3><p>يعالج Milvus فهرسة توقيعات MinHash <strong>التي تم إنشاؤها مسبقًا</strong> والبحث فيها. ستحتاج إلى إنشاء هذه التواقيع أثناء المعالجة المسبقة باستخدام أدوات مثل <code translate="no">datasketch</code> في Python أو تطبيق مخصص. الخطوات النموذجية هي</p>
<ol>

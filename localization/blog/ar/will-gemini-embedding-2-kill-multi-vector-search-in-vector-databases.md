...
desc: >-
  تضمين Gemini 2 من Google يدمج النص والصور والفيديو والصوت في متجه واحد. هل هذا
  يجعل البحث متعدد المتجهات عفا عليه الزمن؟ نحن نحلل أين يناسب كل نهج.
origin: >-
...
<p>قبل نماذج كهذه، لم يكن لديك خيار سوى تشغيل كل طريقة من خلال نموذج متخصص خاص بها وتخزين كل مخرج في عمود متجه منفصل. صُممت الأعمدة متعددة المتجهات في قواعد البيانات المتجهة مثل <a href="https://milvus.io/docs/multi-vector-search.md">ميلفوس</a> خصيصًا لهذا العالم.</p>
<p>لذا فإن السؤال الآن هو: ما مقدار هذا التعقيد الذي يمكن أن يحل محل Gemini Embedding 2 - وأين يقصر؟ يستعرض هذا المنشور أين يناسب كل نهج وكيف يعملان معًا.</p>
...
<p>على سبيل المثال، كانت فهرسة بودكاست مع غلاف فني يعني تشغيل CLIP للصورة، و CLAP للصوت، وتشفير النص للنص - ثلاثة نماذج، وثلاثة مساحات متجهة، ومنطق دمج مخصص لجعل نتائجها قابلة للمقارنة في وقت الاستعلام.</p>
...
<p>صورة<strong>إدخال مختلطة</strong> + نص معًا في مكالمة تضمين واحدة</p>
...
<p><strong>الآن،</strong> يمكنك إدخال إطارات الفيديو والصوت والعناوين الفرعية في مكالمة واجهة برمجة تطبيقات واحدة والحصول على متجه واحد موحد يلتقط الصورة الدلالية الكاملة.</p>
...
<p>يقوم<a href="https://milvus.io/">Milvus</a> بتشغيل كلا النموذجين <a href="https://milvus.io/docs/multi-vector-search.md">كعمليات بحث ANN</a> متوازية مع إعادة ترتيب أصلي عبر RRFRanker. تعريف المخطط، والتكوين متعدد الفهارس، و BM25 المدمج - كل ذلك يتم التعامل معه في نظام واحد.</p>
<p>لتوضيح ذلك، ضع في اعتبارك كتالوج منتج حيث يتضمن كل عنصر وصفًا نصيًا وصورة. يمكنك إجراء ثلاث عمليات بحث مقابل تلك البيانات بالتوازي:</p>
<ul>
<li><strong>بحث نصي دلالي.</strong> الاستعلام عن الوصف النصي باستخدام متجهات كثيفة تم إنشاؤها بواسطة نماذج مثل <a href="https://zilliz.com/learn/explore-colbert-token-level-embedding-and-ranking-model-for-similarity-search?_gl=1*d243m9*_gcl_au*MjcyNTAwMzUyLjE3NDMxMzE1MjY.*_ga*MTQ3OTI4MDc5My4xNzQzMTMxNTI2*_ga_KKMVYG8YF2*MTc0NTkwODU0Mi45NC4xLjE3NDU5MDg4MzcuMC4wLjA.#A-Quick-Recap-of-BERT">BERT</a> أو <a href="https://zilliz.com/learn/NLP-essentials-understanding-transformers-in-AI?_gl=1*d243m9*_gcl_au*MjcyNTAwMzUyLjE3NDMxMzE1MjY.*_ga*MTQ3OTI4MDc5My4xNzQzMTMxNTI2*_ga_KKMVYG8YF2*MTc0NTkwODU0Mi45NC4xLjE3NDU5MDg4MzcuMC4wLjA.">Transformers</a> أو واجهة برمجة تطبيقات <a href="https://zilliz.com/learn/guide-to-using-openai-text-embedding-models">OpenAI</a> المضمنة.</li>
...
<h2 id="Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="common-anchor-header">الآن، مع تضمين الجوزاء 2، هل لا يزال البحث متعدد المتجهات مهمًا؟<button data-href="#Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>يعالج Gemini Embedding 2 المزيد من الطرائق في مكالمة واحدة، مما يبسط خطوط الأنابيب إلى حد كبير. لكن التضمين الموحد متعدد الوسائط ليس هو نفسه استرجاع متعدد النواقل. وبعبارة أخرى، لن يصبح البحث متعدد النواقل قديمًا.</p>
<p>يقوم Gemini Embedding 2 بتعيين النصوص والصور والفيديو والصوت والمستندات في مساحة متجهية واحدة مشتركة. <a href="https://developers.googleblog.com/en/gemini-embedding-model-now-available/">تضعه</a> Google للبحث الدلالي متعدد الوسائط واسترجاع المستندات والتوصيات - وهي سيناريوهات تصف فيها جميع الوسائط نفس المحتوى ويؤدي التداخل الكبير بين الوسائط إلى جعل متجه واحد قابل للتطبيق.</p>
...
<tr><td>المتجهات تعيش في فضاءات دلالية مختلفة جوهرياً (القياسات الحيوية، السمات المنظمة)</td><td>أعمدة متعددة المتجهات مع فهارس لكل حقل</td><td>مقاييس تشابه مستقلة وأنواع فهرس مستقلة لكل حقل متجه</td></tr>
...
<p>هذان الأسلوبان لا يستبعد أحدهما الآخر. يمكنك استخدام تضمين Gemini Embedding 2 للعمود متعدد الوسائط الموحد مع الاستمرار في تخزين متجهات إضافية متفرقة أو سمة خاصة في أعمدة منفصلة داخل نفس المجموعة <a href="https://milvus.io/"></a><a href="https://milvus.io/">Milvus</a>.</p>
<h2 id="Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="common-anchor-header">بداية سريعة: إعداد Gemini Embedding 2 + Milvus<button data-href="#Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>إليك عرض توضيحي عملي. أنت بحاجة إلى تشغيل <a href="https://milvus.io/docs/install-overview.md"></a><a href="https://milvus.io/docs/install-overview.md">Milvus أو مثيل Zilliz Cloud</a> و GOOGLE_API_KEY.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>بالنسبة لتضمينات الصور والصوت، استخدم embed_image() و embed_image() و embed_audio() بنفس الطريقة - حيث تهبط المتجهات في نفس المجموعة ونفس مساحة المتجهات، مما يتيح البحث الحقيقي عبر الوسائط.</p>
...
<p>هذا يعني أنك ستحصل على تضمين موحد متعدد الوسائط من Gemini حيثما يناسبك، ومجموعة أدوات Milvus الكاملة متعددة المتجهات - البحث الهجين المتناثر الكثيف، والمخططات متعددة الفهارس، وإعادة الترتيب - حيثما تحتاج إلى تحكم دقيق.</p>

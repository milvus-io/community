...
title: الشروع في استخدام البحث الدلالي المختلط/البحث بالنص الكامل مع ميلفوس 2.5
author: Stefan Webb
...
<p>بعد ذلك نفتح قاعدة البيانات باستخدام Milvus Standalone، أي محليًا، وننشئ مخطط البيانات. يتألف المخطط من مفتاح أساسي عدد صحيح، وسلسلة نصية، ومتجه كثيف بأبعاد 384، ومتجه متناثر (بأبعاد غير محدودة). لاحظ أن ميلفوس لايت لا يدعم حاليًا البحث في النص الكامل، فقط ميلفوس ستاندالوني وميلفوس الموزع.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>ربما لاحظت المعلمة <code translate="no">enable_analyzer=True</code>. هذا يخبر ميلفوس 2.5 بتمكين المحلل المعجمي على هذا الحقل وبناء قائمة بالرموز وترددات الرموز الرمزية، وهي مطلوبة للبحث في النص الكامل. سيحتوي الحقل <code translate="no">sparse</code> على تمثيل متجه للوثائق كحقيبة من الكلمات الناتجة عن التحليل <code translate="no">text</code>.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>دعونا أولاً نوضح أولاً البحث عن النص الكامل قبل أن ننتقل إلى البحث المختلط:</p>

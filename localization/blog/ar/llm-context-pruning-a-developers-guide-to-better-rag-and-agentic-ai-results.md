...
---
<p>لقد أصبحت نوافذ السياق في LLMs ضخمة في الآونة الأخيرة. يمكن أن تأخذ بعض النماذج مليون رمز أو أكثر في مسار واحد، ويبدو أن كل إصدار جديد يرفع هذا الرقم إلى أعلى. إنه أمر مثير، ولكن إذا كنت قد بنيت بالفعل أي شيء يستخدم سياقًا طويلًا، فأنت تعلم أن هناك فجوة بين ما هو <em>ممكن</em> وما هو <em>مفيد</em>.</p>
<p>
...
<p>في هذا المقال، سنتحدث عن السبب الذي يجعل السياق الطويل يتصرف غالبًا بشكل مختلف عما تتوقعه، وكيف يساعد التقليم في إبقاء الأمور تحت السيطرة، وكيف تتناسب أدوات التقليم مثل <strong>بروفانس</strong> مع خطوط أنابيب RAG الحقيقية دون أن تجعل إعدادك أكثر تعقيدًا.</p>
...
<p>تقليم السياق، بعبارات بسيطة، هو عملية إزالة المعلومات غير ذات الصلة أو منخفضة القيمة أو المتضاربة تلقائيًا قبل أن تدخل إلى نافذة سياق النموذج. إنه في الأساس مرشح يحتفظ فقط بالأجزاء النصية الأكثر أهمية للمهمة الحالية.</p>
...
<p>يحتوي بروفانس أيضًا على متغير يسمى XProvence، والذي يستخدم نفس البنية ولكن يتم تدريبه على بيانات متعددة اللغات. وهذا يمكّنه من تقييم الاستعلامات والمستندات عبر اللغات - مثل الصينية والإنجليزية والكورية - مما يجعله مناسبًا لأنظمة RAG متعددة اللغات وعبر اللغات.</p>
...
<li><p><strong>تصنيف الصلة على مستوى الرمز المميز (قناع ثنائي):</strong> بالتوازي مع ذلك، يعيّن النموذج تسمية ثنائية لكل رمز مميز، مع تحديد ما إذا كان ذا صلة (<code translate="no">1</code>) أو غير ذي صلة (<code translate="no">0</code>) بالاستعلام.</p></li>
</ol>
...
<p>نظرًا لأن العملية الأساسية متشابهة جدًا، يمكن للنموذج نفسه غالبًا تشغيل كلتا الميزتين. وهذا يجعل من السهل إعادة استخدام المكونات عبر المكدس ويبقي نظام RAG أبسط وأكثر كفاءة بشكل عام.</p>
...
<p>سنستمر في إجراء التجارب وقياس الأداء وشحن الأجزاء التي تُحدث فرقًا فعليًا للمطورين. الهدف واضح ومباشر: تسهيل بناء أنظمة لا تتعطل في ظل البيانات الفوضوية أو الاستعلامات غير المتوقعة أو أعباء العمل واسعة النطاق.</p>

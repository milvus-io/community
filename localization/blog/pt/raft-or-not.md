...
<p>A replicação baseada em consenso é uma estratégia amplamente adotada em muitos bancos de dados distribuídos e nativos da nuvem. No entanto, ela tem algumas deficiências e definitivamente não é uma solução mágica.</p>
<p>Este post tem como objetivo explicar os conceitos de replicação, consistência e consenso em um banco de dados distribuído e nativo da nuvem, em seguida, esclarecer por que algoritmos baseados em consenso como Paxos e Raft não são a bala de prata e, finalmente, propor uma <a href="#a-log-replication-strategy-for-cloud-native-and-distributed-database">solução para a replicação baseada em consenso</a>.</p>
...
<p>É de notar que um sistema distribuído não suporta necessariamente a linearização, mesmo que adopte uma replicação baseada no consenso. No entanto, a linearização é o pré-requisito para a construção de uma base de dados distribuída ACID.</p>
...
<p>Para a replicação multi-datacenter e multi-nuvem, a busca pela consistência compromete não apenas a disponibilidade, mas também <a href="https://en.wikipedia.org/wiki/PACELC_theorem">a latência</a>, resultando em um declínio no desempenho. Por conseguinte, a linearização não é uma necessidade para a tolerância a desastres em vários centros de dados na maioria das aplicações.</p>
...
<h3 id="1-Replication-as-a-service" class="common-anchor-header">1. Replicação como um serviço</h3><p>É necessário um microsserviço separado dedicado à sincronização de dados. O módulo de sincronização e o módulo de armazenamento não devem mais ser fortemente acoplados dentro do mesmo processo.</p>
...
<li>Verbitski A, Gupta A, Saha D, et al. Amazon aurora: Evitar o consenso distribuído para i/os, commits e alterações de membros[C]//Proceedings of the 2018 International Conference on Management of Data. 2018: 789-796.</li>

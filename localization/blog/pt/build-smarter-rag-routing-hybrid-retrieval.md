...
<p><strong>O custo é escalonado com a sua base de conhecimento, não com a sua consulta.</strong> Cada solicitação envia a base de conhecimento completa através do modelo. Para um corpus de 100 mil tokens, são 100 mil tokens de entrada por solicitação - independentemente de a resposta exigir um parágrafo ou dez. Os custos mensais de inferência crescem linearmente com o tamanho do corpus.</p>
...
<li><p><strong>Recuperação frágil em consultas complexas.</strong> Frases ambíguas, sinónimos ou consultas em várias línguas são muitas vezes derrotadas pela semelhança vetorial pura. Quando a recuperação não atinge documentos relevantes, a qualidade da geração cai sem qualquer alternativa.</p></li>
</ol>
...
<h3 id="Node-4-Minimal-Context-Generation--How-Much-Context-Does-the-Model-Need" class="common-anchor-header">Nó 4: Geração de contexto mínimo - De quanto contexto precisa o modelo?</h3><p>Após a recuperação e a <a href="https://zilliz.com/learn/optimize-rag-with-rerankers-the-role-and-tradeoffs">nova classificação</a>, o sistema envia apenas as passagens mais relevantes para o modelo - não documentos inteiros.</p>
...
<h2 id="Why-Hybrid-Retrieval-Matters-for-Enterprise-RAG" class="common-anchor-header">Porque é que a recuperação híbrida é importante para o RAG empresarial<button data-href="#Why-Hybrid-Retrieval-Matters-for-Enterprise-RAG" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Na prática, a seleção da estratégia de recuperação (Nó 3) é onde a maioria das equipas fica bloqueada. Nenhum método de recuperação único abrange todos os tipos de documentos empresariais.</p>
...
<p><a href="https://milvus.io/">O Milvus</a> 2.6 suporta tanto vectores densos (para pesquisa semântica) como vectores esparsos (para pesquisa de palavras-chave do tipo BM25) na mesma coleção. Uma única chamada à API devolve resultados fundidos, com o comportamento de recuperação ajustável alterando o peso entre os tipos de vectores. Sem índices separados, sem problemas de sincronização, sem latência de fusão.</p>
...
<ol>
<li><p><strong>Adicionar roteamento antecipadamente.</strong> Muitas consultas não precisam de recuperação. Filtrá-las antecipadamente reduz a carga e melhora o tempo de resposta com o mínimo de esforço de engenharia.</p></li>
...
<hr>
<p>Algumas questões que surgem frequentemente quando as equipas começam a criar sistemas RAG mais inteligentes:</p>
...
<p>Roteamento de consultas. É a melhoria de maior impacto e menor esforço. A maioria dos sistemas de produção vê uma parte significativa de consultas que não precisam de ser recuperadas - perguntas de senso comum, cálculos simples, conhecimentos gerais. Encaminhá-las diretamente para o LLM reduz as chamadas de recuperação desnecessárias e melhora imediatamente o tempo de resposta.</p>

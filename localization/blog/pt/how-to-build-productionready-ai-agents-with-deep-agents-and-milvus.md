...
<p>Utilizando uma ferramenta <code translate="no">task</code>, o agente principal pode delegar subtarefas a sub-agentes especializados. Cada subagente tem sua própria janela de contexto e ferramentas, o que ajuda a manter o trabalho organizado.</p>
<p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Middleware" class="common-anchor-header">Middleware</h3><p>Você pode escrever middleware personalizado para:</p>
<ul>
...
<tr><td>FilesystemMiddleware</td><td>Fornece ferramentas de operação de ficheiros e guarda automaticamente os resultados de ferramentas de grande dimensão</td></tr>
...
<p>Para permitir a memória persistente, utilizamos <a href="https://milvus.io/"><strong>o Milvus</strong></a> como base de dados vetorial juntamente com <code translate="no">StoreBackend</code>. Funciona assim: o conteúdo importante da conversa e os resultados da ferramenta são convertidos em embeddings (vectores numéricos que representam o significado) e armazenados no Milvus. Quando se inicia uma nova tarefa, o agente efectua uma pesquisa semântica para recuperar memórias passadas relacionadas. Isto permite ao agente "recordar" informações relevantes de sessões anteriores.</p>

...
<p>Juntos, estes quatro desafios definem os principais constrangimentos da recuperação conversacional. Qualquer sistema que pretenda implementar a recuperação a pedido, ao estilo de Claude, deve abordar todos eles de forma coordenada.</p>
...
<h3 id="JSON-Shredding-and-Fast-Scalar-Filtering" class="common-anchor-header">Destruição de JSON e filtragem escalar rápida</h3><p>Milvus 2.6 habilita JSON Shredding por padrão, achatando campos JSON aninhados em armazenamento colunar. Isso melhora o desempenho da filtragem escalar em 3-5× de acordo com os benchmarks oficiais (os ganhos reais variam de acordo com o padrão de consulta).</p>
<p>A recuperação de conversação requer frequentemente a filtragem por metadados, como ID de utilizador, ID de sessão ou intervalo de tempo. Com o JSON Shredding, consultas como <em>"todas as conversas do usuário A na última semana"</em> podem ser executadas diretamente em índices colunares, sem analisar repetidamente blobs JSON completos.</p>
...
<p>Então, porque é que os dois sistemas escolheram caminhos diferentes? Com os constrangimentos técnicos acima descritos, a resposta torna-se clara: <strong>cada arquitetura só é viável se a infraestrutura subjacente a puder suportar</strong>.</p>

...
desc: >-
  ...
  vetor. Será que isso torna a pesquisa multi-vetorial obsoleta? Analisamos onde
  cada abordagem se encaixa.
origin: >-
...
<p>Pode incorporar um videoclipe, uma fotografia de um produto e um parágrafo de texto com uma chamada à API, e todos eles ficam na mesma vizinhança semântica.</p>
<p>Antes de modelos como este, não havia outra opção senão executar cada modalidade através do seu próprio modelo especializado e armazenar cada saída numa coluna vetorial separada. As colunas multi-vectoriais em bases de dados vectoriais como o <a href="https://milvus.io/docs/multi-vector-search.md">Milvus</a> foram criadas precisamente para esse mundo.</p>
<p>Por isso, a questão que se coloca agora é: que parte dessa complexidade pode o Gemini Embedding 2 substituir - e onde é que fica aquém? Este post mostra onde cada abordagem se encaixa e como elas funcionam juntas.</p>
...
<p><strong>Agora</strong>, alimenta os fotogramas, o áudio e as legendas do vídeo numa única chamada à API e obtém um vetor unificado que capta toda a imagem semântica.</p>
...
<p><a href="https://milvus.io/">Milvus</a> executa ambos os padrões como <a href="https://milvus.io/docs/multi-vector-search.md">pesquisas ANN</a> paralelas com reranking nativo via RRFRanker. Definição de esquemas, configuração de multi-índices, BM25 incorporado - tudo tratado num único sistema.</p>
<p>Para tornar isto concreto, considere um catálogo de produtos em que cada item inclui uma descrição de texto e uma imagem. Pode efetuar três pesquisas em paralelo com esses dados:</p>
<ul>
<li><strong>Pesquisa de texto semântico.</strong> Consultar a descrição do texto com vectores densos gerados por modelos como o <a href="https://zilliz.com/learn/explore-colbert-token-level-embedding-and-ranking-model-for-similarity-search?_gl=1*d243m9*_gcl_au*MjcyNTAwMzUyLjE3NDMxMzE1MjY.*_ga*MTQ3OTI4MDc5My4xNzQzMTMxNTI2*_ga_KKMVYG8YF2*MTc0NTkwODU0Mi45NC4xLjE3NDU5MDg4MzcuMC4wLjA.#A-Quick-Recap-of-BERT">BERT</a>, <a href="https://zilliz.com/learn/NLP-essentials-understanding-transformers-in-AI?_gl=1*d243m9*_gcl_au*MjcyNTAwMzUyLjE3NDMxMzE1MjY.*_ga*MTQ3OTI4MDc5My4xNzQzMTMxNTI2*_ga_KKMVYG8YF2*MTc0NTkwODU0Mi45NC4xLjE3NDU5MDg4MzcuMC4wLjA.">Transformers</a> ou a API de embeddings <a href="https://zilliz.com/learn/guide-to-using-openai-text-embedding-models">do OpenAI</a>.</li>
...
<h2 id="Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="common-anchor-header">Agora, com o Gemini Embedding 2, a pesquisa multi-vetorial ainda é importante?<button data-href="#Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>O Gemini Embedding 2 lida com mais modalidades em uma chamada, o que simplifica consideravelmente os pipelines. Mas uma incorporação multimodal unificada não é a mesma coisa que a recuperação multi-vetorial. Por outras palavras, a pesquisa multi-vetorial não se tornará obsoleta.</p>
...
<p>Estas duas abordagens não são mutuamente exclusivas. É possível usar o Gemini Embedding 2 para a coluna multimodal unificada e ainda armazenar vetores adicionais esparsos ou específicos de atributos em colunas separadas dentro da mesma coleção <a href="https://milvus.io/"></a><a href="https://milvus.io/">Milvus</a>.</p>
<h2 id="Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="common-anchor-header">Início Rápido: Configurar o Gemini Embedding 2 + Milvus<button data-href="#Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Aqui está uma demonstração funcional. Você precisa de uma<a href="https://milvus.io/docs/install-overview.md">instância</a> em execução<a href="https://milvus.io/docs/install-overview.md">do</a> <a href="https://milvus.io/docs/install-overview.md"></a><a href="https://milvus.io/docs/install-overview.md">Milvus ou do Zilliz Cloud</a> e de uma GOOGLE_API_KEY.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Para embeddings de imagem e áudio, use embed_image() e embed_audio() da mesma forma - os vetores ficam na mesma coleção e no mesmo espaço vetorial, permitindo uma verdadeira busca cross-modal.</p>
...
<p>Quer experimentar? Comece com o <a href="https://milvus.io/docs/quickstart.md">quickstart do Milvus</a> e execute a demonstração acima, ou consulte o <a href="https://milvus.io/docs/hybrid_search_with_milvus.md">guia de pesquisa híbrida</a> para a configuração completa de multi-vetor com BGE-M3. Traga suas perguntas para o <a href="https://milvus.io/discord">Discord</a> ou para o <a href="https://meetings.hubspot.com/chloe-williams1/milvus-office-hour">Milvus Office Hours</a>.</p>

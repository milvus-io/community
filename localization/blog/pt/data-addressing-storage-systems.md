...
<li>Localizar<strong>a linha de destino</strong> - Examinar os dados de id carregados para encontrar <code translate="no">id = 123</code></li>
...
<tr><td>HDFS</td><td>Caminho → bloco → DataNode</td><td>HashMaps na memória + cálculo de blocos</td><td>1 RPC + N leituras de blocos</td></tr>
...
<p>Criar demasiados ficheiros pequenos. Cada solicitação GET do S3 tem um piso de latência fixo (~50 ms), independentemente da quantidade de dados que retorna. Um sistema que lê 10.000 pequenos ficheiros serializa 500 segundos de latência - mesmo que o volume total de dados seja modesto. A solução é a consolidação: mesclar arquivos pequenos em arquivos maiores, usar formatos colunares como Parquet para leituras seletivas e manter metadados que permitam ignorar arquivos completamente.</p>

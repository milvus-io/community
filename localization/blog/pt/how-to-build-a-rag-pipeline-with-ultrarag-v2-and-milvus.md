...
title: Como criar um pipeline RAG com o UltraRAG v2 e o Milvus
author: Min Yin
date: 2026-3-11
...
desc: >-
  Saiba como criar um pipeline RAG utilizando o UltraRAG v2 e o Milvus com MCP,
  componentes modulares e fluxos de trabalho baseados em YAML.
origin: >-
  https://milvus.io/blog/how-to-build-a-rag-pipeline-with-ultrarag-v2-and-milvus.md
---
<p>A Geração Aumentada por Recuperação (RAG) evoluiu muito para além do simples padrão "recuperar e depois gerar". Os sistemas modernos comportam-se agora mais como motores de raciocínio completos, combinando a recuperação adaptativa, o planeamento em várias etapas e a tomada de decisões dinâmica. Mas este progresso traz consigo dois grandes desafios: <strong>o elevado custo de engenharia</strong> e <strong>a crescente complexidade do sistema</strong>. Reproduzir métodos existentes requer muitas vezes a reconstrução de condutas complexas, enquanto experimentar novas ideias exige um trabalho de orquestração significativo.</p>
<p><a href="https://github.com/OpenBMB/UltraRAG">O UltraRAG v2</a> aborda diretamente estes pontos problemáticos. Desenvolvido por THUNLP, NEUIR, OpenBMB e AI9stars, é a primeira estrutura RAG construída sobre o Protocolo de Contexto de Modelo (MCP). Em vez de escrever à mão uma lógica complexa, os investigadores podem declarar sequências, loops e comportamentos de ramificação em ficheiros YAML simples, permitindo a construção rápida e com pouco código de sistemas RAG de várias fases.</p>
<p>Mesmo com uma estrutura como o UltraRAG, um sistema RAG ainda precisa de uma camada de recuperação forte. É aqui que uma base de dados vetorial ajuda. <a href="https://milvus.io/">Milvus</a>, uma base de dados vetorial de código aberto, armazena embeddings, constrói índices e efectua pesquisas rápidas de semelhanças em grandes conjuntos de dados. Num pipeline UltraRAG, o Milvus recupera informações relevantes para o modelo. Juntos, o UltraRAG e o Milvus facilitam a criação de sistemas RAG que são flexíveis e eficientes.</p>
<p>Nesta publicação, mostraremos como integrar o Milvus no UltraRAG v2 e criar um pipeline RAG completo.</p>
<h2 id="UltraRAG-v2-Architecture-at-a-Glance" class="common-anchor-header">Visão geral da arquitetura do UltraRAG v2<button data-href="#UltraRAG-v2-Architecture-at-a-Glance" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Em diferentes sistemas RAG, as funções principais, como a recuperação, a geração e a avaliação, são semelhantes mas implementadas de formas diferentes, o que dificulta a reutilização ou a combinação de componentes. O MCP resolve este problema normalizando a forma como os LLM comunicam com ferramentas externas através de uma arquitetura simples Cliente-Servidor.</p>
<p>Inspirado por esta arquitetura, o UltraRAG v2 foi construído em torno de três ideias centrais:</p>
<ul>
<li><strong>Encapsulamento modular:</strong> O UltraRAG v2 agrupa os principais recursos do RAG em servidores MCP independentes com interfaces de ferramentas unificadas. Isto cria uma estrutura limpa e modular onde se pode concentrar na lógica de raciocínio e não na cablagem de backend. Novos componentes podem ser adicionados, substituídos ou atualizados como plugins, sem necessidade de edições no código principal.</li>
<li><strong>Configuração YAML:</strong> Os pipelines RAG complexos e de várias etapas são difíceis de depurar. O UltraRAG v2 torna-os transparentes ao mover toda a lógica de controlo para YAML. Sequências, loops e ramificações condicionais são definidos de forma declarativa, e as entradas e saídas de cada etapa são claramente rastreáveis. Isso simplifica muito a depuração e acelera a iteração do fluxo de trabalho.</li>
<li><strong>Orquestração de fluxo de trabalho leve:</strong> Um cliente MCP integrado executa o pipeline, mantendo o comportamento do fluxo de trabalho totalmente desacoplado das implementações subjacentes. Enquanto os sistemas RAG tradicionais requerem frequentemente a edição do código principal para adicionar novas funcionalidades, o UltraRAG v2 adopta um modelo semelhante a um micro-serviço em que os novos módulos podem ser implementados independentemente, tal como a instalação de um plug-in.</li>
</ul>
<p>
...
<h2 id="Why-and-How-to-Integrate-Milvus-into-the-UltraRAG-Pipeline" class="common-anchor-header">Porquê e como integrar o Milvus no pipeline do UltraRAG<button data-href="#Why-and-How-to-Integrate-Milvus-into-the-UltraRAG-Pipeline" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Na pilha UltraRAG v2, a base de dados de vectores desempenha um papel fundamental na qualidade da recuperação e no desempenho do sistema. <strong>O Milvus</strong>, uma base de dados de vectores de código aberto, é uma boa opção graças à sua escalabilidade, indexação eficiente e capacidades de integração perfeita.</p>
<p>Assim que o Milvus estiver integrado num pipeline UltraRAG, pode criar índices e executar consultas utilizando comandos simples, como ultrarag build e ultrarag run. O UltraRAG carregará automaticamente a sua configuração e coordenará todos os módulos necessários para concluir a tarefa.</p>
<p>Nesta demonstração, iremos percorrer quatro objectivos:</p>
<ol>
<li><p>Integrar o Milvus num projeto UltraRAG v2</p></li>
<li><p>Criar um pipeline personalizado que utilize o Milvus para recuperação</p></li>
<li><p>Executar o pipeline completo para verificar se tudo funciona de ponta a ponta</p></li>
<li><p>Verificar os resultados da execução (opcional)</p></li>
</ol>
<h3 id="Dataset-Setup" class="common-anchor-header">Configuração do conjunto de dados</h3><p>Para esta demonstração, utilizaremos o conjunto de dados Milvus FAQ do repositório oficial do Milvus. O conjunto de dados é fornecido no formato JSONL.</p>
...
<h3 id="Step-1-Deploy-the-Milvus-Vector-Database" class="common-anchor-header">Etapa 1: implantar o banco de dados de vetores do Milvus</h3><p><strong>Descarregar os ficheiros de implementação</strong></p>
<pre><code translate="no">wget https://github.com/Milvus-io/Milvus/releases/download/v2.5.12/Milvus-standalone-docker-compose.yml -O docker-compose.yml
<button class="copy-code-btn"></button></code></pre>
<p><strong>Iniciar o serviço Milvus</strong></p>
...
<h3 id="Step-2-Clone-the-Project" class="common-anchor-header">Etapa 2: Clonar o projeto</h3><pre><code translate="no">git <span class="hljs-built_in">clone</span> https://github.com/OpenBMB/UltraRAG.git
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Implement-the-Pipeline" class="common-anchor-header">Etapa 3: implementar o pipeline</h3><p><strong>Integrar o banco de dados vetorial do Milvus</strong></p>
<p>Nota: Milvus é adicionado como um dos tipos de banco de dados vetoriais suportados no módulo de recuperação.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Definir o arquivo de configuração de parâmetros</strong></p>
<p>Nota: Este arquivo especifica todas as configurações dos parâmetros usados no pipeline.</p>
...
embedding_dim: 1024
<button class="copy-code-btn"></button></code></pre>
<p><strong>Definir o arquivo de configuração do servidor</strong></p>
<p>Observação: essa configuração inclui a integração com as APIs do Alibaba Cloud.</p>
...
retriever:
  ...
  tools:
    ...
    retriever_search_Milvus:
      ...
      - ret_psg
<button class="copy-code-btn"></button></code></pre>
<p><strong>Definir o</strong> <strong>índice de construção</strong></p>
<p>Observação: Converta o corpus do documento em embeddings de vetor e armazene-os no Milvus. Configure os principais parâmetros de indexação necessários para esse processo.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>Executar RAG</strong></p>
<p>Executa o fluxo de trabalho completo do RAG, incluindo recuperação, geração de respostas e avaliação. Configure os principais parâmetros de indexação necessários para este processo.</p>
...
- evaluation.evaluate
<button class="copy-code-btn"></button></code></pre>
<p><strong>Executar a criação de índice</strong></p>
<p>Nota: Após uma execução bem sucedida, o sistema gera as incorporações vectoriais e os ficheiros de índice. O pipeline RAG pode então usá-los diretamente para executar a recuperação.</p>
...
<p><strong>Executar a consulta RAG</strong></p>
<p>Nota: Construa e execute o pipeline RAG completo de ponta a ponta.</p>
...
<h2 id="Conclusion" class="common-anchor-header">Conclusão<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Construir um pipeline RAG tradicional significa frequentemente escrever centenas ou mesmo milhares de linhas de código. O UltraRAG v2 adopta uma abordagem muito diferente. Com seu design modular baseado em MCP, configuração YAML declarativa e modelo de orquestração leve, é possível construir o mesmo pipeline de ponta a ponta em apenas algumas dezenas de linhas. Além disso, o YAML é usado para descrever as etapas do fluxo de trabalho. Na verdade, não é necessário escrever código real. Isto torna muito mais fácil trazer RAG de nível empresarial para aplicações do mundo real.</p>
<p>A integração do Milvus neste fluxo de trabalho acrescenta outra vantagem: uma base de dados vetorial de elevado desempenho e pronta para produção, concebida especificamente para uma recuperação semântica escalável. Juntos, o Milvus e o UltraRAG v2 tornam muito mais fácil criar protótipos rapidamente, iterar com confiança e implantar sistemas RAG que podem lidar com cargas de trabalho reais.</p>
<p><strong>Pronto para simplificar o seu desenvolvimento RAG?</strong> Experimente o UltraRAG v2 com o Milvus. Explore o pipeline de exemplo, execute-o você mesmo e crie um fluxo de trabalho RAG completo com apenas algumas linhas de configuração.</p>
<p>Se tiver alguma dúvida, junte-se ao nosso <a href="https://milvusio.slack.com/join/shared_invite/zt-3nntzngkz-gYwhrdSE4~76k0VMyBfD1Q#/shared-invite/email">canal Slack</a> ou marque uma sessão de 20 minutos <a href="https://milvus.io/blog/join-milvus-office-hours-to-get-support-from-vectordb-experts.md">do Milvus Office Hours</a> para discutir o seu caso de utilização.</p>

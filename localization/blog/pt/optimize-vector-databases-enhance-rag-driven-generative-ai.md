...
<li><strong>A recuperação</strong> é a proporção de correspondências verdadeiras, ou itens relevantes, encontrados nos <a href="https://redis.io/docs/data-types/probabilistic/top-k/">K</a> resultados <a href="https://redis.io/docs/data-types/probabilistic/top-k/">principais</a> recuperados pelo algoritmo de pesquisa. Valores de recuperação mais elevados indicam uma melhor recuperação de itens relevantes.</li>
...
<p>Para testar a latência de carga, precisávamos de uma grande coleção de vectores, que <a href="https://docs.hippo.transwarp.io/docs/performance-dataset">o deep-image-96-angular</a> oferece. Para testar o desempenho da geração de índices e do cálculo de semelhanças, os vectores de elevada dimensão proporcionam mais stress. Para tal, escolhemos o conjunto de dados 500K de 1536 vectores de dimensão.</p>
<h3 id="Performance-Tools" class="common-anchor-header">Ferramentas de desempenho</h3><p>Abordámos as formas de sobrecarregar o sistema para identificar métricas de interesse, mas vamos examinar o que está a acontecer a um nível inferior: qual o nível de ocupação da unidade de computação, o consumo de memória, as esperas em bloqueios, etc.? Estes fornecem pistas sobre o comportamento da base de dados, particularmente úteis na identificação de áreas problemáticas.</p>
...
<h2 id="Redis-Vector-Search-Acceleration-through-Software-Prefetch" class="common-anchor-header">Aceleração da pesquisa vetorial do Redis por meio de Software Prefetch<button data-href="#Redis-Vector-Search-Acceleration-through-Software-Prefetch" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Para a pesquisa de similaridade de vectores, o Redis suporta dois algoritmos, nomeadamente força bruta e HNSW. O algoritmo HNSW foi especificamente criado para localizar eficientemente os vizinhos mais próximos aproximados em espaços de alta dimensão. Utiliza uma fila de prioridades denominada <em>candidate_set</em> para gerir todos os candidatos a vectores para cálculo da distância.</p>

...
<p>Um exemplo de filtragem colaborativa seria uma estação de rádio personalizada no Spotify que se baseia no histórico de audição, interesses e biblioteca de música de um utilizador, entre outros. A estação reproduz música que o utilizador não guardou nem manifestou interesse, mas que outros utilizadores com gostos semelhantes costumam ouvir. Um exemplo de filtragem baseada no conteúdo seria uma estação de rádio baseada numa canção ou artista específico que utiliza atributos da entrada para recomendar música semelhante.</p>

...
<h3 id="Different-types-of-vector-databases" class="common-anchor-header">Diferentes tipos de bases de dados vectoriais</h3><p>As bases de dados vectoriais surgiram para resolver as limitações das bibliotecas acima referidas, fornecendo uma solução mais abrangente e prática para aplicações de produção.</p>
...
<h3 id="Operability-and-Observability" class="common-anchor-header">Operacionalidade e observabilidade</h3><p>Para melhor servir os utilizadores empresariais, as bases de dados vectoriais devem oferecer uma gama de caraterísticas de nível empresarial para uma melhor operacionalidade e observabilidade. O Milvus suporta vários métodos de implementação, incluindo K8s Operator e Helm chart, docker-compose e pip install, tornando-o acessível a utilizadores com diferentes necessidades. O Milvus também fornece um sistema de monitorização e alarme baseado em Grafana, Prometheus e Loki, melhorando a sua observabilidade. Com uma arquitetura distribuída nativa da nuvem, o Milvus é o primeiro banco de dados vetorial do setor a oferecer suporte ao isolamento de vários locatários, RBAC, limitação de cota e atualizações contínuas. Todas essas abordagens tornam o gerenciamento e o monitoramento do Milvus muito mais simples.</p>

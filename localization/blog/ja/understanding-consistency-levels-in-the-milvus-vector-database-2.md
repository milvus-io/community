...
desc: Milvusベクトルデータベースにおける整合性レベルの調整可能なメカニズムの解明。
...
<p>Milvusでは、<code translate="no">ServiceTime</code> に加えて、様々なユーザーによる様々なレベルの一貫性と可用性のニーズを満たすために、タイムスタンプの一種であるギャランティタイムスタンプ(<code translate="no">GuaranteeTS</code>)を採用しています。つまり、Milvusベクトルデータベースのユーザは、検索やクエリを実行する際に、<code translate="no">GuaranteeTs</code> 以前の全てのデータが表示され、関与しているはずであることをクエリノードに通知するために、<code translate="no">GuaranteeTs</code> を指定することができる。</p>
...
<ul>
<li><p><code translate="no">CONSISTENCY_STRONG</code> <code translate="no">GuaranteeTs</code> は最新のシステム・タイムスタンプと同じ値に設定され、クエリ・ノードはサービス時間が最新のシステム・タイムスタンプに進むまで検索またはクエリ要求を処理するのを待ちます。</p></li>

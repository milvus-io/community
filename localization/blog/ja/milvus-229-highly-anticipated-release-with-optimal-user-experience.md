...
<h2 id="Optimized-user-experience-with-JSON-support" class="common-anchor-header">JSONサポートによるユーザーエクスペリエンスの最適化<button data-href="#Optimized-user-experience-with-JSON-support" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvusは待望のJSONデータ型のサポートを導入し、ユーザーのコレクション内のベクターのメタデータと共にJSONデータをシームレスに保存できるようになりました。この機能強化により、ユーザーはJSONデータを効率的に一括挿入し、JSONフィールドのコンテンツに基づいて高度なクエリやフィルタリングを実行することができます。さらに、データセットのJSONフィールドに合わせた式や操作の実行、クエリの構築、JSONフィールドの内容や構造に基づくフィルタの適用が可能になり、関連情報の抽出やデータ操作がより効率的に行えるようになります。</p>

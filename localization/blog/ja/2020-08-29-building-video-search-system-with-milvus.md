...
<p>動画を取り込む際には、OpenCVライブラリを用いて各動画をフレーム単位に切り出し、画像特徴抽出モデルVGGを用いてキーフレームのベクトルを抽出し、抽出したベクトル（埋め込み）をmilvusに挿入する。元の動画の保存にはMinioを、動画とベクトルの相関関係の保存にはRedisを使用しています。</p>
...
<h2 id="Data-preparation" class="common-anchor-header">データの準備<button data-href="#Data-preparation" class="anchor-icon" translate="no">
    ...
    </button></h2><p>この記事では、動画を検索するためのエンドツーエンドのソリューションを構築する際のサンプルデータセットとして、Tumblrの約10万GIFファイルを使用する。独自の動画リポジトリを使用することもできる。</p>
<h2 id="Deployment" class="common-anchor-header">配置<button data-href="#Deployment" class="anchor-icon" translate="no">
    ...
    </button></h2><p>この記事の動画検索システムを構築するコードはGitHubにある。</p>

...
<p>IVFの強みは2段階のフィルタリング処理にある。まずセントロイド（クラスター）レベルで粗視化フィルタを実行し、候補セットを素早く絞り込み、次に選択されたクラスター内で細かい距離計算を行うことができる。これにより、データの大部分がフィルタリングされた場合でも、安定した予測可能な性能を維持することができる。</p>

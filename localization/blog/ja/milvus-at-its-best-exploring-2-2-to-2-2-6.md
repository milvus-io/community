...
<p>Milvusフォロワーの皆さん、お帰りなさい！この最先端のオープンソース・ベクターデータベースの最新情報をお伝えしてから、しばらく時間が経ってしまいました。しかし、心配はご無用です。昨年8月以降に行われたエキサイティングな開発について、私たちがキャッチアップするためにここにいるからです。</p>
...
<li><p><strong>コーディネーターの高可用性(HA)：</strong>この機能により、Milvusのコーディネータはアクティブ・スタンバイ・モードで動作し、シングルポイント障害のリスクを低減します。予期せぬ災害時でも、復旧時間は最大30秒に短縮されます。</p></li>
</ul>
<p>これらの新機能に加え、Milvus v2.2.3では、バルクインサートのパフォーマンス向上、メモリ使用量の削減、モニタリングメトリクスの最適化、メタストレージのパフォーマンス向上など、多数の改善とバグフィックスが含まれています。詳細は<a href="https://milvus.io/docs/release_notes.md#v223">Milvus v2.2.3リリースノートを</a>ご参照ください。</p>
...
<li>RootCoordメッセージのバックログによるシステムの遅延</li>

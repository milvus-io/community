...
<p><code translate="no">.env</code> ファイルには、ベクターデータベースの選択、ベクターデータベースへのアクセスに必要な認証情報、Difyアプリのアドレスなど、Difyアプリを起動するために必要な設定が含まれています。</p>
...
<p>DifyでRAGアプリを作成するのはとても簡単です。先ほどの "Knowledge "ではなく、"Studio "に移動し、"Create from Blank "をクリックします。次に、アプリのタイプとして "チャットボット "を選択し、提供されるフィールド内にアプリの名前を付けます。完了したら、"Create" をクリックします。次のページが表示されます：</p>
<p>
  <span class="img-wrapper">
...
<p>Instruction"（指示）フィールドには、"Answer the query from the user concisely"（ユーザーからの問い合わせに簡潔に答える）のようなシステム・プロンプトを書くことができる。次に、"Context "として、"Add "マークをクリックし、先ほど作成したナレッジベースを追加します。こうすることで、RAGアプリはユーザーのクエリに答えるために、この知識ベースから可能性のあるコンテキストをフェッチします。</p>
...
<p>これで完了です！これで、LLMに「Attention is All You Need」論文やナレッジベースに含まれる文書に関連することを何でも質問できるようになりました。</p>
<p>
  <span class="img-wrapper">

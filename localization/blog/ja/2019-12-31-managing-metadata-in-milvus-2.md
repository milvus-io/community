...
<p>0.6.0では、<code translate="no">owner_table</code>，<code translate="no">partition_tag</code> ，<code translate="no">version</code> を含むいくつかの新しいフィールドで、テーブル・パーティショニングが有効になっています。ベクトル・テーブル、<code translate="no">table_1</code> は、<code translate="no">table_1_p1</code> というパーティションを持ちます。<code translate="no">partition_name</code> は、<code translate="no">table_id</code> に対応します。パーティション・テーブルのフィールドは<code translate="no">owner table</code> から継承され、オーナー・テーブル・フィールドがオーナー・テーブルの名前を指定し、<code translate="no">partition_tag</code> フィールドがパーティションのタグを指定する。</p>
<p>

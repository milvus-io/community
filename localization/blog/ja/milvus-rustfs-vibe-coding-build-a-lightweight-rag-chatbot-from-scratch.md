...
---
<p><em>このブログはMilvusコミュニティのJinghe Ma氏による</em> <em>寄稿で、許可を得て掲載しています。</em></p>
...
<p>バックエンドはFastAPI、チャット・インターフェースはNext.jsです。この投稿では、エンド・ツー・エンドのシステム全体をカバーします。コードはhttps://github.com/majinghe/chatbot。これは、プロダクション・レディのシステムではなく、ワーキング・プロトタイプですが、ゴールは、あなた自身の使用に適応させることができる、明確で拡張可能なビルドを提供することです。以下の各セクションでは、インフラストラクチャーからフロントエンドまで、1つのレイヤーについて説明します。</p>
...
<li><strong>起動時に</strong> <strong>yq****で</strong><strong>パッチを当てる</strong> <strong>。</strong>コンテナのコマンドを変更して、Milvusプロセスが開始する前に/milvus/config/milvus.yamlに対してyqを実行する。</li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Starting-Everything" class="common-anchor-header">全てを起動する</h3><p>設定が完了したら、4つのサービスをすべて立ち上げる：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>このチャンク戦略は意図的にシンプルにしています。ヘッダーで分割したり、コードブロックを保存したり、より良い検索のためにチャンクをオーバーラップさせるなど、より厳密な制御が必要な場合は、ここで繰り返し行います。</p>
...
<li><strong>ナレッジベースのベクトル化</strong>RustFSのドキュメント（80個のMarkdownファイル）がチャンクされ、text-embedding-3-largeで埋め込まれ、466個のベクターとしてMilvusに格納される。</li>
...
<li><strong><a href="https://rustfs.com/">RustFSは</a></strong> <strong>、オブジェクト・ストレージのバックエンドとして働くのが楽しい。</strong>MinIOのためにRustFSを導入するのは、予想以上に労力がかかりませんでした。</li>

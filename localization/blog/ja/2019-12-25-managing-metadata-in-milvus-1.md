...
<h2 id="SQLite" class="common-anchor-header">SQLite<button data-href="#SQLite" class="anchor-icon" translate="no">
    ...
    </button></h2><p>SQLiteを使用する場合、Milvus起動後、<code translate="no">meta.sqlite</code> ファイルがデータディレクトリ(設定ファイル<code translate="no">server_config.yaml</code> の<code translate="no">primary_path</code> で定義)に生成されます。このファイルを表示するには、SQLiteクライアントをインストールするだけです。</p>
...
<p>この記事が気に入った、または役に立ったと思ったら、拍手を忘れずに！</p>

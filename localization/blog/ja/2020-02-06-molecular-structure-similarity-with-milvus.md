...
<p>億単位の化合物ライブラリからフラグメント空間内で利用可能な化合物を発見するためには、通常、部分構造検索と類似性検索のために化学的フィンガープリントが検索される。しかし、10億スケールの高次元化学フィンガープリントを検索する場合、従来の方法では時間がかかり、エラーも発生しやすい。また、その過程で失われる可能性のある化合物もある。本稿では、Milvus（大規模ベクトル用類似性検索エンジン）をRDKitと組み合わせて使用し、高性能な化学構造類似性検索システムを構築する方法について述べる。</p>
...
<h2 id="System-demo" class="common-anchor-header">システムのデモ<button data-href="#System-demo" class="anchor-icon" translate="no">
    ...
    </button></h2><p>システムがどのように機能するかをよりよく示すために、Milvusを使用して9000万以上の化学フィンガープリントを検索するデモを作成しました。使用されたデータは ftp://ftp.ncbi.nlm.nih.gov/pubchem/Compound/CURRENT-Full/SDF からのものです。初期インターフェースは以下のようになる：</p>
<p>

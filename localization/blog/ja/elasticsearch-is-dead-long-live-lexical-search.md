...
<p><strong>非クラウドネイティブ・アーキテクチャー：</strong>クラウドネイティブアーキテクチャが普及する前に開発されたElasticsearchの設計は、ストレージとコンピートを強固に結合しており、パブリッククラウドやKubernetesのような最新のインフラとの統合を制限しています。リソースの拡張には、ストレージとコンピュート両方を同時に増やす必要があり、柔軟性が低下します。マルチレプリカ・シナリオでは、各シャードが独立してインデックスを構築する必要があり、計算コストが増加し、リソース効率が低下する。</p>
...
<p>世界最大のベクトル検索クラスターを維持し、何千ものAIアプリケーション開発者をサポートしてきた経験から、Zilliz Cloudはセルフホストソリューションに比べて運用のオーバーヘッドとコストを大幅に削減します。</p>

...
<p><br/></p>
<h3 id="Milvus-distributions" class="common-anchor-header">Milvusディストリビューション</h3><p>Milvusには、CPUのみのディストリビューションとGPU対応のディストリビューションがあります。前者はインデックス構築と検索をCPUのみに依存し、後者はMilvusをさらに高速化するCPUとGPUのハイブリッド検索とインデックス構築を可能にする。例えば、ハイブリッドディストリビューションを使用すると、CPUを検索に、GPUをインデックス構築に使用することができ、クエリの効率をさらに向上させることができる。</p>

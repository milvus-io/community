...
desc: >-
  Max-Minセマンティック・チャンキングが、よりスマートなチャンクを作成し、コンテキストの質を向上させ、より優れた検索性能を実現する埋め込み優先のアプローチにより、RAGの精度をどのように向上させるかをご覧ください。
origin: >-
...
<p><a href="https://link.springer.com/article/10.1007/s10791-025-09638-7"><strong>Max-Minセマンティック・チャンキングは</strong></a>この問題に異なるアプローチをとる。最初にチャンキングする代わりに、テキストを前もって埋め込み、意味的類似性を使って境界を形成する場所を決定する。カットする前に埋め込むことで、パイプラインは任意の長さ制限に頼るのではなく、意味の自然なシフトを追跡できる。</p>
<p>前回のブログでは、Jina AIの<a href="https://milvus.io/blog/smarter-retrieval-for-rag-late-chunking-with-jina-embeddings-v2-and-milvus.md"><strong>Late Chunkingの</strong></a>ような方法について説明した。Jina AIは、「埋め込み優先」のアイデアを普及させ、それが実際に機能することを示した。<strong>Max-Min Semantic Chunkingは</strong>、新しいチャンクを正当化するのに十分なほど意味が変化するタイミングを特定するシンプルなルールで、同じコンセプトに基づいています。この投稿では、Max-Minがどのように機能するかを説明し、実際のRAGワークロードにおける長所と限界を検証する。</p>
<h2 id="How-a-Typical-RAG-Pipeline-Works" class="common-anchor-header">典型的なRAGパイプラインの仕組み<button data-href="#How-a-Typical-RAG-Pipeline-Works" class="anchor-icon" translate="no">
    ...
    </button></h2><p>フレームワークに関係なく、ほとんどのRAGパイプラインは同じ4段階の組み立てラインに従う。おそらく、あなた自身もこれのいくつかのバージョンを書いたことがあるだろう：</p>
...
<h2 id="Max–Min-Semantic-Chunking-Embed-First-Chunk-Second" class="common-anchor-header">最大-最小のセマンティック・チャンキング：まず埋め込み、次にチャンク<button data-href="#Max–Min-Semantic-Chunking-Embed-First-Chunk-Second" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h2 id="Conclusion" class="common-anchor-header">結論<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Max-Min Semantic ChunkingはすべてのRAG問題を解決する魔法のような方法ではないが、チャンクの境界をよりまともに考える方法を与えてくれる。トークンの制限でアイデアの切り分けを決めるのではなく、埋め込みを使って実際に意味が移動する場所を検出するのだ。API、仕様書、ログ、リリースノート、トラブルシューティングガイドなど、実世界の多くの文書では、これだけで検索品質を著しく向上させることができる。</p>
<p>このアプローチで私が気に入っているのは、既存のRAGパイプラインに自然にフィットすることだ。すでに文章や段落を埋め込んでいる場合、追加コストは基本的にいくつかの余弦類似度チェックだけだ。余分なモデルや複雑なクラスタリング、ヘビー級の前処理は必要ない。そして、この方法がうまく機能すれば、生成されるチャンクはより「人間的」なものに感じられる。</p>
<p>しかし、この方法にはまだ盲点がある。局所的にしか意味を見出せず、意図的にばらばらになった情報をつなぎ直すことができないのだ。オーバーラッピングウィンドウ、マルチパススキャン、その他の文脈を保持するトリックは、特に参考文献と説明が離れた場所にある文書にはまだ必要である。</p>

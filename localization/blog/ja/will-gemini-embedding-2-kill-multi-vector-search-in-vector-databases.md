...
<li>リクエストあたり6枚までの<strong>画像</strong>（PNG、JPEG）</li>
...
<p>Milvusはすべてのベクトルフィールドを個別に検索し、リランカーを使って候補セットをマージする。APIでは、各リクエストは異なるフィールドと検索設定に対応し、hybrid_search()は結合された結果を返す。</p>

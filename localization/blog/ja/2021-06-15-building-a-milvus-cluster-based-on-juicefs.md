...
<h3 id="Step-3-Start-Milvus" class="common-anchor-header"><strong>ステップ3: Milvusの起動</strong></h3><p>クラスタ内のすべてのノードにMilvusをインストールし、各Milvusノードに読み取り権限または書き込み権限を設定する必要があります。Milvusノードは1台のみ書き込みノードとして設定でき、残りは読み込みノードとして設定する必要があります。まず、Milvusシステム設定ファイル<strong>server_config.yaml</strong> のセクション<code translate="no">cluster</code> と<code translate="no">general</code> のパラメータを設定します：</p>

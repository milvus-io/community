...
title: 'LangExtract + Milvus: ハイブリッド文書処理・検索システム構築実践ガイド'
author: 'Cheney Zhang, Lumina Wang'
date: 2025-08-28T00:00:00.000Z
desc: >-
  LangExtractとMilvusを組み合わせてハイブリッドコード検索を実現する方法を学びましょう-1つのインテリジェントパイプラインでセマンティック検索と正確なフィルタリングを実現します。
...
---
<p><a href="https://milvus.io/blog/why-im-against-claude-codes-grep-only-retrieval-it-just-burns-too-many-tokens.md">以前のブログで</a>、多くのコーディングエージェントにおけるコード検索への2つの一般的なアプローチを比較した：</p>
<ul>
...
<li><p>キーワードだけに頼れば、コード（またはテキスト）が実際に何を意味するのか、意味論的な理解を失うことになります。</p></li>
</ul>
<p>このチュートリアルでは、<strong>両方のアプローチをスマートに組み合わせる</strong>方法を紹介します。<a href="https://github.com/google/langextract">LangExtract（</a>LLMを使用して、乱雑なテキストを正確なソース帰属を持つ構造化データに変換するPythonライブラリ）を、オープンソースの高性能ベクトルデータベースである<a href="https://milvus.io/">Milvusと共に</a>使用して、よりインテリジェントで高品質な文書処理・検索システムを構築する方法を紹介します。</p>
<h3 id="Key-Technologies-We’ll-Use" class="common-anchor-header">使用する主要技術</h3><p>この文書処理・検索システムの構築を始める前に、このチュートリアルで使用する主要技術を見てみましょう。</p>
<h3 id="What-is-LangExtract" class="common-anchor-header">LangExtractとは？</h3><p><a href="https://github.com/langextract/langextract">LangExtractは</a>、Googleによってオープンソース化された新しいPythonライブラリで、LLMを利用して、厄介な非構造化テキストをソースの帰属を伴う構造化データに変換します。情報抽出のようなタスクを非常にシンプルにするため、すでに人気があります（13K+ GitHub stars）。</p>
<p>
...
主な機能は以下の通り：</p>
<ul>
<li><p>構造化抽出：スキーマを定義し、名前、日付、場所、料金、その他の関連情報を抽出します。</p></li>
<li><p>ソースのトレーサビリティ：抽出されたすべてのフィールドは元のテキストにリンクされ、幻覚の可能性を低減します。</p></li>
<li><p>長い文書にも対応：チャンキング＋マルチスレッドで数百万文字を処理します。</p></li>
</ul>
<p>
  <span class="img-wrapper">
...
<p>LangExtractは、法律、医療、法医学など、精度が重要な分野で特に役立ちます。例えば、RAGで巨大なテキストブロックを取得する代わりに、LangExtractは、意味的な文脈を保持したまま、気になる日付、句、患者の属性だけを抽出することができます。</p>
<h3 id="What’s-Milvus" class="common-anchor-header">Milvusとは？</h3><p><a href="https://milvus.io/">Milvusは</a>オープンソースのベクトルデータベースで、Github上で36K以上のスターを持ち、様々な業界で10K以上の企業で採用されています。Milvusは、RAGシステム、AIエージェント、レコメンデーションエンジン、異常検知、セマンティック検索などで広く使用されており、AIを活用したアプリケーションの中核となるビルディングブロックとなっています。</p>
<h2 id="Building-a-High-Quality-Document-Processing-System-with-LangExtract-+-Milvus" class="common-anchor-header">LangExtract + Milvusによる高品質文書処理システムの構築<button data-href="#Building-a-High-Quality-Document-Processing-System-with-LangExtract-+-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>このガイドでは、<a href="https://github.com/google/langextract">LangExtractと</a><a href="https://milvus.io/"> Milvusを組み合わせて</a>インテリジェントな文書処理・検索システムを構築するプロセスを説明します。</p>
<ul>
<li><p>LangExtractはクリーンで構造化されたメタデータを生成し、それをMilvusで効率的に保存・検索することで、正確なフィルタリングとセマンティックな検索という両方の長所を発揮します。</p></li>
<li><p>Milvusは検索バックボーンとして機能し、LangExtractによって抽出された埋め込み（セマンティック検索用）と構造化メタデータの両方を保存し、正確でインテリジェントなハイブリッドクエリを大規模に実行できるようにします。</p></li>
</ul>
<h3 id="Prerequisites" class="common-anchor-header">前提条件</h3><p>この作業に入る前に、以下の依存関係がインストールされていることを確認してください：</p>
<pre><code translate="no">! pip install --upgrade pymilvus langextract google-genai requests tqdm pandas
<button class="copy-code-btn"></button></code></pre>
<p>この例では、LLMとしてGeminiを使用します。<a href="https://aistudio.google.com/app/apikey"> APIキーを</a>環境変数として設定する必要があります：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Setting-Up-the-LangExtract-+-Milvus-Pipeline" class="common-anchor-header"><strong>LangExtract + Milvusパイプラインのセットアップ</strong></h3><p>構造化情報抽出にLangExtractを、ベクトルストアとしてMilvusを使用するパイプラインを定義することから始めましょう。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Configuration-and-Setup" class="common-anchor-header"><strong>設定とセットアップ</strong></h3><p>次に、統合のためのグローバルパラメータを設定します。Geminiの埋め込みモデルを使用して、ドキュメントのベクトル表現を生成します。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Initializing-the-Milvus-Client" class="common-anchor-header"><strong>Milvusクライアントの初期化</strong></h3><p>Milvusクライアントを初期化しましょう。簡単のため、ローカルのデータベースファイルを使用しますが、この方法はMilvusサーバーのフルデプロイメントにも簡単に拡張できます。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong> <code translate="no">MilvusClient</code> パラメータについて：</strong></p>
<p><code translate="no">uri</code> をローカルファイル（<code translate="no">./milvus.db</code> のような）として設定する方法は、自動的に<a href="https://milvus.io/docs/milvus_lite.md"> Milvus Liteを</a>使用してすべてのデータをこのファイルに格納するため、最も便利な方法です。</p>
<p>大規模なデータの場合は、<a href="https://milvus.io/docs/quickstart.md"> DockerやKubernetes</a>上でよりパフォーマンスの高いMilvusサーバをセットアップすることができます。このセットアップでは、代わりにサーバuri（[<code translate="no">http://localhost:19530](http://localhost:19530)</code> のようなもの）を使用します。</p>
<p><a href="https://zilliz.com/cloud"> Zilliz Cloud</a>(Milvusのフルマネージドクラウドサービス)を利用する場合は、<code translate="no">uri</code> と<code translate="no">token</code> をZilliz Cloudの<a href="https://docs.zilliz.com/docs/on-zilliz-cloud-console#free-cluster-details"> Public EndpointとAPIキーに合わせて</a>ください。</p>
<h3 id="Preparing-Sample-Data" class="common-anchor-header"><strong>サンプルデータの準備</strong></h3><p>このデモでは、サンプルドキュメントとして映画の説明を使用します。これは、LangExtractが構造化されていないテキストからジャンル、キャラクター、テーマなどの構造化された情報を抽出する方法を紹介します。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Setting-Up-the-Milvus-Collection" class="common-anchor-header"><strong>Milvusコレクションのセットアップ</strong></h3><p>抽出したデータを保存する前に、適切なスキーマを持つMilvusコレクションを作成する必要があります。このコレクションには、元のドキュメントテキスト、ベクトル埋め込み、抽出されたメタデータフィールドが格納される。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Defining-the-Extraction-Schema" class="common-anchor-header"><strong>抽出スキーマの定義</strong></h3><p>LangExtractはプロンプトと例を使用して、LLMが構造化情報を抽出するのをガイドします。映画の説明のための抽出スキーマを定義して、抽出する情報とそれを分類する方法を正確に指定しましょう。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Providing-Examples-to-Improve-Extraction-Quality" class="common-anchor-header"><strong>抽出品質を向上させるための例の提供</strong></h3><p>抽出品質と一貫性を向上させるために、LangExtractに注意深く作られた例を提供します。こ れ ら の例は、 予期 さ れ る フ ォーマ ッ ト を示し、 私達の特定の抽出要件をモデルが理解す る のを助け ます。</p>
...
<h3 id="Processing-and-Vectorizing-Results" class="common-anchor-header"><strong>結果の処理とベクトル化</strong></h3><p>次に、抽出結果を処理し、各文書のベクトル埋め込みを生成する必要があります。また、Milvusで簡単に検索できるように、抽出された属性を別々のフィールドに平坦化します。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Inserting-Data-into-Milvus" class="common-anchor-header"><strong>Milvusへのデータ挿入</strong></h3><p>処理したデータの準備ができたので、Milvusコレクションに挿入してみましょう。これにより、セマンティック検索と正確なメタデータフィルタリングの両方が可能になる。</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Demonstrating-Metadata-Filtering" class="common-anchor-header"><strong>メタデータフィルタリングのデモ</strong></h3><p>LangExtractとMilvusを組み合わせることの重要な利点の一つは、抽出されたメタデータに基づいて正確なフィルタリングを実行できることです。フィルター式検索で実際に見てみましょう。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>完璧です！検索結果は "スリラー "と "軍隊文字 "のフィルター条件に正確にマッチしています。</p>
<h3 id="Combining-Semantic-Search-with-Metadata-Filtering" class="common-anchor-header"><strong>セマンティック検索とメタデータフィルタリングの組み合わせ</strong></h3><p>この統合の本当の力が発揮されるのはここからです。セマンティックなベクトル検索と正確なメタデータフィルタリングを組み合わせるのです。これにより、抽出された属性に基づいて特定の制約を適用しながら、意味的に類似したコンテンツを見つけることができます。</p>
...
=== Demo Complete ===
<button class="copy-code-btn"></button></code></pre>
<p>ご覧の通り、Milvusを使ったセマンティック検索結果は、ジャンルフィルター条件を満たし、クエリテキストコンテンツとの高い関連性を示しています。</p>
<h2 id="What-Youve-Built-and-What-It-Means" class="common-anchor-header">構築したものとその意味<button data-href="#What-Youve-Built-and-What-It-Means" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>構造化抽出とセマンティック検索を組み合わせたハイブリッド文書処理システムが完成しました。このアプローチは、信頼性を確保しながら非構造化データの価値を最大化するため、金融、医療、法律などの領域における重要なシナリオに最適です。</p>
<p>構造化画像分析とセマンティック検索を組み合わせることで、より優れた電子商取引のレコメンデーションを実現したり、動画コンテンツに適用することで、自律走行データマイニングを強化したりすることができます。</p>
<p>巨大なマルチモーダルデータセットを管理する大規模な展開のために、私たちの今後の<strong>ベクトルデータレイクは</strong>、より費用対効果の高いコールドストレージ、幅広いテーブルのサポート、合理化されたETL処理を提供します。ご期待ください。</p>
<p>ご質問や結果を共有したいですか？<a href="https://github.com/zilliztech/VectorDBBench"> GitHubで</a>会話に参加するか、<a href="https://discord.com/invite/FG6hMJStWu">Discordで</a>私たちのコミュニティと接続してください。</p>

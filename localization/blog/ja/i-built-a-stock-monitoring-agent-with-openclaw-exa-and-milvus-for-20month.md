...
<p>そこで私は、私の代わりに市場を監視し、同じ間違いを犯さないようにするエージェントをオープンクローで構築した。私の資金を売買したり触ったりするのではなく、一度後悔したことをしないようにしてくれるのだ。</p>
<p>このエージェントは3つの部分から成り、月々約20ドルかかる：</p>
<ul>
<li><strong>OpenClawはすべてを自動操縦で実行する。</strong>OpenClawは30分のハートビートでエージェントを作動させ、実際に重要なことがあったときだけピンポンを鳴らしてくれる。以前は、価格を見れば見るほど、衝動的に反応していた。</li>
...
<h2 id="Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="common-anchor-header">ステップ1：Exaでリアルタイムのマーケット・インテリジェンスを得る<button data-href="#Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="anchor-icon" translate="no">
    ...
    </button></h2><p>以前は、金融アプリを見たり、スクレイパーを書いたり、プロのデータ端末を調べたりしていた。アプリはシグナルをノイズに埋もれさせ、スクレイパーは常に壊れ、専門的なAPIは機関トレーダー向けの価格だった。ExaはAIエージェントのために作られた検索APIで、上記の問題を解決する。</p>
<p>
...
<p><strong><a href="https://exa.ai/">Exaは</a></strong>、構造化されたAI対応データを返すウェブ検索APIだ。これは<a href="https://zilliz.com/cloud">Zilliz Cloud</a>（Milvusのフルマネージドサービス）によって提供されている。  Perplexityが人間が使う検索エンジンなら、ExaはAIが使う。エージェントがクエリを送信すると、Exaは記事テキスト、キーセンテンス、要約をJSONとして返す。</p>
...
<h2 id="Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="common-anchor-header">ステップ2：取引履歴をmilvusに保存し、より賢い決断を下す<button data-href="#Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>エクサは私の情報の問題を解決してくれた。しかし、私はまだ同じ取引を繰り返していた。数日で回復する下落局面でパニック売りをしたり、すでに割高だった銘柄に勢いを追ったり。私は感情で行動し、後悔し、同じような状況が訪れる頃には教訓を忘れていた。</p>
<p>私には個人的な知識ベースが必要だった。過去の取引、理由、失敗談。手作業で見直すのではなく（試したことはあるが、続けたことはない）、同じような状況が訪れるたびにエージェントが勝手に検索してくれるようなものだ。もし私がミスを繰り返しそうになったら、ボタンを押す前にエージェントに教えてもらいたい。現在の状況 "と "過去の経験 "のマッチングは、ベクトルデータベースが解決する類似検索の問題であるため、私のデータはすべて1つに格納する必要があった。</p>
<p>私は<a href="https://github.com/milvus-io/milvus-lite">Milvus Liteを</a>使った。Milvusの軽量版で、サーバーのセットアップなしでローカルで動作する。プロトタイピングや実験に最適です。私はデータを3つのコレクションに分割した。埋め込み次元は、OpenAIのtext-embedding-3-smallモデルに合わせて1536とした：</p>
...
<tr><td><strong>外部知識</strong></td><td>調査レポート、SEC提出書類、公開データ</td><td>Milvusには保存されず、Exaを通じて検索可能</td></tr>
</tbody>
</table>
<p>これらを1つのコレクションに混ぜることは、無関係な取引履歴ですべてのプロンプトを肥大化させるか、現在のクエリと十分に一致しない場合にコアバイアスを失うことを意味する。</p>
<p>コレクションが存在すれば、エージェントは自動的にそれらを入力する必要がある。私はすべての会話の後にメモを書きたくないので、各チャットセッションの最後に実行されるメモリエクストラクタを作りました。</p>
<p>これは2つのことをする：抽出と重複排除だ。エクストラクタはLLMに、会話から構造化された洞察（決定、好み、パターン、教訓）を引き出すよう依頼し、それぞれを適切なコレクションにルーティングする。何かを保存する前に、すでにあるものとの類似性をチェックする。新しい洞察が既存のエントリーと92％以上類似している場合、それはスキップされる。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>例えば、3月上旬に中東情勢の緊迫化でハイテク株が3～4％下落したとき、エージェントは3つの情報を引き出してくれた。2024年10月の教訓として、地政学的な下落局面ではパニック売りをしないこと、私が地政学的リスクをオーバーウエートする傾向があるという嗜好メモ、そして、地政学に起因するハイテク株の下落は通常1～3週間で回復するという私が記録したパターンだ。</p>
<p>同僚はこれを「個人投資家のための強化学習」と呼んだ。なるほど。少なくとも今、私がしたことと私がしようとしていることの間にフィードバック・ループがある。</p>
<h2 id="Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="common-anchor-header">ステップ 3: エージェントに OpenClaw スキルで分析することを教える<button data-href="#Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>この時点でエージェントは良い情報<a href="https://exa.ai/">（Exa</a>）と個人的な記憶<a href="https://github.com/milvus-io/milvus-lite">（milvus</a>）を持っている。しかし、その両方をLLMに渡して「これを分析しろ」と言うと、一般的な、ヘッジの効いた回答が返ってくる。ありとあらゆる角度から言及し、「投資家はリスクを天秤にかけるべきだ」と結論づける。</p>
<p>解決策は、独自の分析フレームワークを作成し、それをエージェントに明確な指示として与えることだ。どの指標を重視するか、どの状況を危険と考えるか、いつ保守的になるか、いつ積極的になるか。これらのルールは投資家ごとに異なるので、自分で定義する必要があります。</p>
<p>OpenClaw は、スキル（skills/ ディレクトリ内のマークダウン・ファイル）を使ってこれを行います。エージェントが関連する状況に遭遇すると、マッチするスキルがロードされ、自由奔放に動くのではなく、あなたのフレームワークに従います。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>最後の一行が最も重要だ。私は恐怖心がデータに優先する傾向がある。Milvusの履歴に、急落後に売ったことを後悔していることが記されていたら、はっきりとそう言いましょう。"これは私の心理に特化した修正メカニズムだ。あなたのバージョンは、あなた自身の傾向をエンコードするでしょう。</p>
...
<h2 id="Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="common-anchor-header">ステップ 4: OpenClaw Heartbeat でエージェントを起動する<button data-href="#Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="anchor-icon" translate="no">
    ...
    </button></h2><p>上記の全てはまだ手動でトリガーする必要があります。更新が必要なたびにターミナルを開かなければならないのであれば、振り出しに戻ってしまいます。おそらくまた会議中にブローカーアプリをドゥームスクロールすることになるでしょう。</p>
<p>OpenClawのHeartbeatメカニズムがこれを解決します。ゲートウェイはエージェントに30分ごとにpingを送り（設定可能）、エージェントはHEARTBEAT.mdファイルをチェックして、その瞬間に何をすべきかを決定します。HEARTBEAT.mdは、時間ベースのルールが記述されたマークダウンファイルです：</p>
...
<p>総費用：<span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mn>OpenClawが</mn><mi>10</mi><mn>ドル/月</mn><mo separator="true">、</mo></mrow><annotation encoding="application/x-tex">OpenClawが10ドル/月、</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1em;vertical-align:-0.25em;"></span><span class="mord"></span><span class="mord mathnormal" style="margin-right:0.02691em;">OpenClawが</span><span class="mord">10ドル/月</span><span class="mpunct">、</span></span></span></span>Exaが10ドル/月、milvus Liteを稼働させるための電気代が少し。</p>
...
<ul>
<li><strong><a href="https://exa.ai/">Exaを使って</a></strong><strong>信頼できるマーケット・ニュースを自動的に収集する</strong>。</li>
<li><strong> <a href="https://milvus.io/">Milvusを</a></strong>個人的なベクトル・データベースとして使い、<strong>私の過去のトレードを記憶し、</strong>間違いを繰り返しそうになると<strong>警告して</strong>くれる。</li>
<li><strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">OpenClaw</a></strong>SkillsとHeartbeatを使用し、<strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">OpenClawの</a></strong> <strong>自動操縦で</strong><strong>実行さ</strong> <strong>れ、重要な時だけ私にpingを送る</strong>。</li>
</ul>
<p>総費用は$20/月。エージェントは私の資金を取引したり触ったりしません。</p>
<p>最大の変化は、データでもアラートでもない。マーケットを見なくなったことだ。先週の水曜日、私はこのことをすっかり忘れていた。今でも時々損をすることはあるが、その頻度はずっと減り、週末を再び楽しむことができるようになった。同僚はまだジョークを更新していないけど、時間をおいてね。</p>

...
<h2 id="Document-ETL-Structuring-Unstructured-Text" class="common-anchor-header">ドキュメントETL: 構造化されていないテキストの構造化<button data-href="#Document-ETL-Structuring-Unstructured-Text" class="anchor-icon" translate="no">
    ...
    </button></h2><p>このセクションでは、ベクトル埋め込み、Milvusインデックス作成、Spring AI のRAGパイプラインを使用して、非構造化ファイルを検索可能なインテリジェントなレスポンスに変換します。</p>
...
<li><p>OpenAIと互換性のある埋め込みモデルを使用して埋め込みを生成する。</p></li>
...
<button class="copy-code-btn"></button></code></pre>
<p>よりスムーズなフロントエンドエクスペリエンスのために、リアクティブストリームAPIを使用して、サーバー送信イベント（SSE）を介して<code translate="no">Flux</code> ：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>💡<strong>テクニカルノート:</strong>Spring Boot 3.2はOTELスターターを導入しているが、gRPC(milvusが使用)をカバーしていない。エンドツーエンドの可視性を確保するために、このプロジェクトではJavaAgentのアプローチを使用しています。</p>

...
<p><strong>Function</strong>モジュールは、エンベッディング生成をネイティブなデータベース機能に変えます。Milvusは、エンベッディングパイプライン、バックグラウンドワーカー、リランカーサービスを個別に実行する代わりに、設定されたプロバイダーにリクエストを送り、エンベッディングを取得し、データと一緒に保存します。これにより、エンベッディングインフラストラクチャを管理するための運用上のオーバーヘッドがなくなります。</p>

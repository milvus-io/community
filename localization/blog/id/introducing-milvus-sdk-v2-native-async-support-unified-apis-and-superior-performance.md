...
---
<h2 id="TLDR" class="common-anchor-header">LAI TB, DR<button data-href="#TLDR" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><strong>"API RESTful kehilangan fitur-fitur penting."</strong> Fungsi penting seperti manajemen partisi dan konstruksi indeks tidak tersedia, sehingga memaksa para pengembang untuk beralih ke SDK yang berbeda.</p>
...
<p>Lebih baik lagi, <code translate="no">create_collection()</code> mendukung pembuatan indeks sebagai bagian dari pemanggilan yang sama. Jika parameter indeks disediakan, Milvus akan secara otomatis membangun indeks dan memuat data ke dalam memori - tidak perlu pemanggilan <code translate="no">create_index()</code> atau <code translate="no">load()</code> secara terpisah. Satu metode melakukan semuanya: <em>buat koleksi → bangun indeks → muat koleksi.</em></p>
...
<p>Parameter <code translate="no">index_params</code> dari metode <code translate="no">create_collection</code> menghilangkan kebutuhan untuk pemanggilan terpisah untuk <code translate="no">create_index</code> dan <code translate="no">load_collection</code>-semuanya terjadi secara otomatis.</p>

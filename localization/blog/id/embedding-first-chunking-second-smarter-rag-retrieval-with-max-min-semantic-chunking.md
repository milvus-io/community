...
title: >-
  Menanamkan Pertama, Memotong Kedua: Pengambilan RAG yang Lebih Cerdas dengan
  Pemenggalan Semantik Max-Min
author: Rachel Liu
...
<p>Masalahnya adalah bahwa strategi chunking tradisional biasanya membagi teks tanpa pemahaman semantik. Chunking dengan panjang tetap memotong berdasarkan jumlah token, dan chunking rekursif menggunakan struktur tingkat permukaan, tetapi keduanya masih mengabaikan makna teks yang sebenarnya. Akibatnya, ide-ide yang terkait sering kali dipisahkan, baris yang tidak terkait dikelompokkan bersama, dan konteks yang penting terfragmentasi.</p>
<p><a href="https://link.springer.com/article/10.1007/s10791-025-09638-7"><strong>Max-Min Semantic Chunking</strong></a> mendekati masalah ini secara berbeda. Alih-alih memotong terlebih dahulu, ia menyematkan teks di awal dan menggunakan kesamaan semantik untuk memutuskan di mana batas-batas harus dibentuk. Dengan menyematkan sebelum memotong, pipeline dapat melacak pergeseran makna secara alami daripada mengandalkan batas panjang yang sewenang-wenang.</p>
<p>Di blog kami sebelumnya, kami membahas metode seperti <a href="https://milvus.io/blog/smarter-retrieval-for-rag-late-chunking-with-jina-embeddings-v2-and-milvus.md"><strong>Late Chunking</strong></a> dari Jina AI, yang membantu mempopulerkan ide "embed-first" dan menunjukkan bahwa metode ini dapat bekerja dalam praktiknya. <strong>Max-Min Semantic Chunking</strong> dibangun di atas konsep yang sama dengan aturan sederhana yang mengidentifikasi kapan makna berubah cukup untuk menjamin adanya potongan baru. Dalam artikel ini, kita akan membahas cara kerja Max-Min dan memeriksa kekuatan dan keterbatasannya untuk beban kerja RAG yang sebenarnya.</p>
<h2 id="How-a-Typical-RAG-Pipeline-Works" class="common-anchor-header">Cara Kerja Pipa RAG pada Umumnya<button data-href="#How-a-Typical-RAG-Pipeline-Works" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Sebagian besar pipeline RAG, apa pun kerangka kerjanya, mengikuti jalur perakitan empat tahap yang sama. Anda mungkin telah menulis beberapa versi dari ini sendiri:</p>
...
<h2 id="Max–Min-Semantic-Chunking-Embed-First-Chunk-Second" class="common-anchor-header">Pemenggalan Semantik Maks-Min: Sematkan Pertama, Pisahkan Kedua<button data-href="#Max–Min-Semantic-Chunking-Embed-First-Chunk-Second" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ul>
<li><p>Jika <strong>kemiripan maksimal kalimat baru</strong> dengan potongan <strong>C</strong> <strong>lebih besar atau sama dengan</strong> <strong>kemiripan minimal di dalam C</strong>, → Kalimat baru cocok dan tetap berada di dalam potongan tersebut.</p></li>
...
<p>Ada pertanyaan atau ingin menggali lebih dalam untuk meningkatkan kinerja RAG? Bergabunglah dengan <a href="https://discord.com/invite/8uyFbECzPX">Discord</a> kami dan terhubung dengan para insinyur yang membangun dan menyetel sistem pengambilan yang sebenarnya setiap hari.</p>

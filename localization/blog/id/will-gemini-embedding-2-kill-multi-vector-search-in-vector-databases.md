...
title: >-
  Akankah Penyematan Gemini 2 Membunuh Pencarian Multi-Vektor di Basis Data
  Vektor?
author: Jack Li
date: 2026-3-13
...
desc: >-
  Google Gemini menyematkan 2 teks peta, gambar, video, dan audio ke dalam satu
  vektor. Apakah itu membuat pencarian multi-vektor menjadi usang? Kami
  menguraikan di mana setiap pendekatan cocok.
origin: >-
...
<p>Anda menyematkan klip video, foto produk, dan paragraf teks dengan satu panggilan API, dan semuanya berada di lingkungan semantik yang sama.</p>
<p>Sebelum model seperti ini, Anda tidak punya pilihan selain menjalankan setiap modalitas melalui model spesialisnya sendiri dan menyimpan setiap output dalam kolom vektor terpisah. Kolom multi-vektor dalam basis data vektor seperti <a href="https://milvus.io/docs/multi-vector-search.md">Milvus</a> dibangun dengan tepat untuk dunia tersebut.</p>
<p>Jadi pertanyaannya sekarang adalah: berapa banyak dari kerumitan tersebut yang dapat digantikan oleh Gemini Embedding 2 - dan di mana kekurangannya? Artikel ini akan menjelaskan di mana setiap pendekatan cocok dan bagaimana mereka bekerja bersama.</p>
<h2 id="What’s-Different-About-Gemini-Embedding-2-When-Compared-to-CLIPCLAP" class="common-anchor-header">Apa yang Berbeda Tentang Gemini Embedding 2 Jika Dibandingkan dengan CLIP/CLAP<button data-href="#What’s-Different-About-Gemini-Embedding-2-When-Compared-to-CLIPCLAP" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><strong>Sekarang</strong>, Anda memasukkan frame, audio, dan subtitle video ke dalam satu panggilan API dan mendapatkan satu vektor terpadu yang menangkap gambaran semantik secara penuh.</p>
...
<p>Milvus mencari setiap bidang vektor secara independen, lalu menggabungkan set kandidat menggunakan perangking ulang. Dalam API, setiap permintaan dipetakan ke bidang dan konfigurasi pencarian yang berbeda, dan hybrid_search() mengembalikan hasil gabungan.</p>
...
<p><a href="https://milvus.io/">Milvus</a> menjalankan kedua pola tersebut sebagai <a href="https://milvus.io/docs/multi-vector-search.md">pencarian ANN</a> paralel dengan pemeringkatan ulang melalui RRFRanker. Definisi skema, konfigurasi multi-indeks, BM25 bawaan - semua ditangani dalam satu sistem.</p>
<p>Untuk memperjelas hal ini, pertimbangkan katalog produk di mana setiap item menyertakan deskripsi teks dan gambar. Anda dapat menjalankan tiga pencarian terhadap data tersebut secara paralel:</p>
<ul>
...
<h2 id="Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="common-anchor-header">Sekarang, dengan Gemini Embedding 2, Apakah Pencarian Multi-Vektor Masih Penting?<button data-href="#Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Gemini Embedding 2 menangani lebih banyak modalitas dalam satu panggilan, yang sangat menyederhanakan pipeline. Namun, penyematan multimodal terpadu bukanlah hal yang sama dengan pencarian multi-vektor. Dengan kata lain, pencarian multi-vektor tidak akan menjadi usang.</p>
<p>Gemini Embedding 2 memetakan teks, gambar, video, audio, dan dokumen ke dalam satu ruang vektor bersama. Google <a href="https://developers.googleblog.com/en/gemini-embedding-model-now-available/">memposisikannya</a> untuk pencarian semantik multimodal, pencarian dokumen, dan rekomendasi - skenario di mana semua modalitas menggambarkan konten yang sama dan tumpang tindih antar modalitas yang tinggi membuat vektor tunggal menjadi layak.</p>
...
<p><strong>Alat bantu agen.</strong> Asisten pengkodean seperti OpenClaw menyimpan vektor semantik yang padat untuk riwayat percakapan ("masalah penyebaran itu dari minggu lalu") di samping vektor BM25 yang jarang untuk pencocokan yang tepat pada nama file, perintah CLI, dan parameter konfigurasi. Tujuan pengambilan yang berbeda, jenis vektor yang berbeda, jalur pencarian yang independen, kemudian diurutkan ulang.</p>
...
<p>Kedua pendekatan ini tidak saling eksklusif. Anda dapat menggunakan Gemini Embedding 2 untuk kolom multimodal terpadu dan masih menyimpan vektor jarang atau atribut tambahan dalam kolom terpisah dalam koleksi <a href="https://milvus.io/"></a><a href="https://milvus.io/">Milvus</a> yang sama.</p>
<h2 id="Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="common-anchor-header">Mulai Cepat: Menyiapkan Gemini Embedding 2 + Milvus<button data-href="#Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Berikut ini adalah sebuah demo kerja. Anda memerlukan<a href="https://milvus.io/docs/install-overview.md">instance</a> <a href="https://milvus.io/docs/install-overview.md"></a><a href="https://milvus.io/docs/install-overview.md">Milvus atau Zilliz Cloud</a> yang sedang berjalan dan GOOGLE_API_KEY.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Untuk penyematan gambar dan audio, gunakan embed_image() dan embed_audio() dengan cara yang sama - vektor mendarat di koleksi yang sama dan ruang vektor yang sama, sehingga memungkinkan pencarian lintas-modal yang sebenarnya.</p>

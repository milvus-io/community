...
desc: >-
  Découvrez les nouveautés de Milvus 2.6, y compris les changements
  d'architecture et les fonctionnalités clés, et apprenez comment effectuer une
  mise à niveau à partir de Milvus 2.5.
origin: >-
...
<li><p><strong>Quantification</strong><a href="https://milvus.io/docs/ivf-rabitq.md#RaBitQ"><strong>RaBitQ</strong></a> <strong>pour les index primaires</strong> - Une nouvelle méthode de quantification à 1 bit qui compresse les index vectoriels à <strong>1/32</strong> de leur taille d'origine. Combinée avec le reranking SQ8, elle réduit l'utilisation de la mémoire à ~28%, augmente le QPS de 4×, et maintient un rappel de ~95%, réduisant de manière significative les coûts matériels.</p></li>
<li><p><strong>Recherche plein texte</strong><a href="https://milvus.io/docs/full-text-search.md#BM25-implementation"><strong>optimisée par BM25</strong></a> - Notation native BM25 alimentée par des vecteurs de poids de termes épars. La recherche par mot-clé est <strong>3 à 4 fois plus rapide</strong> (jusqu'à <strong>7 fois</strong> sur certains ensembles de données) qu'Elasticsearch, tout en maintenant la taille de l'index à environ un tiers des données textuelles d'origine.</p></li>
...
<p>Lorsque <code translate="no">spec.components.mixCoord</code> est configuré, l'opérateur lance MixCoord et attend qu'il soit prêt. Une fois que MixCoord est pleinement opérationnel, l'opérateur arrête de manière gracieuse les anciens composants du coordinateur - RootCoord, QueryCoord et DataCoord - achevant ainsi la migration sans nécessiter d'intervention manuelle.</p>
...
<p><strong>1. Mode autonome :</strong> Pour les déploiements sensibles aux coûts, RocksMQ est recommandé.</p>
...
<p><strong>3. Woodpecker (introduit dans Milvus 2.6) :</strong> Woodpecker supprime la nécessité d'une file d'attente de messages externe, ce qui réduit les coûts et la complexité opérationnelle.</p>
<ul>
...
<p>Avec des capacités de recherche améliorées, des types de données plus riches, un stockage hiérarchisé et des options de file d'attente de messages améliorées, Milvus 2.6 est bien positionné pour prendre en charge les applications d'IA modernes qui nécessitent une ingestion en temps réel, des performances de requête élevées et des opérations efficaces à l'échelle.</p>

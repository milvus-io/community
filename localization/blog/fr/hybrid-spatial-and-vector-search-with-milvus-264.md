...
desc: >-
  ...
  hybride à l'aide de Geometry et R-Tree, avec des aperçus des performances et
...
<h2 id="How-R-Tree-Indexing-Works-Inside-Milvus" class="common-anchor-header">Fonctionnement de l'indexation R-Tree dans Milvus<button data-href="#How-R-Tree-Indexing-Works-Inside-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>La prise en charge de la GÉOMÉTRIE est intégrée au moteur de requête Milvus et n'est pas simplement exposée en tant que fonctionnalité API. Les données spatiales sont indexées et traitées directement dans le moteur à l'aide de l'index R-Tree (Rectangle Tree).</p>
...
<h2 id="Performance-Improvements-with-R-Tree" class="common-anchor-header">Amélioration des performances avec R-Tree<button data-href="#Performance-Improvements-with-R-Tree" class="anchor-icon" translate="no">
    ...
    </button></h2><h3 id="Query-Latency-Stays-Flat-as-Data-Grows" class="common-anchor-header">La latence des requêtes reste stable au fur et à mesure que les données augmentent.</h3><p>Sans index R-Tree, le temps de requête est linéairement proportionnel à la taille des données - 10 fois plus de données signifient des requêtes environ 10 fois plus lentes.</p>

...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Insert-data" class="common-anchor-header">Insérer des données</h3><p>Interroger les lignes de texte, générer des embeddings et insérer les données dans Milvus. Le champ <code translate="no">text</code> n'est pas défini dans le schéma. Il est automatiquement ajouté en tant que champ dynamique soutenu par le champ JSON réservé de Milvus :</p>

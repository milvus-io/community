...
desc: >-
  Gemini Embedding 2 de Google permet d'intégrer du texte, des images, des
  vidéos et du son dans un seul vecteur. Cela rend-il la recherche
  multi-vectorielle obsolète ? Nous analysons la place de chaque approche.
origin: >-
  https://milvus.io/blog/will-gemini-embedding-2-kill-multi-vector-search-in-vector-databases.md
---
<p>Google a publié <a href="https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-embedding-2/">Gemini Embedding 2</a>, le premier modèle d'intégration multimodale qui associe le texte, les images, la vidéo, l'audio et les documents dans un espace vectoriel unique.</p>
<p>Vous intégrez un clip vidéo, une photo de produit et un paragraphe de texte en un seul appel d'API, et ils atterrissent tous dans le même voisinage sémantique.</p>
<p>Avant de disposer de tels modèles, vous n'aviez d'autre choix que de faire passer chaque modalité par son propre modèle spécialisé et de stocker chaque résultat dans une colonne vectorielle distincte. Les colonnes multi-vectorielles des bases de données vectorielles telles que <a href="https://milvus.io/docs/multi-vector-search.md">Milvus</a> ont été conçues précisément pour ce monde.</p>
<p>La question est donc de savoir dans quelle mesure Gemini Embedding 2 peut remplacer cette complexité et où il échoue. Cet article présente la place de chaque approche et la manière dont elles fonctionnent ensemble.</p>
<h2 id="What’s-Different-About-Gemini-Embedding-2-When-Compared-to-CLIPCLAP" class="common-anchor-header">Les différences entre Gemini Embedding 2 et CLIP/CLAP<button data-href="#What’s-Different-About-Gemini-Embedding-2-When-Compared-to-CLIPCLAP" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Les modèles d'incorporation convertissent les données non structurées en vecteurs denses de sorte que les éléments sémantiquement similaires se regroupent dans l'espace vectoriel. Ce qui distingue Gemini Embedding 2, c'est qu'il le fait nativement à travers les modalités, sans modèles séparés et sans pipelines d'assemblage.</p>
...
<p><strong>Désormais,</strong> vous introduisez les images, le son et les sous-titres de la vidéo dans un seul appel API et vous obtenez un vecteur unifié qui capture l'ensemble de l'image sémantique.</p>
...
<ul>
<li><strong>La recherche vectorielle dense et éparse.</strong> Vous disposez d'un catalogue de produits dans lequel les utilisateurs saisissent des requêtes telles que "Nike Air Max rouge taille 10". Les vecteurs denses saisissent l'intention sémantique ("chaussures de course, rouge, Nike"), mais ne connaissent pas la taille exacte. Les vecteurs peu denses via <a href="https://milvus.io/docs/full-text-search.md">BM25</a> ou des modèles tels que <a href="https://milvus.io/docs/full_text_search_with_milvus.md">BGE-M3</a> permettent d'obtenir la correspondance avec le mot-clé. Les deux doivent être exécutés en parallèle, puis reclassés, car ni l'un ni l'autre ne permet d'obtenir de bons résultats pour les requêtes qui mélangent le langage naturel et des identifiants spécifiques tels que les références, les noms de fichiers ou les codes d'erreur.</li>
...
<p><a href="https://milvus.io/">Milvus</a> exécute les deux modèles en tant que <a href="https://milvus.io/docs/multi-vector-search.md">recherches ANN</a> parallèles avec un reclassement natif via RRFRanker. Définition du schéma, configuration multi-index, BM25 intégré - tout est géré dans un seul système.</p>
<p>Pour rendre cela concret, considérons un catalogue de produits dans lequel chaque article comprend une description textuelle et une image. Vous pouvez effectuer trois recherches en parallèle sur ces données :</p>
<ul>
...
<h2 id="Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="common-anchor-header">Aujourd'hui, avec Gemini Embedding 2, la recherche multi vectorielle est-elle encore importante ?<button data-href="#Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Gemini Embedding 2 traite plus de modalités en un seul appel, ce qui simplifie considérablement les pipelines. Mais l'intégration multimodale unifiée n'est pas la même chose que la recherche multivectorielle. En d'autres termes, la recherche multi-vectorielle ne deviendra pas obsolète.</p>
...
<p><strong>Commerce électronique avec intention mixte.</strong> La vidéo promotionnelle et les images détaillées d'un produit fonctionnent bien en tant qu'intégration Gemini unifiée. Mais lorsqu'un utilisateur veut des "robes qui ressemblent à ceci" <em>et</em> "même tissu, taille M", vous avez besoin d'une colonne de similarité visuelle et d'une colonne d'attributs structurés avec des index distincts et une couche d'extraction hybride.</p>
...
<p>Ces deux approches ne s'excluent pas mutuellement. Vous pouvez utiliser Gemini Embedding 2 pour la colonne multimodale unifiée et continuer à stocker des vecteurs épars ou spécifiques à un attribut dans des colonnes distinctes au sein de la même collection<a href="https://milvus.io/">Milvus</a> <a href="https://milvus.io/"></a><a href="https://milvus.io/"></a>.</p>
<h2 id="Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="common-anchor-header">Démarrage rapide : Configuration de Gemini Embedding 2 + Milvus<button data-href="#Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Voici une démonstration fonctionnelle. Vous avez besoin d'une<a href="https://milvus.io/docs/install-overview.md">instance</a> <a href="https://milvus.io/docs/install-overview.md"></a><a href="https://milvus.io/docs/install-overview.md">Milvus ou Zilliz Cloud</a> en cours d'exécution et d'une clé GOOGLE_API_KEY.</p>

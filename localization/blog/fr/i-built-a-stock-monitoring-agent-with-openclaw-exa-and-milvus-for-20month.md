...
<h2 id="Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="common-anchor-header">Étape 1 : Obtenir des informations en temps réel sur les marchés avec Exa<button data-href="#Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Auparavant, j'avais essayé de parcourir des applications financières, d'écrire des scraps et d'examiner des terminaux de données professionnels. Les applications dissimulaient le signal sous le bruit, les scrapers tombaient constamment en panne et les API professionnelles étaient réservées aux traders institutionnels. Exa est une API de recherche conçue pour les agents d'intelligence artificielle qui résout les problèmes susmentionnés.</p>
<p>
...
<h2 id="Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="common-anchor-header">Étape 2 : Stocker l'historique des transactions dans Milvus pour prendre des décisions plus intelligentes<button data-href="#Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Exa a résolu mon problème d'information. Mais je continuais à répéter les mêmes opérations - vente de panique lors des baisses qui se rétablissaient en quelques jours, poursuite de l'élan dans des actions déjà surévaluées. J'agissais sous le coup de l'émotion, je le regrettais et j'oubliais la leçon à chaque fois qu'une situation similaire se présentait.</p>
<p>J'avais besoin d'une base de connaissances personnelles : mes transactions passées, mon raisonnement, mes erreurs. Il ne s'agissait pas d'une base que je devais consulter manuellement (j'avais déjà essayé et je n'avais jamais réussi à la maintenir), mais d'une base que l'agent pouvait consulter de lui-même lorsqu'une situation similaire se présentait. Si je suis sur le point de répéter une erreur, je veux que l'agent me le dise avant que j'appuie sur le bouton. Faire correspondre la "situation actuelle" à l'"expérience passée" est un problème de recherche de similarité que les bases de données vectorielles résolvent, de sorte que toutes mes données devaient être stockées dans l'une d'entre elles.</p>
<p>J'ai utilisé <a href="https://github.com/milvus-io/milvus-lite">Milvus Lite</a>, une version légère de Milvus qui s'exécute localement sans installation de serveur. Elle est parfaite pour le prototypage et l'expérimentation. J'ai divisé mes données en trois collections. La dimension d'intégration est de 1536 pour correspondre au modèle text-embedding-3-small d'OpenAI, qui peut être utilisé directement :</p>
...
<p>Mélanger ces éléments en une seule collection reviendrait soit à gonfler chaque invite avec un historique de transactions non pertinent, soit à perdre des préjugés fondamentaux lorsqu'ils ne correspondent pas suffisamment à la requête en cours.</p>
<p>Une fois que les collections existent, l'agent doit les alimenter automatiquement. Je ne veux pas prendre de notes après chaque conversation, c'est pourquoi j'ai construit un extracteur de mémoire qui s'exécute à la fin de chaque session de chat.</p>
<p>Il fait deux choses : extraire et dédupliquer. L'extracteur demande au LLM d'extraire des informations structurées de la conversation - décisions, préférences, schémas, leçons - et de les acheminer vers la bonne collection. Avant de stocker quoi que ce soit, il vérifie la similarité avec ce qui existe déjà. Si une nouvelle information est similaire à plus de 92 % à une entrée existante, elle est ignorée.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Par exemple, lorsque les valeurs technologiques ont chuté de 3 à 4 % en raison des tensions au Moyen-Orient au début du mois de mars, l'agent a retrouvé trois éléments : une leçon d'octobre 2024 sur la nécessité de ne pas vendre dans la panique lors d'un creux géopolitique, une note de préférence indiquant que j'ai tendance à surpondérer le risque géopolitique et un modèle que j'avais enregistré, selon lequel les baisses des valeurs technologiques provoquées par la géopolitique se rétablissent généralement en une à trois semaines.</p>
<p>Mon collègue a appelé cela "l'apprentissage par renforcement pour les investisseurs particuliers", où je suis le modèle formé sur ma propre fonction de perte. C'est tout à fait normal. Au moins, il y a maintenant une boucle de rétroaction entre ce que j'ai fait et ce que je m'apprête à faire.</p>
<h2 id="Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="common-anchor-header">Étape 3 : Apprendre à votre agent à analyser avec les compétences d'OpenClaw<button data-href="#Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>À ce stade, l'agent dispose de bonnes informations<a href="https://exa.ai/">(Exa</a>) et d'une mémoire personnelle<a href="https://github.com/milvus-io/milvus-lite">(Milvus</a>). Mais si vous donnez les deux à un LLM et lui dites " analysez ceci ", vous obtiendrez une réponse générique, qui couvre tout. Elle mentionne tous les angles possibles et se termine par "les investisseurs doivent peser les risques", autrement dit, rien.</p>
<p>La solution consiste à rédiger votre propre cadre analytique et à le donner à l'agent sous forme d'instructions explicites. Quels sont les indicateurs qui vous intéressent, quelles sont les situations que vous considérez comme dangereuses, quand faut-il être conservateur ou agressif ? Ces règles sont différentes pour chaque investisseur, vous devez donc les définir vous-même.</p>
...
<h2 id="Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="common-anchor-header">Étape 4 : Démarrer votre agent avec OpenClaw Heartbeat<button data-href="#Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Tout ce qui précède nécessite encore un déclenchement manuel. Si vous devez ouvrir un terminal à chaque fois que vous voulez une mise à jour, vous revenez à la case départ - et vous risquez à nouveau de condamner votre application de courtage pendant les réunions.</p>
...
<p>Coût total : <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><mrow><mn>10</mn><mi>euros/mois</mi></mrow></semantics></math></span></span>pour <span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML"><semantics><annotation encoding="application/x-tex">OpenClaw, 10 euros/mois pour OpenClaw,</annotation></semantics></math></span><span class="katex-html" aria-hidden="true"><span class="base"><span class="strut" style="height:1em;vertical-align:-0.25em;"></span><span class="mord">10 euros/mois pour</span><span class="mord mathnormal" style="margin-right:0.02691em;">OpenClaw</span><span class="mpunct">,</span></span></span></span>10 euros/mois pour Exa, et un peu d'électricité pour faire fonctionner Milvus Lite.</p>
...
<ul>
<li>Il<strong>recueille automatiquement des informations fiables sur les marchés</strong> <strong>à l'aide d'</strong> <strong><a href="https://exa.ai/">Exa</a></strong>, ce qui me permet de commencer chaque matin avec un briefing propre au lieu de passer une heure à scruter l'avenir.</li>
<li><strong>Il se souvient de mes transactions passées avec <a href="https://milvus.io/">Milvus</a> et m'avertit</strong> lorsque je suis sur le point de répéter une erreur, en utilisant Milvus comme base de données vectorielle personnelle.</li>
<li><strong>Fonctionne sur le</strong> <strong>pilote automatique d'</strong> <strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">OpenClaw</a></strong> <strong>et ne m'envoie des pings que lorsque c'est important</strong>, en utilisant OpenClaw Skills et Heartbeat - pas de tâches cron, pas de code collé.</li>
</ul>
<p>Coût total : 20 $/mois. L'agent ne négocie pas et ne touche pas à mon argent.</p>

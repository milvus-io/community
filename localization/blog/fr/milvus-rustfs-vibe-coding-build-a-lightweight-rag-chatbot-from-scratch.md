...
<p>Milvus peut fonctionner avec n'importe quel stockage objet compatible S3, bien que MinIO soit le backend par défaut dans la configuration standard. Comme MinIO n'accepte plus les contributions de la communauté, nous le remplacerons par RustFS dans cet exemple.</p>
...
<pre><code translate="no">etcd:
    ...
    healthcheck:
      ...
      retries: 3
<button class="copy-code-btn"></button></code></pre>
<p><strong>Attu</strong> - une interface visuelle pour Milvus, développée et mise en open-source par Zilliz (note : les versions postérieures à 2.6 sont fermées) :</p>
...
<h3 id="FastAPI-Backend" class="common-anchor-header">Backend FastAPI</h3><p>FastAPI enveloppe la logique RAG dans un seul point d'extrémité POST. N'importe quel client peut maintenant interroger votre base de connaissances avec une requête JSON :</p>
...
<h2 id="Conclusion" class="common-anchor-header">Conclusion<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Ce qui a commencé comme une curiosité sur le backend de stockage de Milvus s'est transformé en un chatbot RAG pleinement fonctionnel - et le chemin de l'un à l'autre a été plus court que prévu. Voici ce que couvre la construction, de bout en bout :</p>
<ul>

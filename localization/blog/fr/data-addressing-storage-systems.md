...
<h2 id="Milvus-V2-How-Segment-Level-Parquet-Cuts-API-Calls-by-10x" class="common-anchor-header">Milvus V2 : Comment le Parquet au niveau des segments divise par 10 les appels à l'API<button data-href="#Milvus-V2-How-Segment-Level-Parquet-Cuts-API-Calls-by-10x" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<ol>
<li><strong>Consultation des métadonnées</strong> - Extraction de la liste des segments → Lecture des pieds de page du Parquet <code translate="no">[12345, 12346, …]</code></li>
...
<ul>
<li><strong>Des lectures plus efficaces.</strong> Les <a href="https://zilliz.com/glossary/vector-embeddings">encastrements vectoriels</a> dominent la taille du stockage. Mélangés à de petits champs, ils limitent le nombre de lignes que peut contenir un groupe de lignes, ce qui augmente le nombre d'accès au fichier. En les isolant, les groupes de lignes de petits champs peuvent contenir beaucoup plus de lignes, tandis que les grands champs utilisent des mises en page optimisées pour leur taille.</li>
<li><strong>Évolution souple du <a href="https://milvus.io/docs/schema.md">schéma</a>.</strong> L'ajout d'une colonne implique la création d'un nouveau fichier. Pour en supprimer une, il suffit de l'ignorer au moment de la lecture. Aucune réécriture des données historiques n'est nécessaire.</li>
</ul>
...
<ul>
<li>HashMap calcule un index de tableau à partir de <code translate="no">hash(key)</code> au lieu d'effectuer un balayage</li>

...
desc: >-
  ...
  vettore. Questo rende la ricerca multivettoriale obsoleta? Analizziamo le
  caratteristiche di ciascun approccio.
origin: >-
...
<p>È possibile incorporare un videoclip, una foto di un prodotto e un paragrafo di testo con una sola chiamata API, e tutti finiscono nella stessa zona semantica.</p>
<p>Prima di modelli come questo, non c'era altra scelta che eseguire ogni modalità attraverso il proprio modello specializzato e memorizzare ogni risultato in una colonna vettoriale separata. Le colonne multivettore nei database vettoriali come <a href="https://milvus.io/docs/multi-vector-search.md">Milvus</a> sono state costruite proprio per questo mondo.</p>
<p>La domanda da porsi è quindi: quanta parte di questa complessità può essere sostituita da Gemini Embedding 2 e dove non è all'altezza? Questo post spiega dove si colloca ciascun approccio e come funzionano insieme.</p>
...
<p>Immagine<strong>di input mista</strong> + testo insieme in un'unica chiamata di embedding</p>
...
<h2 id="Gemini-Embedding-2’s-Advantage-Pipeline-Simplification" class="common-anchor-header">Il vantaggio di Gemini Embedding 2: Semplificazione della pipeline<button data-href="#Gemini-Embedding-2’s-Advantage-Pipeline-Simplification" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Prendiamo uno scenario comune: la costruzione di un motore di ricerca semantico su una breve videoteca. Ogni clip presenta fotogrammi visivi, audio parlato e testo dei sottotitoli, che descrivono lo stesso contenuto.</p>
...
<p>Naturalmente, si è tentati di concludere che le colonne multivettore sono morte. Ma questa conclusione confonde la "rappresentazione unificata multimodale" con il "recupero vettoriale multidimensionale". Risolvono problemi diversi e la comprensione della differenza è importante per scegliere l'approccio giusto.</p>
...
<p><a href="https://milvus.io/">Milvus</a> esegue entrambi i modelli come <a href="https://milvus.io/docs/multi-vector-search.md">ricerche ANN</a> parallele con reranking nativo tramite RRFRanker. Definizione dello schema, configurazione di più indici, BM25 integrato: tutto gestito in un unico sistema.</p>
<p>Per renderlo concreto, si consideri un catalogo di prodotti in cui ogni articolo include una descrizione testuale e un'immagine. È possibile eseguire tre ricerche in parallelo su questi dati:</p>
<ul>
...
<h2 id="Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="common-anchor-header">Ora, con Gemini Embedding 2, la ricerca multivettoriale è ancora importante?<button data-href="#Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Gemini Embedding 2 gestisce più modalità in un'unica chiamata, semplificando notevolmente le pipeline. Ma un embedding multimodale unificato non è la stessa cosa di un recupero multivettoriale. In altre parole, la ricerca multivettoriale non diventerà obsoleta.</p>
<p>Gemini Embedding 2 mappa testo, immagini, video, audio e documenti in un unico spazio vettoriale condiviso. Google <a href="https://developers.googleblog.com/en/gemini-embedding-model-now-available/">lo posiziona</a> per la ricerca semantica multimodale, il reperimento di documenti e la raccomandazione - scenari in cui tutte le modalità descrivono lo stesso contenuto e l'elevata sovrapposizione cross-modale rende fattibile un unico vettore.</p>
<p><a href="https://milvus.io/docs/multi-vector-search.md">La</a> ricerca multivettoriale<a href="https://milvus.io/docs/multi-vector-search.md">di Milvus</a> risolve un problema diverso. È un modo per cercare lo stesso oggetto attraverso <strong>più campi vettoriali -</strong>per esempio, titolo e descrizione, o testo e immagine - e poi combinare questi segnali durante il recupero. In altre parole, si tratta di conservare e interrogare <strong>più viste semantiche</strong> dello stesso oggetto, non solo di comprimere tutto in un'unica rappresentazione.</p>
...
<tr><td>È necessaria la precisione delle parole chiave insieme al richiamo semantico (BM25 + denso)</td><td>Colonne multivettore con hybrid_search()</td><td>I vettori sparsi e densi servono a obiettivi di ricerca diversi che non possono confluire in un unico incorporamento.</td></tr>
...
<p>Questi due approcci non si escludono a vicenda. È possibile utilizzare Gemini Embedding 2 per la colonna multimodale unificata e memorizzare altri vettori sparsi o specifici per gli attributi in colonne separate all'interno della stessa raccolta <a href="https://milvus.io/"></a><a href="https://milvus.io/">Milvus</a>.</p>
<h2 id="Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="common-anchor-header">Avvio rapido: Impostazione di Gemini Embedding 2 + Milvus<button data-href="#Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Ecco una demo funzionante. È necessaria un'<a href="https://milvus.io/docs/install-overview.md">istanza</a> <a href="https://milvus.io/docs/install-overview.md"></a><a href="https://milvus.io/docs/install-overview.md">Milvus o Zilliz Cloud</a> funzionante e una GOOGLE_API_KEY.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Per le incorporazioni di immagini e audio, usare embed_image() e embed_audio() allo stesso modo: i vettori finiscono nella stessa collezione e nello stesso spazio vettoriale, consentendo una vera ricerca cross-modale.</p>

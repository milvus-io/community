...
<p>Per evitare ciò, il sistema utilizza il chunking a finestra scorrevole con sovrapposizione. Ogni chunk condivide una porzione di coda con il successivo, in modo che il contenuto limite appaia in entrambe le finestre. In questo modo si mantiene intatto il significato ai margini del chunk e si migliora il richiamo del testo.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-3-Store-Vectors-and-Metadata-in-Milvus" class="common-anchor-header">Fase 3: memorizzare i vettori e i metadati in Milvus</h3><p>Una volta pronti i chunk puliti, il passo successivo consiste nell'immagazzinarli in modo tale da poterli recuperare in modo rapido e preciso.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Step-5-Rerank-Results-Before-Answer-Generation" class="common-anchor-header">Fase 5: Riclassificazione dei risultati prima della generazione delle risposte</h3><p>I chunk restituiti dalla fase di ricerca non sono ugualmente rilevanti. Quindi, prima di generare la risposta finale, un passaggio di reranking li riclassifica.</p>
...
<p>Il sistema rileva i termini in maiuscolo come "Milvus" o "RAG" come probabili nomi propri e identifica i termini tecnici con più parole come possibili frasi chiave.</p>
...
<li><strong>Risultato pratico</strong>Sia che le formule compaiano nella risposta del modello o nei sommari dei documenti, vengono rese correttamente sulla pagina.</li>
</ul>

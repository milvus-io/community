...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Verify-the-result" class="common-anchor-header">Verifica del risultato</h3><p>Il kubelet controlla se il ConfigMap montato è fresco a ogni sincronizzazione periodica. Quando il ConfigMap consumato nel volume viene aggiornato, anche le chiavi proiettate vengono automaticamente aggiornate. Quando il nuovo pod è di nuovo in esecuzione, è possibile verificare se la modifica è avvenuta con successo nel pod. I comandi per verificare il parametro <code translate="no">rootcoord.dmlChannelNum</code> sono condivisi di seguito.</p>

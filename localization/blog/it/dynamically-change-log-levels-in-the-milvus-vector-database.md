...
<p>Per evitare che un eccesso di log influisca sulle prestazioni del disco e del sistema, Milvus emette per impostazione predefinita i log al livello <code translate="no">info</code> durante l'esecuzione. Tuttavia, a volte i log al livello <code translate="no">info</code> non sono sufficienti per aiutarci a identificare efficacemente bug e problemi. Inoltre, in alcuni casi, la modifica del livello di log e il riavvio del servizio possono portare alla mancata riproduzione dei problemi, rendendo ancora più difficile la risoluzione dei problemi. Di conseguenza, il supporto per la modifica dinamica dei livelli di log nel database dei vettori di Milvus è urgentemente necessario.</p>
...
<h3 id="Change-the-log-level" class="common-anchor-header">Modifica del livello di log</h3><p>Supponiamo che l'indirizzo IP del proxy Milvus sia <code translate="no">192.168.48.12</code>.</p>

...
<p>Entrambi gli scenari rendono la latenza dell'API di embedding una metrica di performance non negoziabile per i sistemi RAG di produzione.</p>
<h2 id="Measuring-Real-World-Embedding-API-Latency-with-Milvus" class="common-anchor-header">Misurare la latenza delle API di embedding nel mondo reale con Milvus<button data-href="#Measuring-Real-World-Embedding-API-Latency-with-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Milvus è un database vettoriale open-source ad alte prestazioni che offre una nuova interfaccia <code translate="no">TextEmbedding</code> Function. Questa funzione integra i modelli di embedding più diffusi di OpenAI, Cohere, AWS Bedrock, Google Vertex AI, Voyage AI e molti altri fornitori direttamente nella pipeline di dati, ottimizzando la pipeline di ricerca vettoriale con una sola chiamata.</p>
<p>Utilizzando questa nuova interfaccia funzionale, abbiamo testato e sottoposto a benchmark diverse API di embedding popolari di fornitori di modelli americani come OpenAI e Cohere, nonché di fornitori asiatici come AliCloud e SiliconFlow, misurando la loro latenza end-to-end in scenari di implementazione realistici.</p>
<p>La nostra suite di test completa ha coperto varie configurazioni di modelli:</p>
<table>
<thead>
...
<h2 id="7-Key-Findings-from-Our-Benchmarking-Results" class="common-anchor-header">7 Risultati chiave dei nostri benchmark<button data-href="#7-Key-Findings-from-Our-Benchmarking-Results" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Abbiamo testato rinomati modelli di embedding provenienti dal Nord America e dall'Asia in base a diverse dimensioni di batch, lunghezza dei token e condizioni di rete, misurando la latenza mediana in tutti gli scenari. I nostri risultati rivelano intuizioni fondamentali che cambieranno il modo di pensare alla selezione e all'ottimizzazione delle API di embedding. Diamo un'occhiata.</p>
<h3 id="1-Global-Network-Effects-Are-More-Significant-Than-You-Think" class="common-anchor-header">1. Gli effetti globali della rete sono più significativi di quanto si pensi</h3><p>L'ambiente di rete è forse il fattore più critico che influisce sulle prestazioni delle API di embedding. Lo stesso fornitore di servizi API di embedding può avere prestazioni molto diverse a seconda dell'ambiente di rete.</p>
<p>
...
<p>Ciò significa che è necessario selezionare sempre i fornitori di embedding in base all'ubicazione della distribuzione e alla geografia dell'utente: le affermazioni sulle prestazioni senza un contesto di rete sono prive di significato.</p>
...
<ul>
<li><p><strong>Modelli nordamericani (latenza mediana)</strong>: Cohere &gt; Google Vertex AI &gt; VoyageAI &gt; OpenAI &gt; AWS Bedrock</p></li>
<li><p><strong>Modelli asiatici (latenza mediana)</strong>: SiliconFlow &gt; AliCloud Dashscope</p></li>
</ul>
...
<p>Nota: a causa dell'impatto significativo dell'ambiente di rete e delle regioni geografiche dei server sulla latenza delle API di embedding in tempo reale, abbiamo confrontato le latenze dei modelli nordamericani e asiatici separatamente.</p>
...
<h3 id="4-Token-Length-and-Batch-Size-Create-Complex-Trade-offs" class="common-anchor-header">4. La lunghezza dei token e la dimensione dei lotti creano compromessi complessi</h3><p>A seconda dell'implementazione del backend, in particolare delle strategie di batching, la lunghezza del token potrebbe non influire in modo significativo sulla latenza fino a quando le dimensioni dei batch non aumentano. I nostri test hanno rivelato modelli distinti:</p>
<ul>
...
<li><p><strong>VoyageAI</strong> ha mostrato chiari effetti sulla lunghezza dei token, il che implica una minima ottimizzazione del backend batching.</p></li>
</ul>
...
<p>Latenza TEI</p>
<h3 id="7-Milvus-Overhead-Is-Negligible" class="common-anchor-header">7. L'overhead di Milvus è trascurabile</h3><p>Poiché abbiamo usato Milvus per testare la latenza dell'API di incorporazione, abbiamo convalidato che l'overhead aggiuntivo introdotto dalla funzione TextEmbedding di Milvus è estremamente ridotto e praticamente trascurabile. Le nostre misurazioni mostrano che le operazioni di Milvus aggiungono solo 20-40 ms in totale, mentre le chiamate all'API di embedding richiedono da centinaia di millisecondi a diversi secondi: ciò significa che <strong>Milvus aggiunge meno del 5% di overhead</strong> al tempo totale dell'operazione. Il collo di bottiglia delle prestazioni risiede principalmente nella trasmissione di rete e nelle capacità di elaborazione dei fornitori di servizi API di embedding, non nel livello del server Milvus.</p>
...
<h3 id="5-Implement-Strategic-Caching" class="common-anchor-header">5. Implementare una cache strategica</h3><p>Per le query ad alta frequenza, memorizzare nella cache sia il testo della query che le sue incorporazioni generate (utilizzando soluzioni come Redis). Le interrogazioni successive identiche possono essere eseguite direttamente nella cache, riducendo la latenza a pochi millisecondi. Si tratta di una delle tecniche di ottimizzazione della latenza delle query più efficaci dal punto di vista dei costi e dell'impatto.</p>
...
<p>Per molti singoli sviluppatori o piccoli team, la mancanza di GPU di livello enterprise può sembrare un ostacolo alla distribuzione locale di modelli di embedding ad alte prestazioni. Tuttavia, questo non significa abbandonare del tutto l'inferenza locale. In combinazione con motori di inferenza ad alte prestazioni come <a href="https://github.com/huggingface/text-embeddings-inference">il text-embeddings-inference di Hugging Face</a>, anche l'esecuzione di modelli di embedding di piccole o medie dimensioni su CPU può raggiungere prestazioni decenti che possono superare le chiamate API ad alta latenza, soprattutto per la generazione di embedding offline su larga scala.</p>
...
<li><p><strong>Compatibilità con l'ecosistema open source</strong>: Sia che si generino embeddings tramite la nostra funzione integrata <code translate="no">TextEmbedding</code>, l'inferenza locale o un altro metodo, Milvus fornisce capacità di archiviazione e recupero unificate.</p></li>
</ul>

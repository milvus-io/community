...
title: Come iniziare con la ricerca semantica ibrida / full text con Milvus 2.5
author: Stefan Webb
...
---
<p>In questo articolo vi mostreremo come utilizzare rapidamente la nuova funzione di ricerca full-text e come combinarla con la ricerca semantica convenzionale basata su embeddings vettoriali.</p>
...
<h2 id="Requirement" class="common-anchor-header">Requisiti<button data-href="#Requirement" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Per prima cosa, assicuratevi di aver installato Milvus 2.5:</p>
<pre><code translate="no">pip install -U pymilvus[model]
<button class="copy-code-btn"></button></code></pre>
<p>e avere un'istanza funzionante di Milvus Standalone (ad esempio, sul computer locale) utilizzando le <a href="https://milvus.io/docs/prerequisite-docker.md">istruzioni di installazione contenute nei documenti di Milvus</a>.</p>
<h2 id="Building-the-Data-Schema-and-Search-Indices" class="common-anchor-header">Creare lo schema dei dati e gli indici di ricerca<button data-href="#Building-the-Data-Schema-and-Search-Indices" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Importiamo le classi e le funzioni necessarie:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Avrete notato il parametro <code translate="no">enable_analyzer=True</code>. Questo parametro indica a Milvus 2.5 di abilitare il parser lessicale su questo campo e di creare un elenco di token e di frequenze di token, necessari per la ricerca full-text. Il campo <code translate="no">sparse</code> conterrà una rappresentazione vettoriale della documentazione come bag-of-words prodotta dal parsing <code translate="no">text</code>.</p>
<p>Ma come possiamo collegare i campi <code translate="no">text</code> e <code translate="no">sparse</code> e dire a Milvus come <code translate="no">sparse</code> deve essere calcolato da <code translate="no">text</code>? È qui che dobbiamo richiamare l'oggetto <code translate="no">Function</code> e aggiungerlo allo schema:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>L'astrazione dell'oggetto <code translate="no">Function</code> è più generale rispetto all'applicazione della ricerca full-text. In futuro, potrà essere utilizzato per altri casi in cui un campo deve essere una funzione di un altro campo. Nel nostro caso, specifichiamo che <code translate="no">sparse</code> è una funzione di <code translate="no">text</code> tramite la funzione <code translate="no">FunctionType.BM25</code>. <code translate="no">BM25</code> si riferisce a una metrica comune nel reperimento di informazioni, utilizzata per calcolare la somiglianza di una query con un documento (rispetto a un insieme di documenti).</p>
<p>Utilizziamo il modello di incorporamento predefinito di Milvus, che è <a href="https://huggingface.co/GPTCache/paraphrase-albert-small-v2">parafrasi-alberto-piccolo-v2</a>:</p>
<pre><code translate="no">embedding_fn = model.DefaultEmbeddingFunction()
<button class="copy-code-btn"></button></code></pre>
<p>Il passo successivo è aggiungere i nostri indici di ricerca. Ne abbiamo uno per il vettore denso e uno separato per il vettore rado. Il tipo di indice è <code translate="no">SPARSE_INVERTED_INDEX</code> con <code translate="no">BM25</code>, poiché la ricerca full-text richiede un metodo di ricerca diverso da quello dei vettori densi standard.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Infine, creiamo la nostra collezione:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>E con ciò, abbiamo un database vuoto impostato per accettare documenti di testo ed eseguire ricerche semantiche e full-text!</p>
<h2 id="Inserting-Data-and-Performing-Full-Text-Search" class="common-anchor-header">Inserimento dei dati ed esecuzione della ricerca full-text<button data-href="#Inserting-Data-and-Performing-Full-Text-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>L'inserimento dei dati non è diverso dalle versioni precedenti di Milvus:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Illustriamo innanzitutto una ricerca full-text prima di passare alla ricerca ibrida:</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Il parametro di ricerca <code translate="no">drop_ratio_search</code> si riferisce alla percentuale di documenti con punteggio inferiore da eliminare durante l'algoritmo di ricerca.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h2 id="Performing-Hybrid-Semantic-and-Full-Text-Search" class="common-anchor-header">Esecuzione di una ricerca ibrida semantica e full-text<button data-href="#Performing-Hybrid-Semantic-and-Full-Text-Search" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Combiniamo ora quanto appreso per eseguire una ricerca ibrida che combina ricerche semantiche e full-text separate con un reranker:</p>
...
<h2 id="Summary" class="common-anchor-header">Riassunto<button data-href="#Summary" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Ora disponete di tutte le conoscenze necessarie per eseguire ricerche full-text e ibride semantiche/full-text con Milvus 2.5. Per ulteriori informazioni su come funziona la ricerca full-text e perché è complementare alla ricerca semantica, consultare i seguenti articoli:</p>
<ul>
<li><a href="https://milvus.io/blog/introduce-milvus-2-5-full-text-search-powerful-metadata-filtering-and-more.md">Presentazione di Milvus 2.5: ricerca full-text, filtraggio dei metadati più potente e miglioramenti dell'usabilità!</a></li>
<li><a href="https://milvus.io/blog/semantic-search-vs-full-text-search-which-one-should-i-choose-with-milvus-2-5.md">Ricerca semantica contro ricerca a tutto testo: Quale scegliere in Milvus 2.5?</a></li>
</ul>

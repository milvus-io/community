...
<p>La parte della ripetizione è quella che mi uccide. Ogni volta che osservo il mercato, finisco per fare un'operazione che non avevo previsto. Il petrolio sale e io vendo nel panico. Quel titolo tecnologico sale del 4% e lo inseguo. Una settimana dopo, guardando la mia cronologia degli scambi, mi chiedo: <em>non</em> ho <em>fatto esattamente questa cosa lo scorso trimestre?</em></p>
<p>Ho quindi creato un agente con OpenClaw che osserva il mercato al posto mio e mi impedisce di commettere gli stessi errori. Non fa trading e non tocca il mio denaro; mi impedisce di fare qualcosa di cui mi sono già pentito una volta.</p>
...
<h2 id="Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="common-anchor-header">Passo 1: ottenere informazioni di mercato in tempo reale con Exa<button data-href="#Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="anchor-icon" translate="no">
    ...
    </button></h2><p>In precedenza, avevo provato a navigare tra le app finanziarie, a scrivere scrapers e a cercare terminali di dati professionali. Le app seppellivano il segnale sotto il rumore, gli scrapers si rompevano continuamente e le API professionali avevano un prezzo da trader istituzionale. Exa è un'API di ricerca costruita per agenti AI che risolve i problemi di cui sopra.</p>
<p>
...
<p><strong><a href="https://exa.ai/">Exa</a></strong> è un'API di ricerca web che restituisce dati strutturati e pronti per l'intelligenza artificiale. È alimentata da <a href="https://zilliz.com/cloud">Zilliz Cloud</a> (il servizio completamente gestito di Milvus).  Se Perplexity è un motore di ricerca usato dagli esseri umani, Exa è usato dall'intelligenza artificiale. L'agente invia una query ed Exa restituisce il testo dell'articolo, le frasi chiave e i riassunti in formato JSON: un output strutturato che l'agente può analizzare e utilizzare direttamente, senza bisogno di scraping.</p>
...
<h2 id="Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="common-anchor-header">Fase 2: memorizzare la storia del trading in Milvus per prendere decisioni più intelligenti<button data-href="#Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Exa ha risolto il mio problema di informazioni. Ma continuavo a ripetere le stesse operazioni: panic-selling durante i ribassi che si riprendevano nel giro di pochi giorni, inseguendo il momentum in titoli che erano già sopravvalutati. Agivo in base alle emozioni, me ne pentivo e dimenticavo la lezione quando si ripresentava una situazione simile.</p>
<p>Avevo bisogno di una base di conoscenze personali: le mie operazioni passate, i miei ragionamenti, i miei errori. Non qualcosa che dovessi rivedere manualmente (ci avevo provato e non avevo mai continuato), ma qualcosa che l'agente potesse cercare da solo ogni volta che si presentava una situazione simile. Se sto per ripetere un errore, voglio che l'agente me lo dica prima che io prema il pulsante. La corrispondenza tra "situazione attuale" ed "esperienza passata" è un problema di ricerca di similarità che i database vettoriali risolvono, quindi tutti i miei dati dovevano essere memorizzati in uno di essi.</p>
<p>Ho usato <a href="https://github.com/milvus-io/milvus-lite">Milvus Lite</a>, una versione leggera di Milvus che funziona localmente senza bisogno di configurare un server. È perfetta per la prototipazione e la sperimentazione. Ho suddiviso i miei dati in tre collezioni. La dimensione di incorporazione è di 1536 per corrispondere al modello text-embedding-3-small di OpenAI, che può essere utilizzato direttamente:</p>
...
<tr><td><strong>Conoscenza esterna</strong></td><td>Rapporti di ricerca, documenti SEC, dati pubblici</td><td>Non memorizzate in Milvus, ma ricercabili tramite Exa.</td></tr>
</tbody>
</table>
<p>Mescolare questi dati in un'unica raccolta significherebbe o gonfiare ogni richiesta di informazioni con una storia di trading irrilevante o perdere i pregiudizi fondamentali quando non corrispondono abbastanza alla richiesta corrente.</p>
<p>Una volta che le raccolte esistono, l'agente deve popolarle automaticamente. Non voglio scrivere note dopo ogni conversazione, quindi ho costruito un estrattore di memoria che viene eseguito alla fine di ogni sessione di chat.</p>
<p>Fa due cose: estrae e deduplica. L'estrattore chiede all'LLM di estrarre dalla conversazione informazioni strutturate - decisioni, preferenze, schemi, lezioni - e le indirizza alla giusta raccolta. Prima di memorizzare qualcosa, controlla la somiglianza con ciò che è già presente. Se una nuova intuizione è simile per più del 92% a una voce esistente, viene saltata.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>Ad esempio, quando i titoli tecnologici sono scesi del 3-4% a causa delle tensioni in Medio Oriente all'inizio di marzo, l'agente ha richiamato tre cose: una lezione dell'ottobre 2024 su come evitare il panic-selling durante un calo geopolitico, una nota di preferenza secondo cui tendo a sovrappesare il rischio geopolitico e uno schema che avevo registrato secondo cui i ribassi dei titoli tecnologici causati dalla geopolitica si riprendono in genere in una o tre settimane.</p>
<p>Il mio collega lo ha definito "apprendimento per rinforzo per gli investitori al dettaglio", dove io sono il modello addestrato sulla mia funzione di perdita. Mi sembra giusto. Almeno ora c'è un ciclo di feedback tra ciò che ho fatto e ciò che sto per fare.</p>
<h2 id="Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="common-anchor-header">Fase 3: insegnare all'agente ad analizzare con le competenze di OpenClaw<button data-href="#Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="anchor-icon" translate="no">
    ...
    </button></h2><p>A questo punto l'agente ha buone informazioni<a href="https://exa.ai/">(Exa</a>) e una memoria personale<a href="https://github.com/milvus-io/milvus-lite">(Milvus</a>). Ma se date entrambe le cose in mano a un LLM e gli dite "analizza questo", otterrete una risposta generica, con una copertura di tutto. La risposta cita ogni possibile angolazione e si conclude con "gli investitori dovrebbero soppesare i rischi", in altre parole, niente.</p>
<p>La soluzione è scrivere il proprio quadro analitico e fornirlo all'agente sotto forma di istruzioni esplicite. Quali indicatori vi interessano, quali situazioni considerate pericolose, quando essere conservativi o aggressivi. Queste regole sono diverse per ogni investitore, quindi dovete definirle voi stessi.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>L'ultima riga è la più importante: "Riemergere sempre dai miei errori passati. Ho la tendenza a lasciare che la paura prevalga sui dati. Se la mia storia Milvus mostra che mi sono pentito di aver venduto dopo un calo, ditelo esplicitamente". Questo è un meccanismo di correzione costruito appositamente per la mia psicologia. La vostra versione codificherebbe le vostre tendenze.</p>
...
<h2 id="Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="common-anchor-header">Fase 4: Avviare l'agente con OpenClaw Heartbeat<button data-href="#Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Tutto quanto detto sopra richiede ancora di essere attivato manualmente. Se dovete aprire un terminale ogni volta che volete un aggiornamento, siete tornati al punto di partenza, probabilmente scorrendo di nuovo la vostra applicazione di brokeraggio durante le riunioni.</p>
...
<ul>
<li><strong>Raccoglie automaticamente notizie di mercato affidabili</strong> <strong>utilizzando</strong> <strong><a href="https://exa.ai/">Exa</a></strong>, in modo che io inizi ogni mattina con un briefing pulito invece che con un'ora di scorrimento di sventure.</li>
<li><strong>Ricorda le mie operazioni passate con <a href="https://milvus.io/">Milvus</a> e mi avverte</strong> quando sto per ripetere un errore, usando Milvus come database vettoriale personale.</li>
<li><strong>Funziona con il</strong> <strong>pilota automatico</strong> <strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">di OpenClaw</a></strong> <strong>e mi avvisa solo quando è importante</strong>, utilizzando OpenClaw Skills e Heartbeat: nessun cron job, nessun codice di colla.</li>
</ul>
<p>Costo totale: $20/mese. L'agente non fa trading e non tocca il mio denaro.</p>

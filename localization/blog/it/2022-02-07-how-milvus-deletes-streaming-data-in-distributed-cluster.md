...
<p>Alla fine, in una ricerca o in un'interrogazione, i nodi di interrogazione generano un set di bit basato sui record di cancellazione per omettere le entità cancellate e cercare tra le entità rimanenti di tutti i segmenti, indipendentemente dallo stato del segmento. Infine, ma non meno importante, il livello di coerenza influisce sulla visibilità dei dati cancellati. Con il livello di consistenza Strong (come mostrato nell'esempio di codice precedente), le entità cancellate sono immediatamente invisibili dopo l'eliminazione. Se invece si adotta il livello di consistenza Bounded, ci saranno alcuni secondi di latenza prima che le entità cancellate diventino invisibili.</p>

...
<p>대부분의 시스템은 여전히 키워드 기반 강조 표시를 사용합니다. 사용자가 "iPhone 성능"을 검색하면 시스템은 정확한 토큰인 "iPhone"과 "성능"을 강조 표시합니다. 하지만 텍스트가 다른 문구를 사용하여 동일한 아이디어를 표현하는 순간 이 방식은 무너지고 맙니다. "A15 바이오닉 칩, 벤치마크에서 백만 점 이상, 랙 없이 부드러운"과 같은 설명은 성능을 분명히 다루지만 키워드가 나타나지 않기 때문에 아무것도 강조 표시되지 않습니다.</p>

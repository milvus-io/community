...
<p>그러나 벡터 데이터베이스의 인기가 높아지면서 전문화된 솔루션의 필요성에 대한 논쟁이 일어났습니다. 수십 개의 스타트업이 벡터 데이터베이스 분야에 뛰어들었습니다. 많은 전통적인 관계형 데이터베이스와 NoSQL 데이터베이스가 벡터를 중요한 데이터 유형으로 취급하기 시작했고, 많은 업체들이 모든 상황에서 전문화된 벡터 데이터베이스로 대체할 수 있다고 주장하고 있습니다.</p>

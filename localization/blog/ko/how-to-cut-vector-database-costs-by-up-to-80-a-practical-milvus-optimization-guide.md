...
<p>이 가이드에서는 실제로 비용이 어디로 가는지, 그리고 이 비용을 60~80%까지 낮추기 위해 할 수 있는 구체적인 방법을 안내해 드리겠습니다. 가장 널리 사용되는 오픈 소스 벡터 데이터베이스인 <a href="https://milvus.io/">Milvus를</a> 주요 예로 사용하지만, 제가 가장 잘 알고 있는 것이기 때문에 대부분의 벡터 데이터베이스에 원칙이 적용됩니다.</p>
<p><em>명확히 말씀드리자면</em> <em><a href="https://milvus.io/">Milvus</a></em> <em>자체는 무료 오픈 소스이므로 소프트웨어에 대한 비용을 지불할 필요가 없습니다. 비용은 전적으로 클라우드 인스턴스, 메모리, 스토리지, 네트워크 등 소프트웨어를 실행하는 인프라에서 발생합니다. 좋은 소식은 이러한 인프라 비용의 대부분을 절감할 수 있다는 것입니다.</em></p>

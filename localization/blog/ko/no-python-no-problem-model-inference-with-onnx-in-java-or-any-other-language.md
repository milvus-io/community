...
<h2 id="Exporting-NN-Models-from-Python" class="common-anchor-header">Python에서 NN 모델 내보내기<button data-href="#Exporting-NN-Models-from-Python" class="anchor-icon" translate="no">
    ...
    </button></h2><p>HuggingFace의 문장 변환기 라이브러리를 사용하여 Python에서 일반적인 텍스트 임베딩 모델( <code translate="no">all-MiniLM-L6-v2</code>)을 열어 보겠습니다. 트랜스포머 함수 뒤에 풀링 및 정규화 레이어를 내보내는 문장 트랜스포머를 둘러싼 래퍼가 필요하므로 .txtai의 util 라이브러리를 통해 간접적으로 HF 라이브러리를 사용할 것입니다. (이러한 레이어는 컨텍스트 종속 토큰 임베딩, 즉 트랜스포머의 출력을 가져와서 단일 텍스트 임베딩으로 변환합니다.)</p>

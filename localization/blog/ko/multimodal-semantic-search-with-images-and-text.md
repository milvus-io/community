...
<p>위 그림은 [사자 옆모습 이미지]와 "이것의 정면 모습"이라는 텍스트가 포함된 임베딩이 텍스트 없이 [사자 정면 이미지]만 포함된 임베딩과 비슷하다는 것을 보여줍니다. 텍스트와 이미지가 포함된 입력과 이미지만 포함된 입력(그리고 텍스트만 포함된 입력) 모두에 동일한 모델이 사용됩니다. <em>이러한 방식으로 모델은 쿼리 텍스트와 쿼리 이미지의 관계에 대한 사용자의 의도를 파악할 수 있습니다.</em></p>

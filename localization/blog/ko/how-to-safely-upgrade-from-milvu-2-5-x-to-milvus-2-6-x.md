...
<li><p><strong>JSON 파쇄를 통한 JSON 경로 색인</strong> - 이제 중첩된 JSON에 대한 구조화된 필터링이 훨씬 더 빨라지고 훨씬 더 예측 가능해졌습니다. 사전 색인된 JSON 경로는 필터 지연 시간을 <strong>140밀리초 → 1.5밀리초</strong> (P99: <strong>480밀리초 → 10밀리초</strong>)로 줄여 하이브리드 벡터 검색 + 메타데이터 필터링의 응답성을 크게 향상시킵니다.</p></li>

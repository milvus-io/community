...
---
<p>자연 검색 트래픽이 감소하는 것은 SEO가 나빠져서가 아닙니다. <a href="https://sparktoro.com/blog/in-2024-half-of-google-searches-end-without-a-click-to-another-web-property/">SparkToro에 따르면</a> 현재 Google 쿼리의 약 60%가 클릭 없이 끝나는데, 이는 사용자가 페이지를 클릭하는 대신 AI가 생성한 요약에서 답변을 얻는다는 의미입니다. 퍼플렉서티, ChatGPT 검색, Google AI 개요는 미래의 위협이 아닙니다. 이들은 이미 여러분의 트래픽을 잡아먹고 있습니다.</p>
...
<blockquote>
<p><strong>참고:</strong> 이 시스템은 실제 마케팅 워크플로우를 위해 구축된 작업 시스템이지만 코드는 시작점일 뿐입니다. 프롬프트, 점수 임계값 및 지식창고 구조를 자체 사용 사례에 맞게 조정해야 합니다.</p>
</blockquote>
...
<p>파이프라인은 두 단계로 실행됩니다. <strong>0단계에서는</strong> 소스 자료를 지식창고로 수집합니다. <strong>1단계에서는</strong> 이 자료에서 문서를 생성합니다.</p>
<p>
  <span class="img-wrapper">
...
<p><strong>다양한 요구 사항에 맞는 여러 배포 옵션.</strong></p>
<ul>
<li><p><a href="https://milvus.io/docs/milvus_lite.md"><strong>Milvus Lite</strong></a>: 노트북에서 한 줄의 코드만으로 실행되는 Milvus의 경량 버전으로, Docker가 필요하지 않습니다. 프로토타이핑에 적합하며 이 튜토리얼에 필요한 모든 것이 포함되어 있습니다.</p></li>
<li><p><a href="https://milvus.io/docs/install_standalone-docker.md"><strong>Milvus</strong></a> 독립형 및 Milvus 분산형: 프로덕션용으로 확장성이 뛰어난 버전입니다.</p></li>
<li><p><a href="https://cloud.zilliz.com/signup"><strong>질리즈 클라우드는</strong></a> 번거로움이 없는 관리형 Milvus입니다. 기술 설정 및 유지 관리에 대해 전혀 걱정할 필요가 없습니다. 무료 티어를 사용할 수 있습니다.</p></li>
</ul>
<p>이 튜토리얼에서는 Milvus Lite를 사용하므로 계정을 만들거나 <code translate="no">pip install pymilvus</code> 이상으로 설치하지 않아도 되며, 모든 것이 로컬에서 실행되므로 커밋하기 전에 전체 파이프라인을 사용해 볼 수 있습니다.</p>
<p>배포의 차이점은 URI에 있습니다:</p>
...
<h2 id="Step-by-Step-Tutorial" class="common-anchor-header">단계별 튜토리얼<button data-href="#Step-by-Step-Tutorial" class="anchor-icon" translate="no">
    ...
    </button></h2><p>전체 파이프라인은 <code translate="no">SKILL.MD</code> 인스트럭션 파일과 코드 구현이 포함된 디렉토리인 OpenClaw 스킬로 패키징됩니다.</p>
...
<p>거기서부터 채팅 메시지를 통해 파이프라인과 상호 작용하세요:</p>
...
<p><strong>예 2:</strong> 책(Wuthering Heights)을 업로드한 다음 3개의 GEO 문서를 생성하여 Lark 문서로 내보내기.</p>
<p>
  <span class="img-wrapper">
...
<h2 id="Taking-This-Pipeline-to-Production" class="common-anchor-header">이 파이프라인을 프로덕션에 적용하기<button data-href="#Taking-This-Pipeline-to-Production" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>이 튜토리얼의 모든 내용은 Milvus Lite에서 실행되므로 노트북에서 실행되고 노트북이 멈추면 중단됩니다. 실제 GEO 파이프라인의 경우 이것만으로는 충분하지 않습니다. 회의 중에도 문서가 생성되기를 원합니다. 다음 주 화요일에 동료가 배치를 실행할 때 지식창고를 사용할 수 있기를 원합니다.</p>
<p>이 시점에는 두 가지 솔루션이 있습니다.</p>
<p><strong>독립형 또는 분산 모드를 사용하여 Milvus를 자체 호스팅합니다.</strong> 엔지니어링 팀이 실제 컴퓨터 또는 AWS와 같은 클라우드 제공업체에서 대여한 전용 컴퓨터인 서버에 정식 버전을 설치합니다. 성능이 뛰어나고 배포를 완벽하게 제어할 수 있지만 설정, 유지 관리 및 확장을 위해 전담 엔지니어링 팀이 필요합니다.</p>
<p><a href="https://cloud.zilliz.com/signup"><strong>질리즈 클라우드</strong></a><strong>사용</strong><strong>.</strong> Zilliz Cloud는 동일한 팀이 구축한 완전 관리형 Milvus에 고급 엔터프라이즈급 기능을 더한 서비스입니다.</p>
<ul>
<li><p><strong>운영 및 유지 관리의 번거로움이 없습니다.</strong></p></li>
<li><p><strong>무료 티어 사용 가능.</strong> <a href="https://cloud.zilliz.com/signup">무료 티어에는</a> 5GB의 저장 공간이 포함되어 있으며, 이는 <em>Wuthering Heights를</em> 360회 또는 360권의 책을 모두 감상할 수 있는 용량입니다. 더 큰 워크로드를 위한 30일 무료 평가판도 있습니다.</p></li>
<li><p><strong>항상 새로운 기능을 가장 먼저 만나보세요.</strong> Milvus가 개선 사항을 릴리스하면 Zilliz Cloud에서 자동으로 제공되므로 팀이 업그레이드 일정을 잡을 때까지 기다릴 필요가 없습니다.</p></li>
</ul>
...
<button class="copy-code-btn"></button></code></pre>
<p><a href="https://cloud.zilliz.com/signup">질리즈 클라우드에 가입</a>하고 사용해 보세요.</p>
<h2 id="When-GEO-Content-Generation-Backfires" class="common-anchor-header">GEO 콘텐츠 생성이 역효과를 낼 때<button data-href="#When-GEO-Content-Generation-Backfires" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>GEO 콘텐츠 생성은 그 뒤에 있는 지식 기반과 함께 작동합니다. 이 접근 방식이 득보다 실이 많은 경우가 몇 가지 있습니다:</p>
<p><strong>공신력 있는 소스 자료가 없는 경우.</strong> 견고한 지식 기반이 없으면 LLM은 학습 데이터에 의존하게 됩니다. 결과는 기껏해야 일반적인 수준이고, 최악의 경우 환각에 가깝습니다. RAG 단계의 핵심은 검증된 정보를 기반으로 생성하는 것인데, 이 단계를 건너뛰면 추가 단계로 신속한 엔지니어링만 수행하게 됩니다.</p>
...
<p><strong>중복 제거 임계값에 대한 참고 사항.</strong> 파이프라인은 코사인 유사도가 0.85를 초과하는 쿼리를 삭제합니다. 중복에 가까운 쿼리가 너무 많이 통과하는 경우 이 임계값을 낮추세요. 파이프라인이 정말 다른 쿼리를 삭제하는 경우 이 임계값을 높이세요. 0.85는 합리적인 시작점이지만, 주제의 범위가 얼마나 좁은지에 따라 적절한 값이 달라집니다.</p>
<h2 id="Conclusion" class="common-anchor-header">결론<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>GEO는 10년 전의 SEO가 있었던 곳이며, 적절한 인프라를 갖추면 진정한 우위를 점할 수 있을 만큼 초기 단계입니다. 이 튜토리얼에서는 LLM 환각 대신 브랜드 고유의 소스 자료에 기반하여 AI 검색 엔진이 실제로 인용하는 문서를 생성하는 파이프라인을 구축합니다. 스택은 오케스트레이션을 위한 <a href="https://github.com/nicepkg/openclaw">OpenClaw</a>, 지식 저장 및 <a href="https://zilliz.com/learn/Retrieval-Augmented-Generation">RAG</a> 검색을 위한 <a href="https://milvus.io/intro">Milvus</a>, 생성 및 스코어링을 위한 LLM으로 구성됩니다.</p>
...
<li>인프라 설정을 건너뛰고 싶으시다면 <a href="https://cloud.zilliz.com/signup">Zilliz Cloud</a> (관리형 Milvus)의 무료 티어를 이용하세요. URI를 한 번만 변경하면 바로 프로덕션에 사용할 수 있습니다.</li>
</ul>
<hr>
...
<p><strong>GEO 콘텐츠는 일반 AI가 생성한 콘텐츠와 어떻게 다른가요?</strong>대부분의 AI가 생성한 콘텐츠는 일반적인 것으로, LLM은 학습 데이터에서 추출하여 합리적으로 들리지만 브랜드의 실제 사실, 주장 또는 데이터에 근거하지 않은 내용을 생성합니다. GEO 콘텐츠는 RAG(검색 증강 생성)를 사용하여 검증된 소스 자료의 지식 기반에 기반을 두고 있습니다. 막연한 일반화 대신 구체적인 제품 세부 정보를, 조작된 통계 대신 실제 수치를, 수십 개의 문서에 걸쳐 일관된 브랜드 목소리를 담는다는 점에서 그 차이가 드러납니다.</p>
...
<p><strong>AI 검색 엔진은 대량 생성된 콘텐츠에 불이익을 주지 않나요? 콘텐츠의</strong>품질이 낮으면 불이익을 받습니다. AI 검색 엔진은 출처가 불분명한 주장, 재활용된 문구, 독창적인 가치를 더하지 않는 콘텐츠를 감지하는 데 점점 더 능숙해지고 있습니다. 이 파이프라인에 근거를 마련하기 위한 지식 기반과 품질 관리를 위한 자체 채점 단계가 포함된 이유가 바로 여기에 있습니다. 더 많은 콘텐츠를 생성하는 것이 목표가 아니라 AI 모델이 인용할 수 있을 만큼 진정으로 유용한 콘텐츠를 생성하는 것이 목표입니다.</p>

...
<p>클러스터에 데이터를 삽입할 때는 <code translate="no">pymilvus</code> 을 사용하여 <code translate="no">pip install pymilvus</code> 으로만 SDK를 설치하거나, 리치 텍스트 임베딩을 추출하려는 경우 <code translate="no">PyMilvus Models</code> 을 설치하여 <code translate="no">pip install pymilvus[model]</code> 을 사용할 수도 있습니다.</p>

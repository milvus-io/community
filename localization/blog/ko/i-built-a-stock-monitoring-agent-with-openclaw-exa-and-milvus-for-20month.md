...
<p>그래서 저 대신 시장을 감시하고 같은 실수를 하지 않도록 막아주는 에이전트를 OpenClaw로 만들었습니다. 이 에이전트는 제 돈을 거래하거나 건드리지 않고 이미 한 번 후회했던 일을 반복하지 않도록 도와줍니다.</p>
<p>이 에이전트는 세 부분으로 구성되며 월 20달러 정도입니다:</p>
<ul>
<li><strong>이 모든 것을 자동 조종으로 실행하는</strong><strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">OpenClaw</a></strong> <strong>.</strong> OpenClaw는 30분 간격으로 에이전트를 실행하고 실제로 중요한 일이 있을 때만 핑을 보내주므로 화면에 계속 붙어 있던 FOMO를 완화해줍니다. 이전에는 가격을 더 많이 볼수록 충동적으로 반응했습니다.</li>
...
<h2 id="Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="common-anchor-header">1단계: Exa로 실시간 시장 정보 얻기<button data-href="#Step-1-Get-Real-Time-Market-Intelligence-with-Exa" class="anchor-icon" translate="no">
    ...
    </button></h2><p>전에는 금융 앱을 검색하고, 스크레이퍼를 작성하고, 전문 데이터 단말기를 살펴보기도 했습니다. 앱은 노이즈에 신호를 묻어버리고, 스크레이퍼는 계속 고장 나고, 전문 API는 기관 트레이더를 위한 가격이 책정되어 있었습니다. Exa는 위와 같은 문제를 해결하는 AI 에이전트용 검색 API입니다.</p>
<p>
...
<p><strong><a href="https://exa.ai/">Exa는</a></strong> 구조화된 AI 지원 데이터를 반환하는 웹 검색 API입니다. 밀버스의 완전 관리형 서비스인 <a href="https://zilliz.com/cloud">질리즈 클라우드에</a> 의해 구동됩니다.  퍼플렉시티가 사람이 사용하는 검색 엔진이라면, 엑사는 AI가 사용하는 검색 엔진입니다. 에이전트가 쿼리를 보내면 Exa는 기사 텍스트, 핵심 문장, 요약을 JSON이라는 구조화된 출력으로 반환하여 에이전트가 스크래핑 없이도 직접 파싱하고 작업할 수 있습니다.</p>
...
<h2 id="Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="common-anchor-header">2단계: 밀버스에 거래 내역을 저장하여 더 현명한 의사 결정하기<button data-href="#Step-2-Store-Trading-History-in-Milvus-for-Smarter-Decisions" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Exa는 제 정보 문제를 해결해 주었습니다. 하지만 저는 여전히 같은 거래를 반복하고 있었습니다. 하락장에서 패닉 매도를 했다가 며칠 만에 회복하고, 이미 고가인 주식에 모멘텀을 쫓는 식이죠. 감정에 따라 행동하고, 후회하고, 비슷한 상황이 닥쳤을 때쯤이면 교훈을 잊어버리곤 했죠.</p>
<p>저는 과거의 거래, 추론, 실수 등 개인적인 지식 기반이 필요했습니다. 제가 수동으로 검토해야 하는 것이 아니라(그런 시도를 해봤지만 번번이 실패했죠) 비슷한 상황이 발생할 때마다 상담원이 스스로 검색할 수 있는 자료가 필요했습니다. 실수를 반복할 것 같으면 버튼을 누르기 전에 상담원에게 미리 알려주었으면 좋겠어요. '현재 상황'과 '과거 경험'을 일치시키는 것은 벡터 데이터베이스가 해결하는 유사성 검색 문제이므로 모든 데이터를 하나에 저장해야 했습니다.</p>
<p>저는 서버 설정 없이 로컬에서 실행되는 Milvus의 경량 버전인 Milvus <a href="https://github.com/milvus-io/milvus-lite">Lite를</a> 사용했습니다. 프로토타이핑과 실험에 적합합니다. 저는 데이터를 세 개의 컬렉션으로 나누었습니다. 임베딩 차원은 1536으로 OpenAI의 텍스트 임베딩 3-소형 모델과 일치하여 바로 사용할 수 있습니다:</p>
...
<p>이를 하나의 컬렉션으로 통합하면 관련 없는 거래 내역으로 모든 프롬프트가 부풀어 오르거나 현재 쿼리와 충분히 일치하지 않을 때 핵심 편향이 손실될 수 있습니다.</p>
<p>일단 컬렉션이 존재하면 에이전트는 이를 자동으로 채워야 합니다. 모든 대화가 끝날 때마다 메모를 작성하고 싶지 않았기 때문에 각 채팅 세션이 끝날 때마다 실행되는 메모리 추출기를 만들었습니다.</p>
<p>이 추출기는 추출과 중복 제거라는 두 가지 작업을 수행합니다. 추출기는 대화에서 의사 결정, 선호도, 패턴, 교훈과 같은 구조화된 인사이트를 추출하도록 LLM에 요청하고 각 인사이트를 적절한 컬렉션으로 라우팅합니다. 무엇이든 저장하기 전에 이미 있는 것과 유사성을 확인합니다. 새로운 인사이트가 기존 항목과 92% 이상 유사하면 건너뛰게 됩니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>예를 들어 3월 초 중동 긴장으로 기술주가 3~4% 하락했을 때 에이전트는 지정학적 하락기에 패닉 매도하지 말라는 2024년 10월의 교훈, 지정학적 리스크에 비중을 두는 경향이 있다는 선호도 메모, 지정학으로 인한 기술주 매도는 일반적으로 1~3주 안에 회복된다는 제가 기록한 패턴 등 세 가지를 가져왔습니다.</p>
<p>제 동료는 이를 "개인 투자자를 위한 강화 학습"이라고 불렀는데, 저는 제 자신의 손실 함수를 학습하는 모델입니다. 그럴 만하죠. 적어도 이제 제가 한 일과 앞으로 할 일 사이에 피드백 루프가 생겼으니까요.</p>
<h2 id="Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="common-anchor-header">3단계: 에이전트에게 OpenClaw 스킬로 분석 교육하기<button data-href="#Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="anchor-icon" translate="no">
    ...
    </button></h2><p>이 시점에서 에이전트는 좋은 정보<a href="https://exa.ai/">(Exa</a>)와 개인 기억<a href="https://github.com/milvus-io/milvus-lite">(Milvus)</a>을 가지고 있습니다. 하지만 이 두 가지를 모두 LLM에게 넘겨주고 "이걸 분석해줘"라고 하면 일반적인 모든 것을 헤지하는 응답이 돌아옵니다. 가능한 모든 각도를 언급하고 "투자자는 위험을 고려해야 한다"는 결론, 즉 아무 것도 언급하지 않습니다.</p>
<p>해결 방법은 자신만의 분석 프레임워크를 작성하여 상담원에게 명시적인 지침으로 제공하는 것입니다. 어떤 지표를 중요하게 생각하는지, 어떤 상황을 위험하다고 생각하는지, 언제 보수적으로 대응해야 하는지, 언제 공격적으로 대응해야 하는지 등입니다. 이러한 규칙은 투자자마다 다르므로 직접 정의해야 합니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>마지막 줄이 가장 중요합니다. "항상 과거의 실수를 드러내라. 저는 두려움이 데이터보다 우선하는 경향이 있습니다. 밀버스 기록에 따르면 하락 후 매도한 것을 후회한 적이 있다면 솔직하게 말하세요." 이것은 제 심리를 위해 특별히 고안된 수정 메커니즘입니다. 사용자 버전은 자신의 성향을 인코딩할 수 있습니다.</p>
<p>저는 감정 분석, 매크로 지표, 섹터 회전 신호에 대해서도 비슷한 스킬을 만들었습니다. 또한 제가 존경하는 투자자들이 같은 상황을 어떻게 평가할지 시뮬레이션하는 스킬(버핏의 가치 프레임워크, 브리지워터의 거시적 접근법)도 만들었습니다. 이는 의사 결정자가 아니라 추가적인 관점입니다.</p>
...
<h2 id="Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="common-anchor-header">4단계: OpenClaw 하트비트로 에이전트 시작하기<button data-href="#Step-4-Start-Your-Agent-with-OpenClaw-Heartbeat" class="anchor-icon" translate="no">
    ...
    </button></h2><p>위의 모든 작업은 여전히 수동으로 트리거해야 합니다. 업데이트를 원할 때마다 터미널을 열어야 한다면 회의 중에 중개 앱을 다시 스크롤해야 하는 원점으로 돌아가게 됩니다.</p>
<p>OpenClaw의 하트비트 메커니즘은 이 문제를 해결합니다. 게이트웨이가 30분마다(설정 가능) 에이전트를 핑하고, 에이전트는 HEARTBEAT.md 파일을 확인하여 그 순간에 수행할 작업을 결정합니다. 이 파일은 시간 기반 규칙이 있는 마크다운 파일입니다:</p>
...
<h2 id="Conclusion" class="common-anchor-header">결론<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>저는 정보가 좋지 않아서 충동적인 거래를 계속했고, 내 기록을 거의 검토하지 않았으며, 하루 종일 시장을 쳐다보는 것이 상황을 악화시켰습니다. 그래서 저는 다음 세 가지를 통해 이러한 문제를 해결하는 AI 에이전트를 만들었습니다:</p>
<ul>
<li><strong><a href="https://exa.ai/">Exa를</a></strong> <strong>사용하여</strong><strong>신뢰할 수 있는 시장 뉴스를 자동으로 수집하므로</strong> 매일 아침 한 시간의 둠스 스크롤 대신 깔끔한 브리핑으로 하루를 시작합니다.</li>
<li><strong> <a href="https://milvus.io/">Milvus를</a></strong> 개인 벡터 데이터베이스로 사용하여<strong>과거에 거래한 내 거래 내</strong> 역을<strong>기억하고</strong> 실수를 반복할 때<strong>경고해</strong> 줍니다.</li>
<li><strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">오픈클로</a></strong> <strong>자동 조종 기능으로 실행되며</strong>, 크론 작업이나 글루 코드 없이 오픈클로 스킬과 하트비트를 사용하여 <strong>중요할 때만 핑을</strong> 보냅니다.</li>
</ul>
<p>총 비용: 월 $20. 에이전트는 제 돈을 거래하거나 건드리지 않습니다.</p>
<p>가장 큰 변화는 데이터나 알림이 아니었습니다. 제가 시장을 보지 않게 된 것이었습니다. 지난 수요일에는 완전히 잊어버렸는데, 수년간 트레이딩을 하면서 이런 일은 처음이었습니다. 여전히 가끔 손실을 보지만 그 빈도가 훨씬 줄어들었고 주말을 다시 즐기게 되었습니다. 동료들은 아직 농담을 업데이트하지 않았지만 시간을 두고 지켜봐 주세요.</p>

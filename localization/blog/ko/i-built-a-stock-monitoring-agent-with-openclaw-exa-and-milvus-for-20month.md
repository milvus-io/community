...
<ul>
<li><strong>이 모든 것을 자동 조종으로 실행하는</strong><strong><a href="https://milvus.io/blog/openclaw-formerly-clawdbot-moltbot-explained-a-complete-guide-to-the-autonomous-ai-agent.md">OpenClaw</a></strong> <strong>.</strong> OpenClaw는 30분 간격으로 에이전트를 실행하고 실제로 중요한 일이 있을 때만 핑을 보내주므로 화면에 계속 붙어 있던 FOMO를 완화해 줍니다. 전에는 가격을 더 많이 볼수록 충동적으로 반응했습니다.</li>
...
<button class="copy-code-btn"></button></code></pre>
<p>저는 이러한 패턴을 사용하여 연준 정책, 기술 기업 실적, 유가, 거시 지표를 포함하는 약 12개의 템플릿을 작성했습니다. 매일 아침 자동으로 실행되고 결과를 제 휴대폰으로 푸시합니다. 예전에는 한 시간이 걸리던 검색이 이제는 커피를 마시며 5분이면 요약을 읽을 수 있습니다.</p>
...
<p>저는 과거의 거래, 추론, 실수를 저장할 수 있는 개인 지식 기반이 필요했습니다. 제가 수동으로 검토해야 하는 것이 아니라(그렇게 하려고 시도해봤지만 실패했습니다), 비슷한 상황이 발생할 때마다 상담원이 스스로 검색할 수 있는 것이었습니다. 실수를 반복할 것 같으면 버튼을 누르기 전에 상담원에게 미리 알려주었으면 좋겠어요. '현재 상황'과 '과거 경험'을 일치시키는 것은 벡터 데이터베이스가 해결하는 유사성 검색 문제이므로 저는 데이터를 저장하기 위해 벡터 데이터베이스를 선택했습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>새로운 시장 상황을 접하고 거래하고 싶은 충동이 생기면 에이전트는 리콜 기능을 실행합니다. 제가 무슨 일이 일어나고 있는지 설명하면 세 가지 컬렉션 모두에서 관련 기록을 검색합니다:</p>
...
<h2 id="Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="common-anchor-header">3단계: 에이전트에게 OpenClaw 스킬로 분석 교육하기<button data-href="#Step-3-Teach-Your-Agent-to-Analyze-with-OpenClaw-Skills" class="anchor-icon" translate="no">
    ...
    </button></h2><p>이 시점에서 에이전트는 신뢰할 수 있는 정보<a href="https://exa.ai/">(Exa</a>)와 개인 기억<a href="https://github.com/milvus-io/milvus-lite">(Milvus)</a>을 가지고 있습니다. 하지만 이 두 가지를 모두 LLM에게 넘겨주고 "분석해줘"라고 하면 일반적인 모든 것을 헤지하는 응답을 받게 됩니다. 가능한 모든 각도를 언급하고 "투자자는 위험을 고려해야 합니다."라고 결론을 내립니다. 차라리 아무 말도 하지 않는 것이 나을 수도 있습니다.</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>마지막 줄이 가장 중요합니다. "항상 과거의 실수를 드러내라. 저는 두려움이 데이터보다 우선하는 경향이 있습니다. 밀버스 기록에 따르면 하락 후 매도한 것을 후회한 적이 있다면 솔직하게 말하세요." 이렇게 하면 상담원에게 제가 어디에서 잘못했는지 정확히 알려주므로 언제 뒤로 물러나야 하는지 알 수 있습니다. 직접 작성한다면 이 부분은 자신의 편견에 따라 사용자 지정할 수 있습니다.</p>
...
<li><strong>주간 검토(주말).</strong> 에이전트가 시장 움직임, 아침 예상과 비교한 결과, 기억해야 할 패턴 등 한 주간의 모든 것을 정리해 줍니다. 저는 토요일에 30분 동안 이 자료를 읽습니다. 나머지 주중에는 일부러 화면을 멀리합니다.</li>
</ul>
...
<p>정보의 질도 더 좋아졌습니다. 트위터에서 화제가 된 내용 대신 로이터와 블룸버그의 요약본을 읽고 있습니다. 그리고 제가 행동하고 싶은 유혹을 느낄 때마다 에이전트가 과거의 실수를 알려주니 충동적인 매매를 크게 줄였습니다. 아직 제가 더 나은 투자자가 되었다는 것을 증명할 수는 없지만 덜 무모한 투자자가 된 것은 분명합니다.</p>
...
<p>가장 큰 변화는 데이터나 알림이 아니었습니다. 제가 시장을 보지 않게 된 것입니다. 지난 수요일에는 완전히 잊어버렸는데, 수년간 트레이딩을 하면서 이런 일은 처음이었습니다. 여전히 가끔 손실을 보지만 그 빈도가 훨씬 줄었고 주말을 다시 즐기게 되었습니다. 동료들은 아직 농담을 업데이트하지 않았지만 시간을 두고 지켜봐 주세요.</p>

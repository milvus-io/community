...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Plaintext-Memory-Makes-Migration-Nearly-Effortless" class="common-anchor-header">마이그레이션을 거의 손쉽게 만들어주는 평문 메모리</h3><p>마이그레이션은 메모리 프레임워크의 가장 큰 숨겨진 비용 중 하나입니다. 한 도구에서 다른 도구로 이동하려면 일반적으로 데이터를 내보내고, 형식을 변환하고, 다시 가져오고, 필드가 호환되기를 바라야 합니다. 이러한 작업은 반나절은 쉽게 소모될 수 있으며, 결과를 보장할 수 없습니다.</p>
...
<p><strong>청킹은</strong> 의미론적 경계(제목과 본문)를 따라 텍스트를 분할하여 관련 콘텐츠가 함께 유지되도록 합니다. 이렇게 하면 "Redis 구성"과 같은 문구가 청크에 걸쳐 분할되는 경우를 방지할 수 있습니다.</p>
<p>예를 들어, 이 마크다운:</p>

...
<p>하지만 <code translate="no">text</code> 와 <code translate="no">sparse</code> 필드를 어떻게 연결하고 <code translate="no">sparse</code> 를 <code translate="no">text</code> 에서 어떻게 계산해야 하는지 Milvus에게 알려줄 수 있을까요? 바로 여기에서 <code translate="no">Function</code> 객체를 호출하고 스키마에 추가해야 합니다:</p>

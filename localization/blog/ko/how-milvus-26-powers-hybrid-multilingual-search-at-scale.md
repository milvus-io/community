...
<p>벡터 검색은 텍스트의 <em>의미론적</em> 의미를 포착하여 고차원 벡터로 변환하는 반면, 전체 텍스트 검색은 단어가 어떻게 형성되는지, 단어의 시작과 끝이 어디인지, 단어들이 서로 어떻게 연관되어 있는지 등 <strong>언어의 구조를</strong> 이해하는 데 달려 있습니다. 예를 들어, 사용자가 영어로 '운동화'를 검색하면 텍스트는 여러 처리 단계를 거칩니다:</p>

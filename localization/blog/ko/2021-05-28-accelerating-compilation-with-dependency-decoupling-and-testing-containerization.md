...
<li>적절한 컴파일 캐시 도구를 선택합니다: 코드를 통합 및 컴파일할 컨테이너를 입력하고 적절한 컴파일 캐시가 있는지 비공개 리포지토리에서 확인합니다. 있다면 다운로드하여 지정된 디렉터리에 압축을 풉니다. 모든 컴포넌트가 컴파일되면 컴파일 캐시 도구에서 생성된 캐시가 프로젝트 버전과 이미지 ID에 따라 패키징되어 비공개 리포지토리에 업로드됩니다.</li>
</ol>
<p><br/></p>

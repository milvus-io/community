...
<p>현대의 대규모 온라인 비즈니스에 서비스를 제공하기 위해 일반적으로 여러 복제본을 채택합니다. 예를 들어, 온라인 전자상거래 업체인 Amazon은 시스템 충돌이나 장애 발생 시 높은 시스템 가용성을 보장하기 위해 여러 데이터 센터, 지역, 심지어 국가에 걸쳐 주문 또는 SKU 데이터를 복제합니다. 이로 인해 여러 복제본에 걸친 데이터 일관성이라는 시스템 문제가 발생합니다. 일관성이 없으면 Amazon 카트에서 삭제된 품목이 다시 표시되어 사용자 경험이 매우 나빠질 가능성이 매우 높습니다.</p>

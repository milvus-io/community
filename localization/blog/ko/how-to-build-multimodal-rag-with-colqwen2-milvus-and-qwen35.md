...
<p>검색 증강 생성(RAG)은 색인 작업과 답변 작업을 분리하여 이 문제를 해결합니다. 문서를 한 번 인코딩하고 그 표현을 벡터 데이터베이스에 저장한 다음 쿼리 시 가장 관련성이 높은 페이지만 검색하여 LLM으로 전송합니다. 모델은 전체 말뭉치가 아니라 쿼리당 3페이지를 읽습니다. 따라서 계속 늘어나는 컬렉션에 대해 문서 Q&amp;A를 구축하는 것이 실용적입니다.</p>

...
---
<p><em>Этот блог написан Цзинхе Ма, участником сообщества Milvus</em> <em>, и публикуется здесь с разрешения.</em></p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Starting-Everything" class="common-anchor-header">Запуск всего</h3><p>Когда конфигурация готова, запустите все четыре службы:</p>
...
<p>Конвейер RAG работает, но запуск Python-скрипта каждый раз, когда вы хотите задать вопрос, вредит цели. Поэтому я обратился к искусственному интеллекту с просьбой предложить стек. Ответ: <strong>FastAPI</strong> для бэкенда - код RAG уже написан на Python, так что обернуть его в конечную точку FastAPI будет естественным решением - и <strong>Next.js</strong> для фронтенда. FastAPI раскрывает логику RAG как конечную точку HTTP; Next.js вызывает ее и отображает ответ в окне чата.</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Nextjs-Frontend" class="common-anchor-header">Next.js Frontend</h3><p>Фронтенд отправляет запрос пользователя в конечную точку FastAPI и отображает ответ. Основная логика получения запроса:</p>
<p>javascript</p>
...
<h2 id="Conclusion" class="common-anchor-header">Заключение<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>То, что начиналось как любопытство по поводу бэкенда хранилища Milvus, превратилось в полноценный рабочий чатбот RAG - и путь от одного к другому оказался короче, чем ожидалось. Вот что включает в себя сборка от конца до конца:</p>
<ul>
...
<li><strong>Кодирование в Vibe происходит быстро, вплоть до того момента, когда область применения переходит за рамки.</strong> Одно за другим - от Milvus до RAG, от чатбота до "может, мне стоит все это докеризировать?". Требования не сходятся сами по себе.</li>

...
<h2 id="Feature-extraction" class="common-anchor-header">Извлечение признаков<button data-href="#Feature-extraction" class="anchor-icon" translate="no">
    ...
    </button></h2><p>В области компьютерного зрения использование искусственного интеллекта стало мейнстримом. Так и в системе поиска по изображению второго поколения для извлечения признаков используется конволюционная нейронная сеть (CNN).</p>

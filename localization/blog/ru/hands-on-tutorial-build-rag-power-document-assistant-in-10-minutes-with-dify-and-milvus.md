...
<p>Готовы создать что-то действительно полезное в рекордные сроки? Давайте начнем!</p>
...
<p>В области "Настройка чанка" вы можете выбрать любое число в качестве максимальной длины чанка (в нашем случае мы установим ее на 100). Далее, в разделе "Метод индексации" нам нужно выбрать опцию "Высокое качество", так как она позволит нам выполнять поиск по сходству для нахождения релевантных контекстов. Для "Модели встраивания" вы можете выбрать любую модель встраивания из OpenAI, но в данном примере мы будем использовать модель text-embedding-3-small. Наконец, для "Retrieval Setting" нам нужно выбрать "Vector Search", так как мы хотим выполнять поиск по сходству, чтобы найти наиболее релевантные контексты.</p>
<p>
  <span class="img-wrapper">

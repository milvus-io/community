...
<h3 id="Step-2-Pruning-the-Graph-with-2-Hop-Detours" class="common-anchor-header">Шаг 2: обрезка графа с помощью 2-ходовых обходов</h3><p>После завершения работы NN-Descent полученный граф является точным, но слишком плотным. NN-Descent намеренно сохраняет лишних соседей-кандидатов, а случайная фаза инициализации вносит много слабых или нерелевантных ребер. В результате каждый узел часто оказывается со степенью, в два раза или даже в несколько раз превышающей целевую степень.</p>
...
<p>Обрезка интегрирована непосредственно в процесс построения с помощью критерия α-RNG: если сосед-кандидат <em>v</em> уже покрыт существующим соседом <em>p′</em> (т. е. <em>d(p′, v) &lt; α × d(p, v)</em>), то <em>v</em> обрезается. Параметр α позволяет точно контролировать разреженность и точность. Ускорение GPU достигается за счет пакетного параллелизма и геометрического пакетного масштабирования, что позволяет найти баланс между качеством индекса и пропускной способностью.</p>

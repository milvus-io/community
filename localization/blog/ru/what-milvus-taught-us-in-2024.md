...
<h3 id="How-do-I-measure-performance" class="common-anchor-header">Как измерить производительность?</h3><p>Пользователи хотят проверить показатели, связанные с производительностью их развертывания, чтобы они могли понять и устранить узкие места. К таким показателям относятся средняя задержка запросов, распределение задержек, объем запросов, использование памяти, дискового пространства и так далее. В то время как получение этих показателей с помощью <a href="https://milvus.io/docs/monitor_overview.md">старой системы мониторинга</a> было непростой задачей, Milvus 2.5 представляет новую систему под названием <a href="https://milvus.io/docs/milvus-webui.md#Milvus-WebUI">WebUI</a> (отзывы приветствуются!), которая позволяет получить доступ ко всей этой информации с помощью удобного веб-интерфейса.</p>
<h3 id="What’s-happening-inside-Milvus-right-now-ie-observe-state" class="common-anchor-header">Что происходит в Milvus прямо сейчас (т.е. наблюдение за состоянием)?</h3><p>В связи с этим пользователи хотят наблюдать за внутренним состоянием своего развертывания. К ним относятся вопросы о том, почему поисковый индекс создается так долго, как определить, здоров ли кластер, и как выполняется запрос на узлах. На многие из этих вопросов можно ответить с помощью нового <a href="https://milvus.io/docs/milvus-webui.md#Milvus-WebUI">WebUI</a>, который обеспечивает прозрачность внутренних действий системы.</p>
...
<p>Пролистав большое количество постов с 2024 года, я задумался: почему это должен делать человек? Разве генеративный ИИ не обещал решить подобную задачу по обработке больших объемов текста и извлечению информации? Присоединяйтесь ко мне во второй части этой статьи (скоро будет), где я исследую разработку и реализацию <em>мультиагентной системы для извлечения инсайтов из дискуссионных форумов.</em></p>

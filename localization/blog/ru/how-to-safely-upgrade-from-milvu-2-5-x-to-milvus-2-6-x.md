...
<p>Такое смешение логики пакетного и реального времени затрудняло независимое масштабирование пакетных рабочих нагрузок. Кроме того, состояние потоковых данных было разбросано по нескольким компонентам, что вносило задержки в синхронизацию, усложняло восстановление после сбоев и повышало сложность эксплуатации.</p>
...
<p>Обновление узла запросов обычно занимает больше времени. На этом этапе узлы данных и индексные узлы Milvus 2.5 могут продолжать выполнять такие операции, как промывка и построение индексов, помогая снизить нагрузку на сторону запроса, пока узлы запроса обновляются.</p>
<p><strong>4. Обновление узла данных</strong></p>
<p>Когда узлы данных Milvus 2.5 будут переведены в автономный режим, операции Flush станут недоступны, а данные в растущих сегментах могут продолжать накапливаться до тех пор, пока все узлы не будут полностью обновлены до Milvus 2.6.</p>

...
<p>Предметы одежды на каждом изображении распознаются и вырезаются, чтобы служить в качестве входных данных модели извлечения признаков, используемых в последующей обработке. Одновременно модель обнаружения объектов предсказывает классификацию одежды в соответствии с заранее определенными классами (топы, верхняя одежда, брюки, юбки, платья и ромперы).</p>

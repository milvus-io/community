...
<p>В реальных приложениях очень часто встречаются условия фильтрации - например, "искать только векторы от определенного пользователя" или "ограничить результаты определенным временным диапазоном". Из-за различий в алгоритмах IVF обычно справляется с отфильтрованным поиском эффективнее, чем HNSW.</p>

...
<p>Это руководство поможет вам построить мультимодальный конвейер RAG с тремя компонентами, доступными по открытой лицензии:</p>
<ul>
...
<li><strong><a href="http://milvus.io">Milvus</a></strong> хранит эти векторы и обрабатывает поиск сходства во время запроса, извлекая только самые релевантные страницы.</li>
...
<p>Традиционный RAG извлекает текст из документов, вставляет его в виде векторов, извлекает ближайшие совпадения во время запроса и передает эти фрагменты текста в LLM. Это хорошо работает для контента с большим объемом текста и чистым форматированием. Он ломается, когда ваши документы содержат:</p>
<ul>
...
<tr><td>База данных векторов</td><td><a href="https://milvus.io/">Milvus Lite</a></td><td>Хранит векторы патчей и обрабатывает поиск сходства; работает локально без настройки сервера</td></tr>
...
<h2 id="Step-by-Step-Implementation-for-Multi-Modal-RAG-with-ColQwen2+-Milvus+-Qwen35-397B-A17B" class="common-anchor-header">Пошаговая реализация мультимодальной RAG с помощью ColQwen2+ Milvus+ Qwen3.5-397B-A17B<button data-href="#Step-by-Step-Implementation-for-Multi-Modal-RAG-with-ColQwen2+-Milvus+-Qwen35-397B-A17B" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p><strong>Агрегирование по страницам</strong> с помощью MaxSim: для каждого токена запроса берется патч с наибольшим количеством баллов на каждой странице, затем эти баллы суммируются по всем токенам. Страница с наибольшей суммарной оценкой является лучшим совпадением.</p>
...
<p>У вас есть вопросы или вы хотите похвастаться тем, что создали? <a href="https://milvus.io/slack">Milvus Slack</a> - это самый быстрый способ получить помощь от сообщества и команды. Если вы предпочитаете беседу один на один, вы можете заказать время в наших <a href="https://meetings.hubspot.com/chloe-williams1/milvus-meeting">офисных часах</a>.</p>

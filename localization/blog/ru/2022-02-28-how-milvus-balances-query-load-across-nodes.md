...
<p>Чтобы избежать подобных ситуаций, координатор запросов (query coord) запрограммирован на равномерное распределение сегментов между узлами запросов в соответствии с использованием оперативной памяти этих узлов. Таким образом, ресурсы ЦП расходуются одинаково на всех узлах, что значительно повышает производительность поиска.</p>

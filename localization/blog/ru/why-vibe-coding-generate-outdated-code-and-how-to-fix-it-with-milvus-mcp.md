...
desc: >-
  ...
  показывает, как специализированные серверы MCP могут решить эту проблему,
...
<h2 id="Meet-Milvus-MCP-Vibe-Coding-with-Always-Up-to-Date-Docs" class="common-anchor-header">Встречайте Milvus MCP: Vibe Coding с постоянно обновляемыми документами<button data-href="#Meet-Milvus-MCP-Vibe-Coding-with-Always-Up-to-Date-Docs" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<li><p>Server-Sent Events (SSE): SSE поддерживает потоковую передачу данных от сервера к клиенту, используя HTTP POST-запросы для связи клиента с сервером.</p></li>
</ul>
...
<p>Режим агента в Cursor позволяет использовать веб-поиск, вызовы MCP и переключать плагины. Несмотря на свою мощь, он иногда непоследователен. Вы можете использовать <code translate="no">@</code> для ручной вставки документов, но для этого нужно сначала найти и прикрепить содержимое.</p>

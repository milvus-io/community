...
<li><p><strong>Запись выделения сегмента и времени истечения срока действия выделенного пространства в сегменте</strong>Пространство в каждом сегменте, выделенное коордом данных, не является постоянным, поэтому коорду данных также необходимо вести запись времени истечения срока действия каждого выделенного сегмента.</p></li>
...
<p>Затем координатор данных возвращает информацию о выделенном сегменте (включая идентификатор сегмента, количество строк, время истечения <code translate="no">t2</code>, и т.д.) прокси-серверу. Прокси отправляет информацию о выделенном сегменте в хранилище сообщений, чтобы эта информация была правильно записана. Обратите внимание, что значение <code translate="no">t1</code> должно быть меньше значения <code translate="no">t2</code>. По умолчанию значение <code translate="no">t2</code> равно 2 000 миллисекунд, и его можно изменить, настроив параметр <code translate="no">segment.assignmentExpiration</code> в файле <code translate="no">data_coord.yaml</code>.</p>

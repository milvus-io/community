...
title: >-
  Сначала встраивание, потом расчленение: Более эффективный поиск по RAG с
  помощью Max-Min семантического расчленения
author: Rachel Liu
...
<p><a href="https://link.springer.com/article/10.1007/s10791-025-09638-7"><strong>Max-Min Semantic Chunking</strong></a> подходит к проблеме по-другому. Вместо того чтобы сначала разбивать текст на куски, он встраивает их в текст и использует семантическое сходство, чтобы решить, где должны быть границы. Благодаря встраиванию перед разрезанием, конвейер может отслеживать естественные изменения смысла, а не полагаться на произвольные ограничения по длине.</p>
<p>В нашем предыдущем блоге мы рассказывали о таких методах, как <a href="https://milvus.io/blog/smarter-retrieval-for-rag-late-chunking-with-jina-embeddings-v2-and-milvus.md"><strong>Late Chunking</strong></a> от Jina AI, которые помогли популяризировать идею "встраивания сначала" и показали, что она может работать на практике. <strong>Max-Min Semantic Chunking</strong> опирается на ту же концепцию с помощью простого правила, которое определяет, когда смысл меняется настолько, что требует нового фрагмента. В этом посте мы расскажем о том, как работает Max-Min, и рассмотрим его сильные стороны и ограничения для реальных рабочих нагрузок RAG.</p>
<h2 id="How-a-Typical-RAG-Pipeline-Works" class="common-anchor-header">Как работает типичный конвейер RAG<button data-href="#How-a-Typical-RAG-Pipeline-Works" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>Этот метод немного умнее. Он разбивает текст на иерархические части, основываясь на таких признаках, как абзацы, переносы строк или предложения. Если раздел слишком длинный, он рекурсивно делит его дальше. В целом результат получается более последовательным, но все равно непоследовательным. Некоторые документы не имеют четкой структуры или имеют неравномерную длину разделов, что снижает точность поиска. А в некоторых случаях этот подход все равно выдает фрагменты, которые выходят за пределы контекстного окна модели.</p>
<p>Оба метода сталкиваются с одним и тем же компромиссом: точность против контекста. Меньшие фрагменты повышают точность поиска, но теряют окружающий контекст; большие фрагменты сохраняют смысл, но рискуют добавить нерелевантный шум. Нахождение правильного баланса - это то, что делает куски одновременно основополагающими и разочаровывающими при разработке систем RAG.</p>
<h2 id="Max–Min-Semantic-Chunking-Embed-First-Chunk-Second" class="common-anchor-header">Максимально-минимальный семантический чанкинг: Сначала встраиваем, потом разбиваем<button data-href="#Max–Min-Semantic-Chunking-Embed-First-Chunk-Second" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<h3 id="5-Adjust-thresholds-as-the-document-changes" class="common-anchor-header">5. Регулировка пороговых значений по мере изменения документа</h3><p>Для оптимизации качества чанков такие параметры, как размер чанка и пороги сходства, могут быть динамически скорректированы. Это позволяет алгоритму адаптироваться к изменяющимся структурам документов и семантической плотности.</p>
...
<p>Эти параметры автоматически настраиваются в зависимости от контекста - для сохранения связности большие куски требуют более жестких порогов сходства.</p>
...
<p>Одна из распространенных проблем - <strong>фрагментация контекста</strong>. Когда важная информация распределена по разным частям документа, алгоритм может поместить эти части в отдельные куски. При этом каждый кусок несет в себе только часть смысла.</p>
...
<h2 id="Conclusion" class="common-anchor-header">Заключение<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Max-Min Semantic Chunking не является волшебным решением всех проблем RAG, но он дает нам более разумный способ думать о границах фрагментов. Вместо того чтобы позволять ограничениям на лексемы решать, где идеи будут измельчаться, он использует вкрапления, чтобы определить, где смысл действительно меняется. Для многих реальных документов - API, спецификаций, журналов, инструкций по выпуску, руководств по устранению неполадок - только это может заметно повысить качество поиска.</p>

...
<h3 id="3-Search-Hybrid-Vector-+-BM25-Retrieval-for-Maximum-Accuracy" class="common-anchor-header">3. Поиск: Гибридный векторный + BM25-поиск для максимальной точности</h3><p>Для поиска используется гибридный подход: векторный поиск с весом 70 % и поиск по ключевым словам BM25 с весом 30 %. Это позволяет сбалансировать две различные потребности, которые часто возникают на практике.</p>
<ul>

...
<p>В Milvus 2.1 используется новый протокол маршрутизации, который больше не полагается на очереди сообщений в канале поиска, что значительно сокращает время поиска для небольших наборов данных. Результаты наших тестов показывают, что теперь Milvus снижает уровень задержки до 5 мс, что соответствует требованиям критически важных онлайн-соединений, таких как поиск по сходству и рекомендации.</p>

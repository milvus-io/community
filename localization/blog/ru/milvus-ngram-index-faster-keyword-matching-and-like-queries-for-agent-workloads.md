...
<h2 id="Evaluating-Ngram-Index-Performance-on-LIKE-Queries" class="common-anchor-header">Оценка производительности индекса Ngram на LIKE-запросах<button data-href="#Evaluating-Ngram-Index-Performance-on-LIKE-Queries" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Цель данного бенчмарка - оценить, насколько эффективно индекс Ngram ускоряет выполнение запросов <code translate="no">LIKE</code> на практике.</p>
...
<p>Результаты бенчмарков выявляют четкую закономерность: индекс Ngram значительно ускоряет выполнение запросов LIKE во всех случаях, причем скорость выполнения запросов сильно зависит от структуры и длины исходных текстовых данных.</p>
<ul>

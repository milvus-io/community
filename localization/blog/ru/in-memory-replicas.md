...
<h2 id="Why-are-in-memory-replicas-important" class="common-anchor-header">Почему важны репликации in-memory?<button data-href="#Why-are-in-memory-replicas-important" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Одним из наиболее значительных преимуществ использования реплик in-memory является увеличение общего QPS (запросов в секунду) и пропускной способности. Кроме того, можно поддерживать несколько сегментных реплик, и система становится более устойчивой к сбоям.</p>

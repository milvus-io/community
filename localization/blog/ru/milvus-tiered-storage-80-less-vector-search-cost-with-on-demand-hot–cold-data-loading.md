...
---
<p><strong>Многие ли из вас до сих пор оплачивают счета за инфраструктуру за данные, к которым ваша система едва прикасается? Скажите честно - большинство команд.</strong></p>
<p>Если вы используете векторный поиск в производстве, вы, вероятно, видели это воочию. Вы выделяете большие объемы памяти и SSD, чтобы все оставалось "готовым к работе с запросами", хотя на самом деле активна лишь малая часть вашего набора данных. И вы не одиноки. Мы видели много подобных случаев:</p>
<ul>
...
<h2 id="Why-Full-Loading-Breaks-Down-at-Scale" class="common-anchor-header">Почему полная загрузка разрушается при масштабировании<button data-href="#Why-Full-Loading-Breaks-Down-at-Scale" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Прежде чем погрузиться в решение, стоит подробнее рассмотреть, почему <strong>режим полной загрузки</strong>, использовавшийся в Milvus 2.5 и более ранних версиях, стал ограничивающим фактором при масштабировании рабочих нагрузок.</p>
...
<table>
<thead>
<tr><th style="text-align:center"><strong>Тип поискового запроса</strong></th><th style="text-align:center"><strong>Задержка Milvus 2.5 P99</strong></th><th style="text-align:center"><strong>Milvus 2.6+ P99 latency</strong></th><th style="text-align:center"><strong>Изменение</strong></th></tr>
</thead>
<tbody>
<tr><td style="text-align:center">Поиск горячих данных</td><td style="text-align:center">15 мс</td><td style="text-align:center">16 мс</td><td style="text-align:center">+6.7%</td></tr>
<tr><td style="text-align:center">Поиск теплых данных</td><td style="text-align:center">15 мс</td><td style="text-align:center">28 мс</td><td style="text-align:center">+86%</td></tr>
<tr><td style="text-align:center">Поиск холодных данных (первый доступ)</td><td style="text-align:center">15 мс</td><td style="text-align:center">120 мс</td><td style="text-align:center">+700%</td></tr>
<tr><td style="text-align:center">Поиск холодных данных (кэшированных)</td><td style="text-align:center">15 мс</td><td style="text-align:center">18 мс</td><td style="text-align:center">+20%</td></tr>
</tbody>
</table>
<blockquote>
<p><em>Под "запросом" здесь подразумеваются запросы векторного поиска, а не API Milvus <code translate="no">query()</code>.</em></p>
</blockquote>
...
<p>Запросы с холодными данными демонстрируют более высокую задержку при первом обращении из-за загрузки по требованию из объектного хранилища. Однако после кэширования их задержка увеличивается всего на 20 %. Учитывая низкую частоту доступа к холодным данным, этот компромисс в целом приемлем для большинства реальных рабочих нагрузок.</p>
...
<li><p><strong>Контролируемая задержка для холодных данных:</strong> При первом обращении к холодным данным возникает более высокая задержка, но это приемлемо, учитывая низкую частоту обращения к ним.</p></li>

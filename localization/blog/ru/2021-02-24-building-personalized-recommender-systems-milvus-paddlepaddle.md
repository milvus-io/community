...
<p>Для пользовательского признака отобразите идентификатор пользователя в векторное представление с размерностью 256, войдите в слой с полным подключением и проделайте аналогичную обработку для трех других признаков. Затем представления четырех признаков полностью соединяются и добавляются отдельно.</p>
...
<h3 id="Step-2Data-Preprocessing" class="common-anchor-header">Шаг 2：Предобработка данных</h3><pre><code translate="no"># Data preprocessing, -f followed by the parameter raw movie data file name

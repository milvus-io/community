...
<p>Близнецы склонны начинать с сильного вердикта ("это плохо" / "выглядит хорошо"), а затем работать в обратном направлении, чтобы обосновать его с точки зрения дизайна/структуры. Иногда это полезно. А иногда создается впечатление, что он сначала пролистал, а потом принял решение.</p>
...
<tr><td>Параллельные заезды</td><td>2</td><td>0</td><td>1</td><td>0</td><td>0</td><td>0</td></tr>
...
<p><strong>Qwen</strong> - самый хорошо подготовленный из пяти. Качество его обзоров соответствует качеству обзоров Клода, и он особенно хорош в объединении различных точек зрения в предложения по исправлению, которые вы можете реально использовать. Кроме того, у него самый высокий показатель обнаружения L2 в режиме контекстной помощи, что делает его надежным помощником по умолчанию для ежедневных PR-обзоров. Единственная слабость: в длинных многораундовых дебатах он иногда теряет контекст и начинает давать непоследовательные ответы в последующих раундах.</p>
<p><strong>MiniMax</strong> оказался самым слабым в самостоятельном поиске ошибок. Его лучше использовать для пополнения группы из нескольких моделей, а не в качестве отдельного рецензента.</p>
...
<p><strong>Модели по своей природе случайны.</strong> Запуск одного и того же запроса дважды может дать разные результаты. Приведенные в этом посте цифры - это единичный снимок, а не стабильное ожидаемое значение. К рейтингу отдельных моделей следует относиться с осторожностью, хотя общие тенденции (дебаты превосходят отдельные модели, разные модели лучше справляются с разными типами ошибок) совпадают.</p>
<p><strong>Порядок выступлений был исправлен.</strong> В дебатах использовался один и тот же порядок во всех раундах, что могло повлиять на реакцию моделей, выступающих позже. В будущем эксперименте можно было бы рандомизировать порядок выступлений в каждом раунде, чтобы проконтролировать это.</p>

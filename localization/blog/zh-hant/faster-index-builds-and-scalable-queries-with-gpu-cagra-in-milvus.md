...
<p>基於圖形的索引 (例如<strong>NSW、HNSW、CAGRA 和 Vamana)</strong>已經成為最廣泛採用的答案。透過瀏覽預先建立的鄰居圖，這些索引能夠以十億級的規模進行快速的最近鄰居搜尋，避免強制掃描和比較查詢的每個向量。</p>
...
<p>這兩種方法在設計目標和技術基礎上有顯著的差異，因此各自適用於不同的資料規模和工作負載模式。</p>
...
<p>每輪精煉之後，CAGRA 會應用額外的圖形剪枝技術 (例如<strong>2 跳迂迴剪枝)，以</strong>移除多餘的邊緣，同時保持搜尋品質。這種反覆精煉和剪枝的結合，可以產生<strong>緊湊但連接良好的圖形</strong>，在查詢時可以有效率地進行遍歷。</p>
...
<p><strong>4.收斂檢查：</strong>隨著迭代的進行，鄰居更新的次數會越來越少。一旦更新連線的數量低於設定的臨界值，演算法就會停止，表示圖形已經有效地穩定。</p>
...
<p>這個想法很簡單：如果節點<em>A</em>可以透過共用鄰居<em>C</em>間接抵達節點<em>B</em>(形成一條路徑 A → C → B)，而且這條間接路徑的距離與<em>A</em>和<em>B</em> 之間的直接距離相當，那麼直接邊 A → B 就被視為多餘，可以移除。</p>
<p>
...
<tr><td style="text-align:center"><strong>真</strong></td><td style="text-align:center"><strong>假</strong></td><td style="text-align:center">使用 GPU_CAGRA 建立 → 序列化為 HNSW → 反序列化為 HNSW →<strong>CPU 查詢</strong></td><td style="text-align:center">當發生參數不匹配時，後續查詢回落至 CPU</td></tr>
...
<h2 id="Putting-GPUCAGRA-to-the-Test" class="common-anchor-header">測試 GPU_CAGRA<button data-href="#Putting-GPUCAGRA-to-the-Test" class="anchor-icon" translate="no">
    ...
    </button></h2><p>為了評估混合執行模式 (使用 GPU 建構索引，CPU 執行查詢) 的效能，我們在標準化環境中進行了一系列受控實驗。評估著重於三個層面：<strong>索引建立效能</strong>、<strong>查詢效能</strong>以及<strong>回復準確度</strong>。</p>
...
<h3 id="1-Index-Build-Performance" class="common-anchor-header">1.索引建立效能</h3><p>在相同的目標圖形程度 64 下，我們比較在 GPU 上建立的 CAGRA 與在 CPU 上建立的 HNSW。</p>
<p>
  <span class="img-wrapper">
...
<li><p><strong>建立時間隨著 NN-Descent 複次的增加而線性增加。</strong>隨著迭代次數的增加，建構時間也以接近線性的方式成長，反映出 NN-Descent 的迭代精煉性質，並在建構成本與圖表品質之間提供可預測的權衡。</p></li>
</ul>
<h3 id="2-Query-performance" class="common-anchor-header">2.查詢效能</h3><p>在本實驗中，CAGRA 圖形在 GPU 上建立一次，然後透過兩種不同的執行路徑進行查詢：</p>
<ul>
<li><p><strong>CPU 查詢</strong>：索引被反序列化為 HNSW 格式，並在 CPU 上進行搜尋</p></li>
...
<h3 id="3-Recall-accuracy" class="common-anchor-header">3.召回準確度</h3><p>在這個實驗中，CAGRA 和 HNSW 都在 CPU 上進行查詢，以比較相同查詢條件下的召回率。</p>
<p>
  <span class="img-wrapper">
...
<p><strong>主要結果</strong></p>
<p><strong>在兩個資料集上，CAGRA 都比 HNSW 取得更高的召回率</strong>，顯示即使在 GPU 上建立 CAGRA 索引，再反序列化供 CPU 搜尋時，圖形品質仍維持良好。</p>
<h2 id="What’s-Next-Scaling-Index-Construction-with-Vamana" class="common-anchor-header">下一步：使用 Vamana 擴展索引建置<button data-href="#What’s-Next-Scaling-Index-Construction-with-Vamana" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Milvus 的 GPU-CPU 混合方法為當今的大型向量搜尋工作負載提供了實用且具成本效益的解決方案。透過在 GPU 上建立高品質的 CAGRA 圖，並在 CPU 上提供查詢服務，Milvus 將快速索引建置與可擴充<strong>、</strong>經濟實惠的查詢執行結合在一起，特別<strong>適合更新頻率低、查詢量大、成本限制嚴格的工作負載。</strong></p>
...
<p>其建構管道遵循三個關鍵階段：</p>
<p><strong>1.幾何批次成長</strong>- 從小批次開始形成骨架圖，然後增加批次大小以最大化並行性，最後再使用大批次完善細節。</p>
...
<li><p><a href="https://milvus.io/blog/unlocking-true-entity-level-retrieval-new-array-of-structs-and-max-sim-capabilities-in-milvus.md">解鎖真正的實體層級檢索：Milvus 中的全新結構陣列與 MAX_SIM 功能</a></p></li>

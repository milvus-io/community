...
<p><strong>1.尋找最近的叢集。</strong>系統會尋找其中心點最接近查詢向量的幾個叢集 - 就像直接前往圖書館中最有可能找到您所需書籍的兩到三個區域一樣。</p>
<p><strong>2.在這些叢集內搜尋。</strong>一旦您進入正確的區域，您只需要檢視一小部分的書籍，而不是整個圖書館。</p>
<p>這種方法可以將計算量減少幾個數量級。您仍然可以獲得高度精確的結果，但速度卻快得多。</p>
<h2 id="How-to-Build-an-IVF-Vector-Index" class="common-anchor-header">如何建立 IVF 向量索引<button data-href="#How-to-Build-an-IVF-Vector-Index" class="anchor-icon" translate="no">
    ...
    </button></h2><p>建立 IVF 向量索引的過程包含三個主要步驟：K-means 聚類、向量分配和壓縮編碼（可選）。完整流程如下：</p>
<p>
...
<h3 id="Step-3-Compression-Encoding-Optional" class="common-anchor-header">步驟 3：壓縮編碼（可選）</h3><p>為了節省記憶體和加快計算速度，每個叢集內的向量都可以經過壓縮編碼。有兩種常見的方法：</p>
<ul>
...
<tr><td><strong>查詢速度 (有過濾)</strong></td><td>穩定 - 在中心點層級執行粗略篩選，以縮小候選人的範圍</td><td>不穩定 - 尤其是當過濾比率很高 (90%+) 時，圖形會變得支離破碎，可能會退化到接近全圖遍歷，甚至比暴力搜尋更慢</td></tr>

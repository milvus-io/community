...
<p>所以很自然地，我們有了一個問題：<strong><em>為什麼這只能在 OpenClaw 中運作？如果任何代理都能擁有這樣的記憶體會如何？</em></strong>我們完全採用 OpenClaw 的記憶體架構，並建立了<a href="https://github.com/zilliztech/memsearch">memsearch</a>- 一個獨立、隨插即用的長期記憶體函式庫，可讓任何代理程式擁有持久、透明、可由人員編輯的記憶體。不需依賴 OpenClaw 的其他部分。只要將它放入，您的代理程式就能獲得由 Milvus/Zilliz Cloud 提供搜尋功能的持久性記憶體，以及 Markdown 日誌作為真相的正統來源。</p>
...
<p>Memsearch 可以解決這個問題，它讓記憶體變成 Markdown 檔案，也就是讓<strong>Git 自動處理版本</strong>。單一指令即可顯示整個歷史記錄：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Plaintext-Memory-Makes-Migration-Nearly-Effortless" class="common-anchor-header">純文字記憶體讓遷移幾乎不費吹灰之力</h3><p>遷移是記憶體框架最大的隱藏成本之一。從一個工具轉換到另一個工具通常意味著匯出資料、轉換格式、重新匯入，並希望欄位能夠相容。這樣的工作很容易花掉半天的時間，而且結果永遠無法保證。</p>
...
<li><p><strong>人類處理：</strong> <code translate="no">MEMORY.md</code> 中的長期原則，如「團隊堆疊」：Python + FastAPI + PostgreSQL"。</p></li>
</ul>
...
<h2 id="How-to-get-started-with-memsearch" class="common-anchor-header">如何開始使用 memsearch<button data-href="#How-to-get-started-with-memsearch" class="anchor-icon" translate="no">
    ...
    </button></h2><p>Memsearch 提供<strong>Python API</strong>和<strong>CLI</strong>，因此您可以在代理程式框架內使用它，或是將它當成獨立的除錯工具。設定非常簡單，系統的設計讓您的本機開發環境和生產部署看起來幾乎完全相同。</p>
...
<tbody>
<tr><td>記憶體搜尋</td><td>透明的明文記憶體、人類與人工智能共同撰寫、零移轉摩擦、容易除錯、Git-native</td><td>無內建的時序圖或複雜的多代理記憶體結構</td><td>重視長期記憶體的控制、簡易性與可攜性的團隊</td></tr>

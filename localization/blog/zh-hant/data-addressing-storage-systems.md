...
---
<p>如果您在後端系統或分散式儲存設備上工作，您可能見過這種情況：網路並未飽和，機器也未超載，但簡單的查詢卻會觸發成千上萬的磁碟 I/O 或物件儲存 API 呼叫 - 而且查詢仍需數秒鐘。</p>
<p>瓶頸很少是頻寬或計算。瓶頸在於<em>尋址</em>- 系統在讀取資料之前，先要找出資料的位置。<strong>資料定址</strong>是將邏輯識別碼（鍵、檔案路徑、偏移量、查詢謂語）轉換為儲存資料實體位置的過程。在規模上，這個過程 (而非實際的資料傳輸) 會主導延遲。</p>
<p>儲存效能可簡化為一個簡單的模型：</p>
<blockquote>
...
<h3 id="The-Core-Data-Structure" class="common-anchor-header">核心資料結構</h3><p>HashMap 的核心是圍繞單一結構建立的：陣列。散列函數會將鍵映射到陣列索引。由於鍵空間遠大於陣列，碰撞是不可避免的 - 不同的鍵可能會散列到相同的索引。這些問題會在每個槽中使用連結清單或紅黑樹局部處理。</p>
<p>陣列提供按索引的固定時間存取。此特性 - 直接、可預測的定址 - 是 HashMap 效能的基礎，也是大型儲存系統有效資料存取的相同原則。</p>
...
<h3 id="How-Does-a-HashMap-Locate-Data" class="common-anchor-header">HashMap 如何定位資料？</h3><p>
   ...
   </span> <span class="img-wrapper"> <span>分步 HashMap 寻址：散列键、计算数组索引、直接跳转到桶并在本地解析 - 无需遍历即可实现 O(1) 查询</span> </span></p>
...
<h2 id="How-Does-Addressing-Change-in-Distributed-Systems" class="common-anchor-header">在分散式系統中，編址方式有何改變？<button data-href="#How-Does-Addressing-Change-in-Distributed-Systems" class="anchor-icon" translate="no">
    ...
    </button></h2><p>HashMap 解決了單一機器內的編址問題，在單一機器中，資料存放在記憶體中，存取成本微不足道。在較大的規模下，限制條件會大幅改變：</p>
<table>
<thead>
...
<p>為了瞭解實際系統如何處理這個問題，我們來看看兩個經典範例。HDFS 將基於計算的定址方式應用於大型、以區塊為基礎的檔案。Kafka 將其應用於僅附加的訊息流。兩者都遵循相同的原則：運算資料所在的位置，而不是搜尋資料。</p>
...
<p>
   ...
   </span> <span class="img-wrapper"> <span>Kafka 稀疏索引設計顯示每 4KB 資料有一個索引項目，與記憶體比較：800MB 的密集索引與僅 2MB 的稀疏索引駐留在記憶體中。</span> </span></p>
...
<tbody>
<tr><td>設計目標</td><td>高效儲存與讀取大量檔案</td><td>訊息流的高吞吐量連續讀/寫</td></tr>
<tr><td>定址模型</td><td>路徑 → 區塊 → 資料節點 (透過記憶體內的 HashMaps)</td><td>透過稀疏索引 + 連續掃描的偏移量 → 區段 → 位置</td></tr>
<tr><td>元資料儲存</td><td>集中於 NameNode 記憶體</td><td>本地檔案，透過 mmap 進行記憶體映射</td></tr>
...
<p>此時，延遲 - 而非頻寬 - 才是瓶頸。無論回傳多少資料，單個 S3 GET 請求的成本為 ~50 毫秒。如果一個查詢會觸發數以千計的此類請求，總延遲時間就會飆長。盡量減少 API 的扇出成為設計的核心限制。</p>
<p>我們將參考兩個現代系統，一個是<a href="https://zilliz.com/learn/what-is-a-vector-database">向量資料庫</a> <a href="https://milvus.io/">Milvus</a>，另一個是湖泊表格式 Apache Iceberg，看看它們如何解決這些挑戰。儘管兩者有所差異，但兩者都運用相同的核心理念：盡量減少高延遲存取、提早減少扇出 (fan-out)、重視運算而非遍歷 (traversal)。</p>
...
<p>
   ...
   </span> <span class="img-wrapper"> <span>Milvus V1 儲存配置顯示一個集合被分割成不同的區段，每個區段在不同的 binlog 檔案中儲存 id、向量、標量資料等欄位，以及用來統計檔案的不同 stats_log 檔案。</span> </span></p>
...
<li><strong>定位目標行</strong>- 掃描已載入的 id 資料以尋找<code translate="no">id = 123</code></li>
...
<p>即使查詢只觸及三個欄位，也要調用 15,000 次物件儲存 API。以每個 S3 請求 50<strong>毫秒</strong>計算，序列化延遲達<strong>750 秒</strong>，單一查詢就超過 12 分鐘。</p>
...
<ul>
<li><strong> <a href="https://milvus.io/docs/scalar_index.md">小標量欄位</a></strong>(如 id、時間戳記) 會儲存在一起</li>
...
<p>這是一項小投資，卻有大回報。統計資料可將檔案存取從盲目讀取轉變為深思熟慮的選擇。無論是 Iceberg 的 list-level pruning 或 Milvus V2 的 Parquet footer statistics，原理都是一樣的：在寫入時，幾個位元組的元資料就能在讀取時省去數以千計的 I/O 作業。</p>
...
<p>隨著資料的成長，儲存空間也從記憶體轉移到磁碟，再轉移到物件儲存，力學會改變，但核心思想不會改變。最好的系統是計算位置而非搜尋、保持元資料的緊密性，並使用統計資料來避免接觸不重要的資料。我們檢視過的每項效能優勢，都來自於減少高延遲存取，以及儘早縮窄搜尋空間。</p>
...
<li><a href="https://milvus.io/office-hours">預約 20 分鐘的免費 Milvus Office Hours 課程</a>，以瞭解您的使用個案 - 無論是儲存佈局、查詢調整或擴充至生產。</li>

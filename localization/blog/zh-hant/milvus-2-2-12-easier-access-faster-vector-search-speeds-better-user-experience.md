...
<p>我們很高興地宣布 Milvus 2.2.12 的最新版本。此更新包含多項新功能，例如支援 RESTful API、<code translate="no">json_contains</code> 功能，以及在 ANN 搜尋過程中因應使用者回饋的向量擷取功能。我們也簡化了使用者體驗、提升向量搜尋速度，並解決了許多問題。讓我們深入了解 Milvus 2.2.12 的功能。</p>
...
<p>有了 Milvus 2.2.12，使用者可以在 ANN 搜尋過程中擷取原始向量，只要將向量欄位設定為輸出欄位，並在 HNSW、DiskANN 或 IVF-FLAT 索引的集合中進行查詢即可。此外，使用者可以期待更快的向量擷取速度。</p>
<h2 id="Support-for-operations-on-JSON-arrays" class="common-anchor-header">支援 JSON 陣列上的操作<button data-href="#Support-for-operations-on-JSON-arrays" class="anchor-icon" translate="no">
    ...
    </button></h2><p>我們最近在 Milvus 2.2.8 中新增了對 JSON 的支援。自此之後，使用者提出了許多要求，希望能支援更多的 JSON 陣列操作，例如包含、排除、交集、聯合、差異等。在 Milvus 2.2.12 中，我們優先支援<code translate="no">json_contains</code> 函式來啟用包含操作。我們將繼續在未來的版本中增加對其他運算符號的支援。</p>

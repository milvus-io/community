...
---
<p>數十年來，<code translate="no">SELECT * FROM WHERE</code> 一直是資料庫查詢的金科玉律。無論是報表系統、財務分析或使用者行為查詢，我們都已習慣使用結構化語言來精確操作資料。即使是曾經宣稱「反 SQL 革命」的 NoSQL，最終也屈服並推出 SQL 支援，承認其看似不可取代的地位。</p>
...
<ul>
<li><p>它應該查詢結構化的表格，還是在使用者嵌入中執行向量相似性搜尋？</p></li>
...
<p>這不是科幻小說。根據 Gartner 的預測，到 2026 年，大多數企業將以自然語言作為主要的查詢介面，而 SQL 則從「必備」技術變成「可選」技術。</p>
<p>這種轉變已經在發生：</p>
...
<h3 id="SQL-+-Vector-A-Beautiful-Idea-That-Executes-Poorly" class="common-anchor-header">SQL + 向量：美麗的想法卻執行不力</h3><p>傳統資料庫為了維持其相關性，紛紛在 SQL 中加入向量功能。PostgreSQL 加入了<code translate="no">&lt;-&gt;</code> 運算符，用於向量相似性搜尋：</p>
...
<p>性能方面的懲罰是非常嚴重的：</p>
<p><strong>真正的基準資料</strong>：在相同的條件下，專為 PostgreSQL 設計的 Milvus 與使用 pgvector 的 PostgreSQL 相比，查詢延遲降低 60%，吞吐量提高 4.5 倍。</p>
...
<p>試著讓 SQL 適用於向量作業，就像把螺絲起子當成榔頭一樣，技術上並不是不可能，但您使用了錯誤的工具。</p>
...
<h3 id="4-Optimized-for-Relevance-Not-Just-Speed" class="common-anchor-header">4.優化相關性，不只是速度</h3><p>與大型語言模型一樣，向量資料庫在效能與召回率之間取得平衡。透過元資料篩選、<a href="https://milvus.io/blog/get-started-with-hybrid-semantic-full-text-search-with-milvus-2-5.md">混合向量與全文</a>檢索，以及重新排序演算法，這些資料庫能持續改善結果品質與相關性，找出真正有價值的內容，而不只是快速擷取。</p>
<h2 id="The-Future-of-Databases-is-Conversational" class="common-anchor-header">資料庫的未來是對話式的<button data-href="#The-Future-of-Databases-is-Conversational" class="anchor-icon" translate="no">
    ...
    </button></h2><p>矢量資料庫代表著我們思考資料互動方式的根本性轉變。矢量資料庫並非取代關聯式資料庫，而是專為 AI 工作負載而設計，並能在 AI 為先的世界中解決完全不同的問題。</p>
...
<p>今日建置 AI 應用程式的開發人員不想要寫 SQL，他們想要描述他們所需要的東西，然後讓智慧型系統找出如何取得這些東西。</p>

...
<h2 id="Why-We-Upgraded-Milvus" class="common-anchor-header">我們為什麼升級 Milvus<button data-href="#Why-We-Upgraded-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
...
<p>在執行 Milvus 2.2.16 時，我們遇到一個嚴重的穩定性問題，這個問題已經影響到業務。在高查詢併發下，<code translate="no">planparserv2.HandleCompare</code> ，可能會導致空指標異常，造成 Proxy 元件恐慌並頻繁重新啟動。這個錯誤在高併發情況下非常容易觸發，並直接影響到我們線上推薦服務的可用性。</p>
...
<p>我們的路徑（從 2.2.16 備份，載入到 2.5.16）滿足了這兩條規則。</p>
<table>
<thead>
...
<h3 id="How-Milvus-Backup-Works" class="common-anchor-header">Milvus 備份如何運作</h3><p>Milvus 備份有助於備份和還原跨 Milvus 實體的元資料、網段和資料。它提供北向介面，例如 CLI、API 和基於 gRPC 的 Go 模組，以彈性操作備份和還原程序。</p>
<p>Milvus Backup 從源 Milvus 實例讀取集合元資料和區段來建立備份。然後，它會從源 Milvus 實例的根目錄中複製收集資料，並將其儲存到備份的根目錄。</p>
<p>要從備份還原，Milvus Backup 會根據備份中的集合中繼資料和區段資訊，在目標 Milvus 實例中創建一個新的集合。然後將備份資料從備份根目錄複製到目標實例的根目錄。</p>
...
<li><p><strong>遷移前先驗證您的備份。</strong>一旦資料採用新版本的格式，就無法輕易回溯。</p></li>
...
<tr><td>遷移</td><td>還原 milvus-backup</td><td>使用 --use_v2_restore 時要小心。在跨版本還原中，除非您清楚瞭解，否則請勿使用非預設邏輯</td></tr>
...
<p>遷移資料時，除了要考慮資料是否存在且準確之外，還需要考慮更多。您還需要注意<strong>資料的</strong> <strong>分佈</strong> <strong>方式</strong>。</p>

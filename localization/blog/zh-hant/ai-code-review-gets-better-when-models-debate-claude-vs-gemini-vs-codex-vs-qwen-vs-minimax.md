...
<p>然而，我的好奇心並未就此結束：如果這些 AI 模型一起工作會如何？我試著讓這些模型互相辯論，經過五輪對抗性的辯論後，錯誤偵測率躍升到 80%。最難的錯誤，也就是需要系統層級理解的錯誤，在辯論模式下的偵測率達到 100%。</p>
...
<p>Gemini 傾向於以強烈的判斷 (「這不好」/「看起來很好」) 開始，然後從設計/結構的角度來反向證明其合理性。有時這是有用的。有時讀起來就像是粗略地看了一下，然後就決定了。</p>
...
<p>資料集包含<a href="https://github.com/milvus-io/milvus">Milvus</a>(由<a href="https://zilliz.com/">Zilliz</a> 建立並維護的開放原始碼向量資料庫) 的 15 個 pull request。這些 PR 是非常有用的基準，因為每個 PR 都是在合併之後，因為生產中出現了 bug 而需要回復或熱修正。因此每個案例都有一個已知的 bug，我們可以對其進行評分。</p>
...
<p>這些結果與我的日常經驗相符。Claude 名列前茅並不令人驚訝。Gemini 的得分比我預期的低，這在事後看來是有道理的：我通常會在多輪會談中使用 Gemini，在會談中一起迭代設計或追尋問題，它在這種互動設定中表現良好。這個基準是一個固定、單次通過的流水線，這正是 Gemini 最弱的形式。稍後的辯論部分將顯示，當您給 Gemini 一個多回合、對抗式的形式時，它的表現會有明顯的改善。</p>
...
<p>為了避免這變成「LLM 大聲同意」，我強制執行了一個硬性規則：<strong>每個主張都必須指向特定的程式碼作為證據</strong>，而且模型不能只說「說得好」，它必須解釋為什麼它改變了主意。</p>
...
<h3 id="What-stands-out" class="common-anchor-header">突出的地方</h3><p><strong>1.L2 檢測增加一倍。</strong>常規、中等難度的 bug 從 3/10 跳到 7/10。這些都是真實程式碼庫中最常出現的 bug，也正是個別模型失誤不一致的地方。辯論機制最大的貢獻就是縮小這些日常差距。</p>
...
<h3 id="What-debate-actually-looks-like-in-practice" class="common-anchor-header">辯論實際上是什麼樣子</h3><p>上面的數字說明了辯論是有效的，但是一個具體的例子說明了它<em>為什麼</em>有效。以下是五個模型如何處理<strong>PR #44474</strong> 的精簡演練，<strong>PR #44474</strong> 在搜尋時引入了主索引鍵的偷懶載入。</p>
...
<p>儘管如此，Claude + Gemini 並不是每種類型的 bug 的最佳組合。當我將結果依 Bug 類別細分時，就會發現一個更微妙的情況：</p>
<table>
<thead>
...
<p><strong>Codex</strong>很少說話。但當它說的時候，它是重要的。它對於真正錯誤的命中率很高，而且它擅長捕捉別人都沒有注意到的東西。在 PR #44474 的範例中，Codex 就是發現零值主鍵問題的典範，而這正是整個連鎖反應的起因。將它視為補充審查員，可以捕捉到您的主要模型所遺漏的東西。</p>

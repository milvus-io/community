...
title: Milvus + RustFS+ Vibe Coding：從零開始打造輕量級 RAG 聊天機器人
author: Jinghe Ma
date: 2026-3-10
...
desc: 以 RustFS 文件為知識庫，使用 Milvus、RustFS、FastAPI 和 Next.js 建立一個輕量級的 RAG 聊天機。
origin: >-
  https://milvus.io/blog/milvus-rustfs-vibe-coding-build-a-lightweight-rag-chatbot-from-scratch.md
---
<p><em>本部落格由 Milvus 社群貢獻者馬景和撰寫</em> <em>，並經允許在此發表。</em></p>
<p>我想要一個聊天機器人，可以回答我自己文件中的問題，而且我想要完全控制它背後的堆疊 - 從物件儲存到聊天介面。這促使我以<a href="https://milvus.io/">Milvus</a>和<a href="https://rustfs.com/">RustFS</a>為核心，建立了一個輕量級的 RAG 聊天機。</p>
<p><a href="https://milvus.io/">Milvus</a>是建立 RAG 應用程式最廣泛採用的開源向量資料庫。它將運算與儲存分開，將熱資料保存在記憶體或 SSD 上以進行快速搜尋，同時依賴底下的物件儲存以進行可擴充、具成本效益的資料管理。由於它能與 S3 相容的儲存一起運作，因此很自然地適合這個專案。</p>
<p>對於儲存層，我選擇了<a href="https://rustfs.com/">RustFS</a>，這是用 Rust 寫成的開放原始碼 S3 相容物件儲存系統。它可以透過二進位、Docker 或 Helm 圖表部署。雖然它仍處於 alpha 狀態，不建議用於生產工作負載，但對於這次的建置而言，它已經夠穩定了。</p>
<p>基礎架構就位後，我需要一個知識庫來查詢。RustFS 文件 - 約 80 個 Markdown 檔案 - 是一個方便的起點。我將文件分塊、產生嵌入、儲存到 Milvus，剩下的就用 vibe 編碼：後端使用<a href="https://fastapi.tiangolo.com/">FastAPI</a>，聊天介面使用<a href="https://nextjs.org/">Next.js</a>。</p>
<p>在這篇文章中，我將介紹端對端的完整系統。程式碼可在 https://github.com/majinghe/chatbot 取得。這是一個可運作的原型，而不是一個可供生產的系統，但我們的目標是提供一個清晰、可擴充的建置，讓您可以為自己的用途進行調整。以下每節都會介紹一個層次，從基礎架構到前端。</p>
<h2 id="Installing-Milvus-and-RustFS-with-Docker-Compose" class="common-anchor-header">使用 Docker Compose 安裝 Milvus 和 RustFS<button data-href="#Installing-Milvus-and-RustFS-with-Docker-Compose" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>讓我們先安裝<a href="https://milvus.io/">Milvus</a>和<a href="https://rustfs.com/">RustFS</a>。</p>
<p>Milvus 可以使用任何 S3 相容的物件儲存，雖然 MinIO 是標準設定中的預設後端。由於 MinIO 不再接受社群貢獻，因此在本範例中我們將以 RustFS 取代它。</p>
<p>要進行這項變更，請更新 Milvus 套件庫內 configs/milvus.yaml 中的物件儲存設定。相關章節如下：</p>
...
<p>有兩種方式可以套用此變更：</p>
<ul>
<li><strong>掛載本機設定檔案。</strong>複製 configs/milvus.yaml 到本機，更新 MinIO 欄位以指向 RustFS，然後透過 Docker volume 將其掛載到容器中。</li>
<li><strong>在啟動時使用</strong> <strong>yq****</strong><strong>進行修補</strong> <strong>。</strong>修改容器的指令，在 Milvus 程序啟動前，針對 /milvus/configs/milvus.yaml 執行 yq。</li>
</ul>
<p>這次的建置使用第一種方式。在 docker-compose.yml 中的 Milvus 服務得到一個額外的 volume entry：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="The-Docker-Compose-Setup" class="common-anchor-header">Docker Compose 設定</h3><p>完整的 docker-compose.yml 會執行四個服務。</p>
<p><strong>etcd</strong>- Milvus 依賴 etcd 來儲存元資料：</p>
<pre><code translate="no">etcd:
    ...
    healthcheck:
      ...
      retries: 3
<button class="copy-code-btn"></button></code></pre>
<p><strong>Attu</strong>- Milvus 的視覺化使用者介面，由 Zilliz 開發並開放源碼 (註：2.6 以後的版本為封閉源碼)：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p><strong>RustFS</strong>- 物件儲存後端：</p>
<pre><code translate="no">rustfs:
    ...
    healthcheck:
      ...
      start_period: 40s
<button class="copy-code-btn"></button></code></pre>
<p><strong>Milvus</strong>- 以獨立模式執行：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Starting-Everything" class="common-anchor-header">啟動一切</h3><p>配置就緒後，啟動所有四個服務：</p>
<pre><code translate="no">docker compose -f docker-compose.yml up -d
<button class="copy-code-btn"></button></code></pre>
<p>您可以透過以下方式驗證一切運作正常：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>當所有四個容器都啟動後，您的服務就可以在以下網址使用：</p>
<ul>
<li><strong>Milvus:</strong> <ip>:19530</li>
<li><strong>RustFS:</strong> <ip>:9000</li>
<li><strong>Attu:</strong> <ip>:8000</li>
</ul>
<h2 id="Vectorizing-the-RustFS-Docs-and-Storing-Embeddings-in-Milvus" class="common-anchor-header">將 RustFS 文件矢量化並將嵌入式儲存於 Milvus<button data-href="#Vectorizing-the-RustFS-Docs-and-Storing-Embeddings-in-Milvus" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>有了 Milvus 和 RustFS 的運行，下一步就是建立知識庫。源資料是 RustFS 中文說明文件：80 個 Markdown 檔案，你將把它們分塊、嵌入並儲存在 Milvus 中。</p>
<h3 id="Reading-and-Chunking-the-Docs" class="common-anchor-header">閱讀並將文件分塊</h3><p>腳本會遞迴讀取 docs 資料夾中的每個 .md 檔案，然後按新行將每個檔案的內容分割成小塊：</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>這個分塊策略是刻意簡單的。如果您想要更嚴格的控制 - 依據標題分割、保留程式碼區塊或重疊區塊以獲得更佳的檢索效果 - 這就是您需要反覆練習的地方。</p>
<h3 id="Embedding-the-Chunks" class="common-anchor-header">嵌入區塊</h3><p>準備好資料塊後，您可以使用 OpenAI 的 text-embedding-3-large 模型來嵌入資料塊，該模型會輸出 3072 個維向量：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Storing-Embeddings-in-Milvus" class="common-anchor-header">在 Milvus 中儲存嵌入資料</h3><p>Milvus 將資料組織成集合，每個集合由一個模式定義。在這裡，每條記錄都會儲存原始文字塊及其嵌入向量：</p>
...
collection.flush()
<button class="copy-code-btn"></button></code></pre>
<p>一旦插入完成，您可以在 Attu 中驗證集合，網址是<ip>:8000 - 您應該會看到 docs_collection 列在集合之下。</p>
<p>
...
<p>您也可以在<ip>:9000 檢查 RustFS，確認底層資料是否落在物件儲存中。</p>
<p>
  <span class="img-wrapper">
...
<h2 id="Building-a-RAG-Pipeline-with-Milvus-and-OpenAI’s-GPT-5" class="common-anchor-header">使用 Milvus 和 OpenAI 的 GPT-5 建立 RAG Pipeline<button data-href="#Building-a-RAG-Pipeline-with-Milvus-and-OpenAI’s-GPT-5" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>有了儲存於 Milvus 的 embeddings，您就有了建立 RAG 管道所需的一切。流程是：嵌入使用者的查詢、從 Milvus 擷取語意最相似的區塊、組合提示，然後呼喚 GPT-5。這裡的建置使用 OpenAI 的 GPT-5，但任何具備聊天功能的模型都可以在這裡運作 - 擷取層才是最重要的，無論最後由哪個 LLM 產生答案，Milvus 都會處理。</p>
...
<p>將一切包裝在 FastAPI + Next.js 聊天機中</p>
<p>RAG 管道正在運作，但每次要發問時都執行 Python 腳本就失去意義了。所以我向 AI 詢問堆疊建議。答案是後端使用<strong>FastAPI</strong>- RAG 程式碼已經是 Python 程式碼，所以將它包裝在 FastAPI 端點是很自然的做法 - 而前端使用<strong>Next.js</strong>。FastAPI 以 HTTP 端點的方式揭露 RAG 邏輯；Next.js 會呼叫它，並在聊天視窗中渲染回應。</p>
<h3 id="FastAPI-Backend" class="common-anchor-header">FastAPI 後端</h3><p>FastAPI 將 RAG 邏輯包裝在單一的 POST 端點中。任何用戶端現在都可以透過 JSON 請求來查詢您的知識庫：</p>
...
<button class="copy-code-btn"></button></code></pre>
<h3 id="Nextjs-Frontend" class="common-anchor-header">Next.js 前端</h3><p>前端將使用者的查詢傳送至 FastAPI 端點，並渲染回應。核心的擷取邏輯：</p>
<p>javascript</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>在瀏覽器中開啟<code translate="no">http://&lt;ip&gt;:3000/chat</code> 。</p>
<p>
  <span class="img-wrapper">
...
<h2 id="Conclusion" class="common-anchor-header">結論<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>從一開始對 Milvus 儲存後端的好奇，到現在完整運作的 RAG 聊天機器人，從一個到另一個的過程比預期的還要短。以下是完整的建置內容：</p>
<ul>
<li><strong><a href="http://milvus.io">Milvus</a></strong> <strong>+</strong> <strong><a href="https://rustfs.com/">RustFS</a></strong> <strong>與 Docker Compose。</strong>Milvus 以獨立模式執行，並以 RustFS 作為其物件儲存後端，取代預設的 MinIO。共有四種服務：etcd、Milvus、RustFS 和 Attu。</li>
<li><strong>將知識庫矢量化。</strong>RustFS 的說明文件 - 80 個 Markdown 檔案 - 經過分塊、text-embedding-3-large 的嵌入，並儲存在 Milvus 中，成為 466 個向量。</li>
<li><strong>RAG 管道。</strong>在查詢時，使用者的問題會以同樣的方式嵌入，Milvus 會擷取語意最相似的三個區塊，而 GPT-5 會根據這些文件產生答案。</li>
<li><strong>聊天機使用者介面。</strong>FastAPI 將管道包裝在單一 POST 端點中；Next.js 則在管道前方放置聊天視窗。再也不用跳到終端機來發問了。</li>
</ul>
<p>我從過程中得到的一些啟發：</p>
<ul>
<li><strong><a href="https://milvus.io/docs">Milvus 的說明文件</a></strong> <strong>很棒。</strong>尤其是部署部分 - 清晰、完整、易於遵循。</li>
<li><strong><a href="https://rustfs.com/">RustFS</a></strong> <strong>作為物件儲存的後端，使用起來非常愉快。</strong>把它丟進 MinIO 比預期花的功夫還要少。</li>
<li><strong>Vibe 的編碼速度很快，直到範圍接管為止。</strong>從 Milvus 到 RAG 到聊天機器人，再到「也許我該把整件事 Docker 化」。需求不會自行匯合。</li>
<li><strong>調試比閱讀教得更多。</strong>這次建置中的每一次失敗，都讓下一節的點選速度比任何文件都要快。</li>
</ul>
<p>這次建置的所有程式碼都在<a href="https://github.com/majinghe/chatbot"></a> github<a href="https://github.com/majinghe/chatbot">.com/majinghe/chatbot</a>。如果您想親自嘗試<a href="http://milvus.io">Milvus</a>，<a href="https://milvus.io/docs/quickstart.md">快速入門</a>是個很好的開始。如果您想討論您正在建置的東西，或是遇到一些意想不到的事情，請來<a href="https://milvus.io/slack">Milvus Slack</a> 找我們。如果您想要進行專屬對話，也可以<a href="https://meetings.hubspot.com/chloe-williams1/milvus-office-hour?uuid=4cb203e5-482a-47e0-90a6-7acc511d61f4">在上班時間預約時段</a>。</p>

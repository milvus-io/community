...
<p>我想要一個聊天機器人，可以回答我自己文件中的問題，而且我想要完全控制它背後的堆疊 - 從物件儲存到聊天介面。這促使我以<a href="https://milvus.io/">Milvus</a>和<a href="https://rustfs.com/">RustFS</a>為核心，建立一個輕量級的 RAG 聊天機。</p>
<p><a href="https://milvus.io/">Milvus</a>是建立 RAG 應用程式最廣泛採用的開放原始碼向量資料庫。它將運算與儲存分開，將熱資料保存在記憶體或 SSD 上以進行快速搜尋，同時依賴底下的物件儲存以進行可擴充、具成本效益的資料管理。由於它可與 S3 相容的儲存一起運作，因此自然適合這個專案。</p>
...
<p>基礎架構就位後，我需要一個知識庫來查詢。RustFS 文件 - 約 80 個 Markdown 檔案 - 是一個方便的起點。我將文件分塊、產生嵌入、儲存在 Milvus 中，剩下的就用 vibe 編碼：後端使用<a href="https://fastapi.tiangolo.com/">FastAPI</a>，聊天介面使用<a href="https://nextjs.org/">Next.js</a>。</p>
...
<p>有兩種方式可以套用此變更：</p>
<ul>
<li><strong>掛載本機設定檔案。</strong>本機複製 configs/milvus.yaml，更新 MinIO 欄位以指向 RustFS，然後透過 Docker volume 將其掛載到容器中。</li>
...
<button class="copy-code-btn"></button></code></pre>
<p>這個分塊策略是刻意簡單的。如果您想要更嚴格的控制 - 依據標題分割、保留程式碼區塊或重疊區塊以獲得更佳的檢索效果 - 就需要在這裡反覆練習。</p>
...
<h2 id="Building-a-RAG-Pipeline-with-Milvus-and-OpenAI’s-GPT-5" class="common-anchor-header">使用 Milvus 和 OpenAI 的 GPT-5 建立 RAG Pipeline<button data-href="#Building-a-RAG-Pipeline-with-Milvus-and-OpenAI’s-GPT-5" class="anchor-icon" translate="no">
    ...
    </button></h2><p>有了儲存於 Milvus 的 embeddings，您就有了建立 RAG 管道所需的一切。流程是：嵌入使用者的查詢、從 Milvus 擷取語意最相似的區塊、組合提示，然後呼喚 GPT-5。這裡的建置使用 OpenAI 的 GPT-5，但任何具備聊天功能的模型都可以在這裡運作 - 擷取層才是最重要的，無論哪個 LLM 產生最終答案，Milvus 都會處理。</p>
...
<li><strong>將知識庫矢量化。</strong>RustFS 的說明文件 - 80 個 Markdown 檔案 - 經過分塊、text-embedding-3-large 嵌入，並儲存在 Milvus 中，成為 466 個向量。</li>
...
<li><strong><a href="https://rustfs.com/">RustFS</a></strong> <strong>作為物件儲存的後端非常好用。</strong>把它加入 MinIO 所花的時間比預期的還要少。</li>

...
<p>最常見的 RAG 用例之一是公司知識庫。使用者上傳 PDF、Word 檔案或其他內部文件，提出自然語言問題，並根據這些資料獲得答案，而非僅根據模型的預先訓練。</p>
...
<li><strong>向量儲存層。</strong>每個分塊都嵌入 384 維向量中，並與元資料 (檔案名稱、頁碼、分塊 ID) 一起儲存於<a href="https://milvus.io"></a><a href="https://milvus.io">Milvus</a>。平行倒轉索引支援關鍵字搜尋。</li>
...
<p>Milvus 對資料庫名稱有嚴格的規定 - 只能使用 ASCII 字母、數字和底線。如果知識庫名稱包含非 ASCII 字元，後端會在建立資料庫前以 kb_ 前綴進行十六進制編碼，並在顯示時進行解碼。這是一個很小的細節，但卻可以防止隱藏錯誤。</p>
...
<ul>
<li><strong>向量搜尋 (密集)：</strong>尋找意思相近的內容，甚至是跨語言的內容，但可能會出現無法直接回答問題的相關段落。</li>
<li><strong>關鍵字搜尋 (稀疏)：</strong>尋找精確匹配的專業詞彙、數字或公式變數 - 向量內嵌通常會忽略的那種標記。</li>
</ul>
<p>系統會使用 RRF (Reciprocal Rank Fusion) 來合併兩個結果清單。每個候選人會根據其在每個清單中的排名獲得一個分數，因此在<em>兩個</em>清單中<em>都</em>接近頂端的資料塊得分最高。向量搜尋會貢獻語意涵蓋率；關鍵字搜尋會貢獻詞彙精確度。</p>
<p>
...
它會使用 OCR 文字來回答問題，因此系統可以繼續工作而不會中斷。</p>
...
<p>從預設的灰色樣式改成白色圓角設計。看起來更簡潔、更現代。</p>
<ul>

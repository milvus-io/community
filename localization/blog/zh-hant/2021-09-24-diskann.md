...
tag: Engineering
---
<custom-h1>DiskANN：基於磁盤的ANNS解決方案，在十億級數據集上具有高回復率和高QPS</custom-h1><blockquote>
<p>李成明，Zilliz 研發工程師，畢業於東南大學，取得計算機科學碩士學位。他目前的研究重點是高維數據上的 ANNS 問題，包括基於圖和基於量化的解決方案。</p>
</blockquote>
...
<ol>
<li>DiskANN 可以在一台擁有 64GB 記憶體的機器上索引和搜尋 100 多個維度的十億級資料集，提供超過 95% 的回復率@1，且延遲時間低於 5 毫秒。</li>
...
<li>調整 alpha &gt; 1 (本文建議 1.2) 並重複步驟 3。而步驟三是基於隨機近鄰圖形，圖形在第一次迭代後品質較低。因此，需要再次迭代以改善圖形品質，這對召回率非常重要。</li>
</ol>
...
<p>文中未提供搜尋參數，可能與索引參數一致。在參數選擇上，文中提到的 NSG 參數是根據 NSG 的 GitHub 套件庫中列出的參數來選擇效能較佳的群組。Vamana 與 NSG 的參數比較接近，所以參數設定也比較接近。然而，HNSW 參數選擇的原因並沒有給出。我們認為 HNSW 的參數 M 設定得相對較大。我們認為 HNSW 的參數 M 設定得比較大，如果它們的 out-degrees 不是設定在相同的水準，可能會導致基於圖的索引之間的比較缺乏說服力。</p>
...
<ul>
<li>partition_with_ram_budget：根據參數 M 決定片段的數量 k。使用 kmeans 對資料集進行採樣，將每個點分配到兩個最近的叢集。分割資料集，每個分割產生兩個檔案：一個資料檔案和一個 ID 檔案。ID 檔案和資料檔案互相對應，ID 檔案中的每個 ID 對應資料檔案中的一個向量。ID 是將原始資料中的每個向量從 0 到 n-1 編號得到的。ID 相對重要，與合併有關。<ul>
...
<li>使用具有 1GB 快取記憶體的閱讀器開啟 num_parts 片段的 Vamana 索引，並使用具有 1GB 快取記憶體的寫入器開啟輸出檔案，準備合併；</li>
...
<li>根據所給的參數 L，每個 L 都會用查詢集再執行一次 cached_beam_search，並輸出召回率、QPS 等統計資料。熱身及統計熱點資料的過程不計入查詢時間。</li>
</ol>
...
<li>對於快取命中的點，使用原始資料和查詢資料計算精確距離，加入結果佇列，然後在加入搜尋佇列前，使用 PQ 計算未到訪過的鄰近點的距離。搜尋佇列的長度受參數限制。</li>

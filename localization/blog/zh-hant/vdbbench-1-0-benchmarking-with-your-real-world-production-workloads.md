...
---
<p>大多數向量資料庫基準都是以靜態資料和預先建立的索引進行測試。但生產系統並非如此-當使用者執行查詢時，資料會不斷流動，篩選器會分割索引，而效能特性會在並發讀取/寫入負載下大幅改變。</p>
<p>今天，我們發佈<a href="https://github.com/zilliztech/VectorDBBench/releases/tag/v1.0.0"><strong>VDBBench 1.0</strong></a>，這是一個開放原始碼基準，從一開始就設計用來測試向量資料庫在實際生產條件下的效能：串流資料擷取、不同選擇性的元資料篩選，以及揭示實際系統瓶頸的並發工作負載。</p>
...
<h2 id="Why-Current-Benchmarks-Are-Misleading" class="common-anchor-header">為什麼目前的 Benchmark 有誤導性？<button data-href="#Why-Current-Benchmarks-Are-Misleading" class="anchor-icon" translate="no">
    ...
    </button></h2><p>老實說，我們業界有個奇怪的現象。每個人都在談論「不要玩基準」，但許多人卻恰恰參與了這種行為。自 2023 年向量資料庫市場爆紅以來，我們已經看過無數系統「基準做得很好」，但在生產中「失敗得很慘」，浪費工程時間並損害專案可信度的例子。</p>
...
<p><strong>舉例來說</strong>：在 Cohere 1M 測試中，Milvus 在所有過濾選擇性層級中都持續維持高召回率，而 OpenSearch 則表現不穩定，召回率在不同過濾條件下大幅波動 - 在許多情況下召回率低於 0.8，這對大多數生產環境而言是無法接受的。</p>
<p>
...
<li><p>記錄全面的指標：延遲分佈 (包括 P99)、持續 QPS 及召回精確度</p></li>
...
<p>圖：Pinecone 與 Elasticsearch 在 Cohere 10M Streaming 測試中的 QPS 與 Recall 比較（500 行/秒的擷取率）。</p>
...
<p>如我們的測試所示，Elasticsearch 在索引最佳化後的 QPS 超越了 Pinecone。但是當 x 軸反映實際經過的時間時，很明顯 Elasticsearch 花了更長的時間才達到這個效能。在生產中，這個延遲很重要。這項比較揭示了一個關鍵的權衡：峰值吞吐量 vs. 服務時間。</p>
<h3 id="🔬-Modern-Datasets-That-Reflect-Current-AI-Workloads" class="common-anchor-header">反映目前 AI 工作負載的現代資料集</h3><p>我們對向量資料庫基準測試所使用的資料集進行了全面調整。VDBBench 不再使用 SIFT 和 GloVe 等傳統測試集，而是使用由 OpenAI 和 Cohere 等最先進嵌入模型產生的向量，這些模型為當今的 AI 應用程式提供了強大的動力。</p>
...
<h2 id="Getting-Started-with-VDBBench-10" class="common-anchor-header">開始使用 VDBBench 1.0<button data-href="#Getting-Started-with-VDBBench-10" class="anchor-icon" translate="no">
    ...
    </button></h2><p><strong>VDBBench 1.0</strong>代表著向生產相關基準測試的根本性轉變。透過涵蓋連續資料寫入、不同選擇性的元資料篩選，以及並發存取模式下的串流負載，VDBBench 1.0 提供了目前最接近實際生產環境的測試。</p>

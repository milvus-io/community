...
<p>傳統的基於關鍵字的高亮度處理無法解決這個問題。舉例來說，如果使用者詢問「如何改善 Python 程式碼的執行效率？」，關鍵字高亮器會挑出「Python」和「效率」，但卻會錯過真正回答問題的句子 -「使用 NumPy 向量運算取代循環」，因為它與查詢的關鍵字並不相同。我們需要的是語意理解，而不是字串匹配。</p>
...
<li><p><strong>支援語言：</strong>英文和中文</p></li>
</ul>
...
<h3 id="Model-Architecture" class="common-anchor-header">模型架構</h3><p>我們建立的模型是一個輕量<strong>級</strong>的<strong>純編碼器</strong>網路，將上下文剪枝視為<strong>標記層級的相關性評分任務</strong>。這個設計的靈感來自於<a href="https://arxiv.org/html/2501.16214v1">Provence</a>，這是 Naver 在 2025 年 ICLR 上推出的一種上下文剪枝方法，它將剪枝工作從「選擇正確的大塊」轉變為「為每個標記評分」。這一框架與語義高亮自然地結合，在語義高亮中，細粒度信號是不可或缺的。</p>
<p>只有編碼器的模型並非最新的架構，但在此仍然非常實用：它們速度快、易於擴展，而且可以並行產生所有標記位置的相關性評分。對於生產型 RAG 系統來說，速度上的優勢遠比使用較大的解碼器模型來得重要。</p>
...
<li><p>針對重新排序進行訓練，這與相關性判斷任務非常接近</p></li>
</ol>
...
<li><p><strong>更好的可觀察性：</strong>我們可以看到<em>為什麼</em>會選擇某個句子，而不是把標籤當成黑箱。</p></li>
...
<p>我們使用<strong>本地 vLLM 服務</strong>而非雲端 API 來執行所有註解。這為我們帶來了高吞吐量、可預測的效能，以及更低的成本 - 實質上是以 GPU 時間換取 API 令牌費用，在產生數百萬個樣本時，這是更划算的交易。</p>
...
<h3 id="Training-Method" class="common-anchor-header">訓練方法</h3><p>一旦模型架構和資料集準備就緒，我們就在<strong>8× A100 GPU</strong>上進行三個 epochs 的模型訓練，從頭到尾大概花了<strong>9 個小時</strong>。</p>
...
<h2 id="Real-World-Case-Study" class="common-anchor-header">實際案例研究<button data-href="#Real-World-Case-Study" class="anchor-icon" translate="no">
    ...
    </button></h2><p>基準測試只能說明部分情況，因此這裡有一個真實案例，顯示模型在常見邊緣情況下的表現：當檢索的文字同時包含正確答案和非常誘人的分心物時。</p>
...
<p>我們很樂意聽取您的使用心得，包括錯誤報告、改進構想，或是您在將其整合至工作流程時所發現的任何問題。</p>

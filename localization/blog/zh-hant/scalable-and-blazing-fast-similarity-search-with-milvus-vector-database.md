...
<h3 id="Milvus-Architecture" class="common-anchor-header">Milvus 架構</h3><p>下圖展示 Milvus 平台的整體架構。Milvus 將資料流與控制流分開，並分為四層，在可擴充性及災難復原方面都是獨立的。</p>
<p>
...
<p>在這個工作流程中，我們可以使用像<a href="https://github.com/towhee-io/towhee">towhee</a>之類的開放源碼框架，利用像 ResNet-50 之類的預先訓練模型，從圖片中萃取向量，輕鬆地在 Milvus 中儲存這些向量並編入索引，同時在 MySQL 資料庫中儲存圖片 ID 到實際圖片的對應。一旦資料被編入索引，我們就可以輕鬆地上傳任何新的圖片，並使用 Milvus 執行規模化的圖片搜尋。下圖顯示了一個可視化圖片搜尋範例。</p>
<p>

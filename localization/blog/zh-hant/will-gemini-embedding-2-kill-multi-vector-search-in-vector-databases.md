...
desc: Google 的 Gemini Embedding 2 將文字、圖片、視訊和音訊映射成一個向量。這會讓多向量搜尋過時嗎？我們分析了每種方法的適用範圍。
origin: >-
...
<p>您只需呼叫一次 API，即可嵌入視訊片段、產品照片和一段文字，而且它們都會出現在相同的語意鄰域中。</p>
<p>在使用這樣的模型之前，您別無選擇，只能將每種模式透過自己的專家模型執行，並將每種輸出儲存於單獨的向量列中。像<a href="https://milvus.io/docs/multi-vector-search.md">Milvus</a>之類的向量資料庫中的多向量列，正是為了這個世界而建立的。</p>
<p>因此，現在的問題是：Gemini Embedding 2 可以取代多少複雜性？這篇文章將介紹每種方法的適用範圍，以及它們如何相互配合。</p>
...
<p><strong>在使用 Gemini Embedding 2 之前</strong>，您需要三個獨立的嵌入模型 (影像、音訊、文字)、三個向量列，以及一個進行多向召回、結果融合和重複資料刪除的檢索管道。這需要建立和維護許多活動的部分。</p>
<p><strong>現在</strong>，您只需將視訊的畫格、音訊和字幕納入單一 API 呼叫，就能得到一個統一的向量，擷取完整的語意畫面。</p>
...
<p><a href="https://milvus.io/">Milvus</a>以平行<a href="https://milvus.io/docs/multi-vector-search.md">ANN 搜尋</a>方式執行這兩種模式，並透過 RRFRanker 進行原生重排。模式定義、多重索引配置、內建 BM25 - 全部都在一個系統中處理。</p>
<p>為了具體說明這一點，請考慮一個產品目錄，其中每一個項目都包含文字描述和圖片。您可以針對這些資料並行執行三種搜尋：</p>
<ul>
...
<h2 id="Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="common-anchor-header">現在，有了 Gemini Embedding 2，多向量搜尋還重要嗎？<button data-href="#Now-with-Gemini-Embedding-2-Does-Multi-Vector-Search-Still-Matter" class="anchor-icon" translate="no">
      <svg translate="no"
        aria-hidden="true"
        focusable="false"
        height="20"
    ...
    </button></h2><p>Gemini Embedding 2 可以在一次呼叫中處理更多的模式，大大簡化了管道。但統一的多模態嵌入與多向量檢索並不是一回事。換句話說，多向量檢索不會過時。</p>
<p>Gemini Embedding 2 將文字、影像、視訊、音訊和文件映射到一個共用的向量空間。Google<a href="https://developers.googleblog.com/en/gemini-embedding-model-now-available/">將其定位</a>為多模態語意搜尋、文件檢索和推薦 - 所有模態描述相同內容的場景，高度的跨模態重疊使得單一向量變得可行。</p>
...
<p>這兩種方法並不互相排斥。您可以使用 Gemini Embedding 2 來處理統一的多模組欄位，同時仍可在同一<a href="https://milvus.io/"></a><a href="https://milvus.io/">Milvus</a>資料集中的獨立欄位中儲存其他稀疏或特定屬性向量。</p>
<h2 id="Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="common-anchor-header">快速入門：設定 Gemini Embedding 2 + Milvus<button data-href="#Quick-Start-Set-Up-Gemini-Embedding-2-+-Milvus" class="anchor-icon" translate="no">
    ...
    </button></h2><p>以下是一個工作示範。您需要一個執行中<a href="https://milvus.io/docs/install-overview.md">的</a> <a href="https://milvus.io/docs/install-overview.md"></a><a href="https://milvus.io/docs/install-overview.md">Milvus 或 Zilliz Cloud 實例</a>和 GOOGLE_API_KEY。</p>
...
<button class="copy-code-btn"></button></code></pre>
<p>對於圖像和音訊嵌入，使用 embed_image() 和 embed_audio()的方式相同 - 向量落在相同的集合和相同的向量空間中，實現真正的跨模組搜尋。</p>

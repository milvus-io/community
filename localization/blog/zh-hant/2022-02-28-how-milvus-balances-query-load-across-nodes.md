...
<h2 id="Load-balance" class="common-anchor-header">負載平衡<button data-href="#Load-balance" class="anchor-icon" translate="no">
    ...
    </button></h2><p>當負載平衡被觸發，查詢協調器首先載入目標段到目的地查詢節點。此時兩個查詢節點都會在任何查詢請求中回傳目標網段的查詢結果，以保證結果的完整性。</p>
...
<p>收到<code translate="no">sealedSegmentChangeInfo</code> 之後，來源查詢節點就會釋放目標網段。</p>
<p>
...
<p>當源查詢節點釋放目標網段時，整個流程就成功了。完成此步驟後，查詢節點間的查詢負載已設定為平衡，意即所有查詢節點的 RAM 使用量都不大於<code translate="no">queryCoord.overloadedMemoryThresholdPercentage</code> ，且負載平衡後來源查詢節點和目標查詢節點的 RAM 使用量差異的絕對值小於負載平衡前的差異。</p>

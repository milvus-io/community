...
<h3 id="Garment-detection" class="common-anchor-header">成衣偵測</h3><p>在成衣偵測模組中，使用<a href="https://pytorch.org/hub/ultralytics_yolov5/">YOLOv5</a>（一階式、基於錨點的目標偵測架構）作為物體偵測模型，因其體積小且可即時推論。它提供四種模型尺寸 (YOLOv5s/m/l/x)，每種特定尺寸都有其優缺點。較大的模型會有較好的表現（精確度較高），但需要較多的運算能力，且執行速度較慢。由於本案例中的物件都是相對較大的項目，而且容易偵測，因此使用最小的模型 YOLOv5s 就足夠了。</p>
...
<h3 id="Outfit-suggestions" class="common-anchor-header">服裝建議</h3><p>透過在 Stylepedia 資料庫中進行相似度搜尋，使用者可以找到包含特定時尚項目的時尚照片。這些可能是某人正在考慮購買的新款服裝，也可能是他們自己收藏中可以穿著或搭配不同款式的服裝。然後，透過對其經常搭配的物品進行聚類，便可產生穿著建議。舉例來說，一件黑色機車夾克可以搭配多種單品，例如一條黑色緊身牛仔褲。使用者就可以瀏覽在所選公式中出現此搭配的相關時尚照片。</p>
<p>

...
<p>本指南將介紹如何使用<strong>LangSmith Agent Builder + Milvus</strong> 來建立一個生產就緒、具備記憶功能的 AI 助理，而且不需要寫任何程式碼。</p>
...
<li><strong>工具：</strong>工具可讓代理採取行動 - 發送電子郵件、張貼到 Slack、建立行事曆事件、搜尋資料或呼叫 API。Agent Builder 透過模型上下文通訊協定 (Model Context Protocol, MCP) 整合這些工具，提供安全、可擴充的方式來揭露功能。使用者可以依賴內建的整合，或新增自訂的 MCP 伺服器，包括用於向量搜尋和長期記憶的 Milvus<a href="https://milvus.io/blog/why-vibe-coding-generate-outdated-code-and-how-to-fix-it-with-milvus-mcp.md">MCP 伺服器</a>。</li>
...
<p>在引擎蓋下，代理程式會保留一個內部記憶體檔案，基本上就是它不斷演進的系統提示。每次啟動時，它都會讀取這個檔案，以決定如何執行。當您提出修正或限制時，代理程式會加入結構化規則來編輯檔案，例如「總是以一首振奮人心的短詩結束簡報」。這種方法遠比依賴對話記錄來得穩定，因為代理程式會主動改寫操作指令，而不是將您的喜好埋藏在記錄檔中。</p>
...
<h2 id="Conclusion" class="common-anchor-header">總結<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>由於 AI 的發展，建立 AI 代理不再需要複雜的工作流程或繁重的工程。使用 LangSmith Agent Builder，您可以僅使用自然語言建立有狀態、長時間運行的助理。您只需專注於描述代理程式應該做什麼，系統則會處理規劃、工具執行以及持續的記憶體更新。</p>
<p>搭配<a href="https://milvus.io/blog">Milvus</a>，這些代理程式就能獲得可靠、持續的記憶，以進行語意搜尋、偏好追蹤和跨會話的長期情境。無論您是在驗證一個想法或是部署一個可擴充的系統，LangSmith Agent Builder 與 Milvus 都提供了一個簡單、靈活的基礎，讓代理程式不只是回應，還能隨時間記憶與改進。</p>

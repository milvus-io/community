...
<p>在這篇文章中，我將說明 Anthropic Skills 如何運作，然後在 Claude Code 中建立一個簡單的 Skill，將自然語言轉換成<a href="https://milvus.io/">Milvus 支援的</a>知識庫 - RAG 的快速設定，不需要額外的佈線。</p>
...
<p>這個格式刻意設計得很簡單，也因為如此，它已經被許多開發人員工具支援或輕鬆地適應 - Claude Code、Cursor、VS Code 擴充、GitHub 整合、Codex 式的設定等等。</p>
<p>Skill 遵循一致的資料夾結構：</p>
...
<p>總體而言，此架構反映了如何將工作移交給新隊友：<code translate="no">SKILL.md</code> 解釋工作、<code translate="no">scripts/</code> 提供即用工具、<code translate="no">templates/</code> 定義標準格式，以及<code translate="no">resources/</code> 提供背景資訊。有了所有這些，代理就能可靠地執行任務，並將猜測減到最少。</p>
<h2 id="Hands-on-Tutorial-Creating-a-Custom-Skill-for-a-Milvus-Powered-RAG-System" class="common-anchor-header">實作教學：為 Milvus 驅動的 RAG 系統建立自訂技能<button data-href="#Hands-on-Tutorial-Creating-a-Custom-Skill-for-a-Milvus-Powered-RAG-System" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在本節中，我們將逐步建立一個自訂的 Skill，它可以設定 Milvus 套件，並透過簡單的自然語言指令組裝完整的 RAG 管線。我們的目標是跳過所有常見的設定工作 - 不需要手動模式設計、不需要索引設定、不需要模板程式碼。您只需告訴代理您想要什麼，Skill 就會為您處理 Milvus 的元件。</p>
...
<tbody>
<tr><td>環境檢查</td><td><code translate="no">check_env.py</code></td><td>檢查 Python 版本、所需的相依性，以及 Milvus 連線。</td></tr>

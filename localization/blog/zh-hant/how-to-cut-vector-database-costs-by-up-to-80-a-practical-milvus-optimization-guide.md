...
<p><strong>有關網路和儲存設備的快速說明：</strong>您可以只傳回所需欄位 (ID、分數、關鍵元資料) 並避免跨區域查詢，以降低網路成本。在儲存方面，Milvus 已經將儲存與運算分開 - 您的向量存放在 S3 之類的廉價物件儲存中，因此即使有 100M 的向量，儲存成本通常也低於 50 美元/月。這兩種方式都無法像記憶體最佳化一樣達到預期效果。</p>
<h2 id="Why-Memory-Is-So-Expensive-for-Vector-Search" class="common-anchor-header">為什麼向量搜尋的記憶體如此昂貴？<button data-href="#Why-Memory-Is-So-Expensive-for-Vector-Search" class="anchor-icon" translate="no">
    ...
    </button></h2><p>如果您來自傳統資料庫，向量搜尋對記憶體的需求可能會令人驚訝。關聯式資料庫可以利用磁碟的 B 樹索引和作業系統的頁面快取。向量搜尋則不同，它涉及大量浮點運算，而 HNSW 或 IVF 等索引需要持續載入記憶體，以提供毫秒級的延遲。</p>
<p>以下是估算記憶體需求的快速公式：</p>
...
<h2 id="2-Stop-Loading-Everything-into-Memory-for-60-80-Cost-Reduction" class="common-anchor-header">2.停止將所有資料載入記憶體，以降低 60%-80% 的成本<button data-href="#2-Stop-Loading-Everything-into-Memory-for-60-80-Cost-Reduction" class="anchor-icon" translate="no">
    ...
    </button></h2><p>即使選擇了更有效率的索引，您記憶體中的資料仍可能多於所需。Milvus 提供兩種方法來解決這個問題：<strong>MMap (自 2.3 起可用) 和分層儲存 (自 2.6 起可用)。兩者都可以減少 60-80% 的記憶體使用量。</strong></p>
...
<p>查詢成本也以相同的方式縮放 - 余弦相似度為 O(D)，因此 768 維向量每次查詢所需的運算量大約是 384 維向量的兩倍。在高 QPS 的工作負載中，這個差異會直接轉換成所需的節點數。</p>
...
<p>這對以下情況很有用</p>
<ul>
<li>日誌和會話資料 - 只保留最近 7 或 30 天的資料</li>
<li>時間敏感的 RAG - 偏好最近的知識，讓舊文件過期</li>
<li>即時建議 - 僅擷取最近的使用者行為</li>
</ul>
<p>壓縮和 TTL 可讓您的系統不會默默地累積浪費。這並不是最大的成本槓桿，但它可以防止緩慢的儲存爬升，讓團隊措手不及。</p>
...
<p>如果您正在對成本敏感的工作負載上執行 HNSW，請改用 IVF_SQ8。單是這一點就可以將記憶體成本降低 ~70%，而召回損失卻微乎其微。</p>
...
<li><strong>設定壓縮和 TTL</strong>- 防止無聲資料膨脹，尤其是在您經常寫入/刪除的情況下。</li>
</ul>
<p>結合這些策略，可以讓您的向量資料庫費用減少 60-80%。並非每個團隊都需要所有四個策略 - 從索引變更開始，衡量影響，然後順著清單往下走。</p>
<p>對於希望減少作業工作並提高成本效益的團隊而言，<a href="https://zilliz.com/">Zilliz Cloud</a>(受管理的 Milvus) 是另一個選擇。</p>

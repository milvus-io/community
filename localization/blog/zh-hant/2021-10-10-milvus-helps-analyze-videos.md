...
<p>從傳統的影像處理演算法發展到深度神經網路 (DNN)，目前物體偵測的主流模型包括 R-CNN、FRCNN、SSD 和 YOLO。本主題所介紹的以 Milvus 為基礎的深度學習影像分析系統，可以智慧且快速地偵測到物體。</p>
<h3 id="Implementation" class="common-anchor-header">執行</h3><p>為了偵測並辨識視訊中的物件，系統必須先從視訊中擷取畫面，並使用物件偵測功能偵測畫面影像中的物件；其次，從偵測到的物件中擷取特徵向量；最後，根據特徵向量來分析物件。</p>
<ul>
<li>畫面擷取</li>
</ul>

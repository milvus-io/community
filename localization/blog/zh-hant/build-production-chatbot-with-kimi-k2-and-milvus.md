...
<h2 id="Import-Libraries-and-Basic-Configuration" class="common-anchor-header">匯入程式庫與基本設定<button data-href="#Import-Libraries-and-Basic-Configuration" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在這裡，PYMILVUS 是 Milvus 向量資料庫運作的函式庫，而 openai 則是用來呼叫 Kimi 和 OpenAI API（Kimi K2 的 API 與 OpenAI 和 Anthropic 相容的好處在這裡顯而易見）。</p>
...
<li><p><strong>定義結構</strong>：三個欄位：id (主索引)、text (文字)、vector (向量)</p></li>
...
<h2 id="Intelligent-Decision-Making-SmartAssistant-Class" class="common-anchor-header">智慧型決策：SmartAssistant 類別<button data-href="#Intelligent-Decision-Making-SmartAssistant-Class" class="anchor-icon" translate="no">
    ...
    </button></h2><p>這是系統的大腦，也稱為智慧決策中心。這是 Kimi K2 自主推理能力真正發揮的地方 - 它不只是執行預先定義的工作流程，而是真正理解使用者的意圖，並針對何時使用何種工具做出智慧型決策。</p>
<p>此處的設計理念是要創造一個自然語言介面，讓人感覺像是在與知識淵博的助理對話，而不是透過語音指令來操作資料庫。</p>
...
<p>基本工具 (4)：</p>
<p><code translate="no">connect_database</code> - 資料庫連線管理<code translate="no">create_collection</code> - 資料集建立<code translate="no">add_documents</code> - 批量新增文件<code translate="no">list_all_collections</code> - 資料集管理</p>
<p>搜尋工具 (1)：</p>
<p><code translate="no">search_documents</code> - 在指定的合集中搜尋</p>
<p>檔案工具 (2)：</p>
<p><code translate="no">read_and_chunk_file</code> - 檔案預覽與分塊<code translate="no">upload_file_to_collection</code> - 檔案上傳處理</p>
...
<p>遇到錯誤時，Kimi K2 也知道應該先建立集合 (create_collection)，然後上傳檔案到集合 (upload_file_to_collection)，並根據錯誤訊息及時修正。與傳統的腳本方式相比，這種自主錯誤復原是一個關鍵優勢。</p>
<p>
  <span class="img-wrapper">
...
<h2 id="The-Dawn-of-Production-AI-Agents" class="common-anchor-header">生產型 AI 代理的曙光<button data-href="#The-Dawn-of-Production-AI-Agents" class="anchor-icon" translate="no">
    ...
    </button></h2><p>透過連接<strong>Kimi K2</strong>與<strong>Milvus</strong>，我們已經超越了傳統的聊天機器人或基本語意搜尋。我們所建立的是一個真正的生產型代理程式，它可以解讀複雜的指令，將其分解為以工具為基礎的工作流程，並以最低的開銷執行端對端任務，例如檔案處理、語意搜尋和智慧型問答。</p>

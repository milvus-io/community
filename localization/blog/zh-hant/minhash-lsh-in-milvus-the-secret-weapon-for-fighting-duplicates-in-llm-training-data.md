...
<h3 id="Step-2-Identifying-Similar-Documents-via-LSH" class="common-anchor-header">步驟 2：透過 LSH 識別相似文件</h3><p>即使使用精簡的 MinHash 簽署，比較數百萬或數十億個文件中的每一對，計算成本仍然很高。這就是<strong>Locality Sensitive Hashing (LSH)</strong>的用武之地。</p>
...
<p>關鍵的想法是：高度相似的文件在其 MinHash 簽署中會有許多匹配的雜湊值。當這些簽章被分割成頻帶時，即使有一個頻帶具有所有的匹配值，也足以將兩個文件放入同一個資料桶中。文件越相似，至少有一個區段出現這種情況的機率就越高，這使得 LSH 可以有效率地浮現候選配對（而無需徹底比較所有簽章）。</p>

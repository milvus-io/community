...
<p><strong>然而，有一個問題卻毀掉了這種氣氛：AI 助手經常會產生過時的程式碼，而這些程式碼在生產過程中會發生問題。</strong>這是因為這些工具的 LLM 經常依賴過時的訓練資料。即使是最聰明的 AI 副駕駛員，也可能會建議落後一年或三年的程式碼。您可能會使用不再適用的語法、已經過時的 API 呼叫，或是現今的框架所不鼓勵的作法。</p>
...
<p>更糟糕的是，當我要求 OpenAI API 程式碼時，Cursor 使用<code translate="no">gpt-3.5-turbo</code>產生了一個片段 - 這個模型現在已被 OpenAI 標示為<em>Legacy</em>，其價格是其後者的三倍，但結果卻比較差。該代碼還依賴<code translate="no">openai.ChatCompletion</code> ，這是一個在 2024 年 3 月就已廢棄的 API。</p>
...
<p>儘管在 Vibe Coding 工具上有這麼多的進步，開發人員仍要花費大量的時間來銜接已產生的程式碼與生產就緒的解決方案之間的「最後一哩路」。氛圍是有了，但準確性卻沒有。</p>
...
<p>當然有。透過結合 Model Context Protocol (MCP) 與 Retrieval-Augmented Generation (RAG)，我們創造了一個稱為<strong>Milvus MCP</strong> 的增強型解決方案。它可以幫助使用 Milvus SDK 的開發人員自動存取最新的文件，讓他們的 IDE 產生正確的程式碼。這項服務即將推出，以下是其背後的架構。</p>
...
<p>上圖顯示一個混合系統，結合了 MCP (Model Context Protocol) 與 RAG (Retrieval-Augmented Generation) 架構，協助開發人員產生正確的程式碼。</p>
...
<p>在這個範例中，我要求 Cursor 使用<code translate="no">pymilvus</code> 產生全文檢索程式碼。Cursor 成功調用正確的 MCP 工具，並輸出符合規格的程式碼。大部分<code translate="no">pymilvus</code> 用例都能與此工具無縫配合。</p>
...
<h2 id="Comparing-Milvus-MCP-with-Context7-DeepWiki-and-Other-Tools" class="common-anchor-header">Milvus MCP 與 Context7、DeepWiki 及其他工具的比較<button data-href="#Comparing-Milvus-MCP-with-Context7-DeepWiki-and-Other-Tools" class="anchor-icon" translate="no">
    ...
    </button></h2><p>我們已經在 Vibe Coding 中討論過 「最後一哩 」的幻覺問題。除了我們的 Milvus MCP，許多其他工具也以解決這個問題為目標，例如 Context7 和 DeepWiki。這些工具通常由 MCP 或 RAG 驅動，協助將最新的文件和程式碼範例注入模型的上下文視窗。</p>
...
<p>Context7 為 LLM 和 AI 程式碼編輯器提供最新、特定版本的文件和程式碼範例。它解決的核心問題是，LLMs 依賴您所使用的函式庫的過時或通用資訊，為您提供過時且基於一年前的訓練資料的程式碼範例。</p>
...
<p>DeepWiki 可自動解析開放原始碼 GitHub 專案，以建立可閱讀的技術文件、圖表和流程圖。它包含一個自然語言問答的聊天介面。然而，它優先處理程式碼檔案而非文件，因此可能會忽略文件中的重要觀點。它目前缺乏 MCP 整合。</p>
...
<h3 id="llmstxt" class="common-anchor-header">llms.txt</h3><p><code translate="no">llms.txt</code> 不是一個工具，它是一個建議的標準，提供 LLM 結構化的網站內容。通常，在 Markdown 中，它會放在網站的根目錄中，並組織標題、文件樹、教學、API 連結等。</p>
...
<p>對於 Milvus 開發人員來說，這意味著不再需要調試被淘汰的<code translate="no">connections.connect()</code> 呼叫或與過時的 ORM 模式搏鬥。這三種工具-pymilvus-code-generator、orm-client-code-convertor 和 language-translator 能自動處理最常見的痛點。</p>

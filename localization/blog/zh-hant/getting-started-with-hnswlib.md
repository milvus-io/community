...
<ol start="3">
<li><strong>跳過清單結構（Skip List-Like Structure）：</strong>HNSW 的層級結構類似於跳躍清單 (skip list)，這是一種概率資料結構，其中較高層級的節點較少，可加快初始搜尋速度。</li>
</ol>
<p>
  <span class="img-wrapper">
...
<li><p><strong>擴充性：</strong>可順利擴充至數百萬個資料點，非常適合推薦系統等中等規模的應用程式。</p></li>
</ul>
...
<ul>
<li><code translate="no">ef_construction=200</code>:此參數控制建立索引時準確性與建立速度的取捨。較高的值可提高召回率 (精確度)，但會增加記憶體使用量和建立時間。常見值範圍從 100 到 200。</li>
</ul>
<ul>
...
<li><p><strong>分散式架構：</strong>Milvus 採用分散式、可擴充的架構，將儲存與運算分開在多個節點上，以提供彈性與穩健性。</p></li>
...
<tr><td style="text-align:center">索引</td><td style="text-align:center">支援 10 種以上的索引演算法，包括 HNSW、DiskANN、量化和二進位。</td><td style="text-align:center">僅使用基於圖形的 HNSW</td></tr>

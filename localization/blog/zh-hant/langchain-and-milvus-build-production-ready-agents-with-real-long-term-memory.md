...
<p>在這篇文章中，我們將探討 LangChain 1.0 如何更新代理體架構，以及整合 Milvus 如何幫助代理體超越推理 - 為真實世界的使用案例實現持久、智慧的記憶體。</p>
<h2 id="Why-the-Chain-based-Design-Falls-Short" class="common-anchor-header">基於 Chain 的設計為何失敗<button data-href="#Why-the-Chain-based-Design-Falls-Short" class="anchor-icon" translate="no">
    ...
    </button></h2><p>在早期（0.x 版本），LangChain 的架構是以 Chain 為中心。每個 Chain 定義了固定的順序，並隨附預先建立的範本，讓 LLM 的協調變得簡單快速。這種設計非常適合快速建立原型。但隨著 LLM 生態系統的演進，以及實際使用案例的複雜性，此架構開始出現裂縫。</p>
<p><strong>1.缺乏彈性</strong></p>
<p>LangChain 的早期版本提供了模組化的管道，例如 SimpleSequentialChain 或 LLMChain，每個管道都遵循固定、線性的流程 - 建立提示 → 模型呼叫 → 輸出處理。這種設計對於簡單且可預測的任務非常有效，而且容易快速建立原型。</p>
...
<p>在示範中運作良好的東西，到了生產中往往就壞了。Chains 並未包含大型、持久性或敏感應用程式所需的保障措施。常見的問題包括</p>
<ul>
...
<p>無論是在多重代理系統中，或是在單一代理執行深入推理時，都會出現相同的控制循環：透過調用目標工具交替執行簡短的推理步驟，然後將觀察結果納入後續決策中，直到代理能夠提供最終答案為止。</p>
...
<h3 id="2-The-Middleware-A-Composable-Layer-for-Production-Ready-Control" class="common-anchor-header">2.中間件：生產就緒控制的可組合層</h3><p>中間件是將 LangChain 從原型帶到生產的關鍵橋梁。它在代理的執行迴圈中的策略點暴露鉤子，讓您可以在不重寫核心 ReAct 程序的情況下加入自訂邏輯。</p>
...
<p>事實上，LangChain 1.0 與 LangGraph 1.0 可視為同一層疊的一部分，其設計目的在於共同運作而非互相取代：LangChain 可協助您快速建立標準的代理，而 LangGraph 則可讓您針對複雜的工作流程進行精細的控制。換句話說，LangChain 可協助您快速行動，而 LangGraph 則可協助您深入瞭解。</p>
...
<h2 id="Conclusion" class="common-anchor-header">總結<button data-href="#Conclusion" class="anchor-icon" translate="no">
    ...
    </button></h2><p>三年前，LangChain 以呼叫 LLM 的輕量級封裝程式起家。如今，它已發展成一個完整的生產級框架。</p>
<p>在核心層中，中介軟體層提供了安全性、合規性和可觀測性。LangGraph 增加了持久執行、控制流程和狀態管理。而在記憶體層，<a href="https://milvus.io/">Milvus</a>填補了一個重要的缺口 - 提供可擴充、可靠的長期記憶體，讓代理可以擷取上下文、推理歷史，並隨時間改進。</p>

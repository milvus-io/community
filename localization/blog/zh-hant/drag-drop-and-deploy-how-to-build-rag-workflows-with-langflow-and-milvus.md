...
<p><strong>Milvus</strong> 是開放原始碼的向量資料庫，可讓 LLM 具備長期記憶與上下文理解能力，搭配<strong>Milvus</strong>，兩者便可構成生產級 RAG 的完整環境。Milvus 可以有效地儲存和擷取企業或特定領域資料中的嵌入，讓 LLMs 可以產生有根據、準確且能感知上下文的答案。</p>
...
<p>Langflow 也包含一個豐富的元件庫，其中有適用於熱門 LLM 與向量資料庫 (包括<a href="https://milvus.io/">Milvus)</a> 的預先建立的適配器。您可以針對特殊的使用個案，建立自訂的 Python 元件，進一步擴充。對於測試和最佳化，Langflow 提供逐步執行、快速測試的 Playground，以及與 LangSmith 和 Langfuse 的整合，以監控、除錯和重播端對端的工作流程。</p>
